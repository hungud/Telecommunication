object ReportProfitFrm: TReportProfitFrm
  Left = 203
  Top = 134
  Caption = #1054#1090#1095#1105#1090' '#1086' '#1076#1086#1093#1086#1076#1085#1086#1089#1090#1080
  ClientHeight = 375
  ClientWidth = 1311
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -14
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  PopupMode = pmAuto
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 120
  TextHeight = 16
  object grData: TCRDBGrid
    Left = 0
    Top = 34
    Width = 1311
    Height = 341
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Align = alClient
    DataSource = DataSource1
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -15
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ReadOnly = True
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = [fsBold]
    OnDblClick = aShowUserStatInfoExecute
    OnKeyDown = grDataKeyDown
    Columns = <
      item
        Expanded = False
        FieldName = 'ACCOUNT_NUMBER'
        Title.Alignment = taCenter
        Title.Caption = #1057#1095#1077#1090
        Width = 60
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'COMPANY_NAME'
        Title.Alignment = taCenter
        Title.Caption = #1053#1072#1079#1074#1072#1085#1080#1077' '#1082#1086#1084#1087#1072#1085#1080#1080
        Width = 136
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PHONE_NUMBER'
        Title.Alignment = taCenter
        Title.Caption = #1058#1077#1083#1077#1092#1086#1085
        Width = 81
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'phone_status'
        Title.Alignment = taCenter
        Title.Caption = #1057#1090#1072#1090#1091#1089
        Width = 60
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TARIFF_NAME'
        Title.Alignment = taCenter
        Title.Caption = #1058#1077#1082#1091#1097#1080#1081' '#1090#1072#1088#1080#1092#1085#1099#1081' '#1087#1083#1072#1085
        Width = 162
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BILL_YEAR'
        Title.Alignment = taCenter
        Title.Caption = #1043#1086#1076
        Width = -1
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'PROFIT_01'
        Title.Alignment = taCenter
        Title.Caption = #1071#1085#1074#1072#1088#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_02'
        Title.Alignment = taCenter
        Title.Caption = #1060#1077#1074#1088#1072#1083#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_03'
        Title.Alignment = taCenter
        Title.Caption = #1052#1072#1088#1090
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_04'
        Title.Alignment = taCenter
        Title.Caption = #1040#1087#1088#1077#1083#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_05'
        Title.Alignment = taCenter
        Title.Caption = #1052#1072#1081
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_06'
        Title.Alignment = taCenter
        Title.Caption = #1048#1102#1085#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_07'
        Title.Alignment = taCenter
        Title.Caption = #1048#1102#1083#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_08'
        Title.Alignment = taCenter
        Title.Caption = #1040#1074#1075#1091#1089#1090
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_09'
        Title.Alignment = taCenter
        Title.Caption = #1057#1077#1085#1090#1103#1073#1088#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_10'
        Title.Alignment = taCenter
        Title.Caption = #1054#1082#1090#1103#1073#1088#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_11'
        Title.Alignment = taCenter
        Title.Caption = #1053#1086#1103#1073#1088#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_12'
        Title.Alignment = taCenter
        Title.Caption = #1044#1077#1082#1072#1073#1088#1100
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_FOR_YEAR'
        Title.Alignment = taCenter
        Title.Caption = #1047#1072' '#1075#1086#1076
        Width = 53
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BALANCE'
        Title.Alignment = taCenter
        Title.Caption = #1041#1072#1083#1072#1085#1089
        Width = 58
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PROFIT_WITH_BALANCE'
        Title.Alignment = taCenter
        Title.Caption = #1044#1086#1093#1086#1076#1085#1086#1089#1090#1100
        Width = 67
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1311
    Height = 34
    Margins.Left = 4
    Margins.Top = 4
    Margins.Right = 4
    Margins.Bottom = 4
    Align = alTop
    TabOrder = 1
    object Label1: TLabel
      Left = 226
      Top = 9
      Width = 61
      Height = 16
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Caption = #1055#1077#1088#1080#1086#1076':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lAccount: TLabel
      Left = 10
      Top = 9
      Width = 75
      Height = 16
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Caption = #1051#1080#1094'. '#1089#1095#1077#1090':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object BitBtn1: TBitBtn
      Left = 377
      Top = 0
      Width = 112
      Height = 36
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Action = aExcel
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
    end
    object BitBtn2: TBitBtn
      Left = 489
      Top = 0
      Width = 112
      Height = 36
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Action = aRefresh
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
    end
    object BitBtn3: TBitBtn
      Left = 601
      Top = 0
      Width = 112
      Height = 36
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Action = aShowUserStatInfo
      Caption = #1055#1086#1076#1088#1086#1073#1085#1086
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
    end
    object cbPeriod: TComboBox
      Left = 293
      Top = 5
      Width = 76
      Height = 24
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Style = csDropDownList
      TabOrder = 3
      OnChange = cbPeriodChange
    end
    object cbSearch: TCheckBox
      Left = 905
      Top = 8
      Width = 74
      Height = 21
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Caption = #1055#1086#1080#1089#1082
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      OnClick = cbSearchClick
    end
    object cbAccounts: TComboBox
      Left = 94
      Top = 5
      Width = 120
      Height = 24
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Style = csDropDownList
      TabOrder = 5
      OnChange = cbAccountsChange
    end
    object CheckBox1: TCheckBox
      Left = 721
      Top = 8
      Width = 176
      Height = 21
      Margins.Left = 4
      Margins.Top = 4
      Margins.Right = 4
      Margins.Bottom = 4
      Caption = #1059#1095#1080#1090#1099#1074#1072#1090#1100' '#1087#1083#1072#1090#1077#1078#1080
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -15
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      OnClick = CheckBox1Click
    end
  end
  object qReport: TOraQuery
    SQL.Strings = (
      'SELECT MV.*, '
      '       AC.ACCOUNT_NUMBER,'
      '       AC.COMPANY_NAME,'
      '       T.TARIFF_NAME'
      '   FROM MV_PROFIT_BY_PHONES_YEAR MV,'
      '        ACCOUNTS AC,'
      '        TARIFFS t '
      '  WHERE BILL_YEAR=:BILL_YEAR'
      '    AND ((:ACCOUNT_ID IS NULL)OR(MV.ACCOUNT_ID=:ACCOUNT_ID))'
      '    AND MV.ACCOUNT_ID = AC.ACCOUNT_ID'
      
        '    AND GET_CURR_PHONE_TARIFF_ID(MV.PHONE_NUMBER) = T.TARIFF_ID ' +
        '(+)')
    FetchRows = 250
    ReadOnly = True
    BeforeOpen = qReportBeforeOpen
    Left = 80
    Top = 112
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'BILL_YEAR'
      end
      item
        DataType = ftUnknown
        Name = 'ACCOUNT_ID'
      end>
  end
  object DataSource1: TDataSource
    DataSet = qReport
    Left = 152
    Top = 176
  end
  object ActionList1: TActionList
    Images = MainForm.ImageList24
    Left = 80
    Top = 64
    object aExcel: TAction
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' MS Excel'
      ImageIndex = 12
      OnExecute = aExcelExecute
    end
    object aRefresh: TAction
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      ImageIndex = 9
      OnExecute = aRefreshExecute
    end
    object aShowUserStatInfo: TAction
      Caption = #1055#1086#1076#1088#1086#1073#1085#1086
      Hint = #1055#1088#1086#1089#1084#1086#1090#1088' '#1080#1085#1092#1086#1088#1084#1072#1094#1080#1080' '#1087#1086' '#1072#1073#1086#1085#1077#1085#1090#1091
      ImageIndex = 22
      ShortCut = 13
      OnExecute = aShowUserStatInfoExecute
    end
  end
  object qPeriods: TOraQuery
    SQL.Strings = (
      'SELECT '
      '  V_PROFIT_BY_PHONES_YEAR.BILL_YEAR'
      'FROM V_PROFIT_BY_PHONES_YEAR'
      'GROUP BY V_PROFIT_BY_PHONES_YEAR.BILL_YEAR'
      'ORDER BY V_PROFIT_BY_PHONES_YEAR.BILL_YEAR DESC')
    ReadOnly = True
    Left = 176
    Top = 112
  end
  object dsPeriods: TDataSource
    AutoEdit = False
    DataSet = qPeriods
    Left = 248
    Top = 72
  end
  object qAccounts: TOraQuery
    SQL.Strings = (
      'SELECT ACCOUNT_ID, LOGIN '
      'FROM ACCOUNTS '
      'ORDER BY LOGIN ASC')
    Left = 176
    Top = 64
  end
  object qFastPeriods: TOraQuery
    SQL.Strings = (
      
        'select distinct substr(YY.YEAR_MONTH,1,4 ) BILL_YEAR from DB_LOA' +
        'DER_BILLS YY'
      'order by BILL_YEAR desc')
    ReadOnly = True
    Left = 240
    Top = 120
  end
end
