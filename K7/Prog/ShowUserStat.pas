...
uses
  ...
  dmShowUserStat, Vcl.OleCtrls, SHDocVw, DMUnit, UnLogProtokObPlat;
...
type
 ...
 mySMTP = class(TThread)
  ...
  TShowUserStatForm = class(TForm)
    ...
    bRefreshSIM: TButton;
    Splitter2: TSplitter;
    gSummaryPhone_Snapshots: TCRDBGrid;
    ...
    pcLog: TPageControl;
    tbLogJournal: TTabSheet;
    tsDealedLog: TTabSheet;
    gDELAYED_ON_OFF_TARIFF_OPTIONS: TCRDBGrid;
    lbVirtBalance: TLabel;
    pDailyAbonPay: TPanel;
    pMCBalance: TPanel;
    lSas: TLabel;
    lMK: TLabel;
    bmcRefresh: TButton;
    lSASBal: TLabel;
    lMKBal: TLabel;
    btDetailFromBillCollector: TBitBtn;
    aAddPromisedPayment: TAction;
    aHide_detail: TAction;
    Label3: TLabel;
    lbSumObPlat: TLabel;
    Label5: TLabel;
    lbDatObPlat: TLabel;
    ...
    tsOldDetails: TTabSheet;
    tsNewDetails: TTabSheet;
    Panel24: TPanel;
    Panel25: TPanel;
    crdbgDetailMonths: TCRDBGrid;
    Splitter4: TSplitter;
    Splitter5: TSplitter;
    Panel26: TPanel;
    Panel27: TPanel;
    crdbgDetailData: TCRDBGrid;
    crdbgDetailStat: TCRDBGrid;
    btDetailStatOut: TBitBtn;
    BitBtn9: TBitBtn;
    BitBtn11: TBitBtn;
    btDetailFromBillsCollector: TBitBtn;
    btSendToEMail: TBitBtn;
    btUnloadCallsFromBill: TBitBtn;
    btUnbilledCallsList: TButton;
    cbHideDetail: TCheckBox;
    cbZeroCalls: TCheckBox;
    CheckBox3: TCheckBox;
    CheckBox4: TCheckBox;
    Panel28: TPanel;
    btSetFilter: TsBitBtn;
    dtpBeginDate: TDateTimePicker;
    dtpEndDate: TDateTimePicker;
    eOldDetailMail: TEdit;
    lBeginDate: TLabel;
    lEndDate: TLabel;
    btDetailDataOut: TBitBtn;
    lbLimitWithOutGPRS_U: TLabel;
    pChangePP: TPanel;
    cbAllTariffs: TDBLookupComboBox;
    btChangePP: TBitBtn;
    Label6: TLabel;
    DBText2: TDBText;
    Label8: TLabel;
    ...
    procedure dsDetailMonthsDataChange(Sender: TObject; Field: TField);
    ...
    procedure qSummaryPhoneSnapshotsBeforeOpen(DataSet: TDataSet);
    ...
    procedure aMPExcelExecute(Sender: TObject);
    procedure aMPRefreshExecute(Sender: TObject);
    procedure tsNewDetailsShow(Sender: TObject);
    procedure TabSheet1Exit(Sender: TObject);
    procedure aMPAddPayExecute(Sender: TObject);
    procedure PageControl2Change(Sender: TObject);
    procedure dpStartDateCloseUp(Sender: TObject);
    procedure dpEndDateCloseUp(Sender: TObject);
    procedure pcLogChange(Sender: TObject);
    procedure bmcRefreshClick(Sender: TObject);
	  procedure DBText13Click(Sender: TObject);
    procedure btDetailFromBillCollectorClick(Sender: TObject);
    procedure aAddPromisedPaymentExecute(Sender: TObject);
    procedure aHide_detailExecute(Sender: TObject);
    procedure BitBtn8Click(Sender: TObject);
    procedure crdbgDetailStatDrawDataCell(Sender: TObject; const Rect: TRect;
      Field: TField; State: TGridDrawState);
    procedure crdbgDetailStatGetCellParams(Sender: TObject; Field: TField;
      AFont: TFont; var Background: TColor; State: TGridDrawState;
      StateEx: TGridDrawStateEx);
    procedure btDetailDataOutClick(Sender: TObject);
    procedure btDetailStatOutClick(Sender: TObject);
    procedure cbZeroCallsClick(Sender: TObject);
    procedure btDetailFromBillsCollectorClick(Sender: TObject);
    procedure btSetFilterClick(Sender: TObject);
    procedure btSendToEMailClick(Sender: TObject);
    procedure btChangePPClick(Sender: TObject);
  private
    FYear : Integer;
    FMonth : Integer;
    FDetailText : String;
...
uses ContractsRegistration_Utils, IntecExportGrid, CRStrUtils, SendSms,
  ...
  PassWord, ShowUserTurnOnOffOption, StrUtils, ReportDetailAPI,ShellAPI, Func_Const;
...
const qHBDetailsStrTableNameNum = 23;
var cbexcept:integer=1;
    cbexcept1:integer=1;
    qexcept:TOraQuery;
    qexcept1:TOraQuery;
...
var ShowUserStatForm: TShowUserStatForm;
    AdminPriv : boolean;
begin
  try
...
procedure ShowUserStatByPhoneNumber(const pPhoneNumber : String;
  const vContractID : Integer; const PageName : String = '');
begin
...
var IsAdmin: boolean;
    ...
    afontColor: integer;
begin
  // Кидаем значения в глобальные переменные формы.
  ...
  IS_BILL_CHECK.Visible:=true;
  if GetConstantValue('ENABLE_INTERNET_KABINET') = '1'
    then btSetPassword.Show
    else btSetPassword.Hide;
  if GetConstantValue('ENABLE_BLOCK_BY_SAVE') = '1'
    then btBlockBySave.Visible := True
    else btBlockBySave.Visible := False;
  //договор
  ...
  try
    ...
    begin
      ...
      if dmShowUserSt.qContractInfo.FieldByName('DISCONNECT_LIMIT').AsInteger = 0
        then lCreditLimit.Visible:=false
        else lCreditLimit.Caption:= 'Кредитный лимит:    '
             + dmShowUserSt.qContractInfo.FieldByName('DISCONNECT_LIMIT').AsString + ' р.';
    end;
     //заполнение информации об отрезках времени для смс
    ...
    begin
      ...
      begin
        ...
        try
          ...
          ShowMessage('Ошибка получения данных qContractInfo!');
        end;
        ValueType := vtTime;
    ...
    //Название компании
    lCompanyName.Caption := dmShowUserSt.qContractInfo.FieldByName('COMPANY_NAME').AsString;
  finally
  ...
  //
   dmShowUserSt.qBlocked.ParamByName('phonenum').AsString := FPhoneNumber;
   dmShowUserSt.qBlocked.Open;
  //
  dmShowUserSt.qTariffInfo.ParamByName('PHONE_NUMBER').AsString := FPhoneNumber;
  if dmShowUserSt.qContractInfo.FieldByName('CONTRACT_CANCEL_DATE').AsString='' then
    dmShowUserSt.qTariffInfo.ParamByName('pBALANCE_DATE').Clear
  else
    dmShowUserSt.qTariffInfo.ParamByName('pBALANCE_DATE').AsDateTime := dmShowUserSt.qContractInfo.FieldByName('CONTRACT_CANCEL_DATEcontract_cancel_date').AsDateTime;

  // Открытие перенесено в
  //qTariffInfo.Open;
  //Групповой баланс
  if GetConstantValue('SHOW_CONTRACT_GROUPS') = '1' then
  begin
    dmShowUserSt.qBalanceGroup.ParamByName('PHONE_NUMBER').AsString := FPhoneNumber;
    dmShowUserSt.qBalanceGroup.ParamByName('CONTRACT_ID').AsInteger := FContractID;
    ...
    begin
      ...
      //в группах своё управлением временем отсылки смс
      //и ручной блок
      SetSmsDisableTime.Enabled:=false;
      smstime.Hint:='Эта настройка является групповой';
      smstime.ShowHint:=true;
      spSMS.Visible:=true;
      pnlNewHB.Enabled:=false;
      pnlShowHB.Hint:='Ручной блок ставится в управлении группой.';
  ...
  //
  dmShowUserSt.qPassword.ParamByName('CONTRACT_ID').AsInteger:=FContractID;
  // Платежи
  dmShowUserSt.qPayments.ParamByName('PHONE_NUMBER').AsString := FPhoneNumber;
  //
  dmShowUserSt.qPhoneStatuses.ParamByName('PHONE_NUMBER').AsString := FPhoneNumber;
  dmShowUserSt.qPhoneStatuses.Open;
  //  заявки БИ.
  dmShowUserSt.oqApiTickets.ParamByName('PHONE').AsString := FPhoneNumber;
  dmShowUserSt.oqApiTickets.Open;
  // логи загрузок АПИ
  dmShowUserSt.oqApi_log.ParamByName('PHONE').AsString := FPhoneNumber;
  //oqApi_log.Open;
  //
  ...
  // Депозиты
  if (GetConstantValue('USE_DEPOSITES') = '1')and(FContractID>0) then
  ...
  // Расшифровка баланса, перенесена в tsBalanceRows.OnShow;
  //RefreshBalanceRows;
  //
  ...
  begin
    ...
    begin
      //btnAddPromisedPayment.Enabled := false;
      aAddPromisedPayment.Enabled := false;
    end;
  ...
  //показываем ограничение по подключению GPRS_U
  dmShowUserSt.qShowLimitGPRS_U.ParamByName('pPHONE_NUMBER').AsString := FPhoneNumber;
  dmShowUserSt.qShowLimitGPRS_U.Close;
  dmShowUserSt.qShowLimitGPRS_U.Open;
  lbLimitWithOutGPRS_U.Visible := false;
  if not dmShowUserSt.qShowLimitGPRS_U.IsEmpty then
    lbLimitWithOutGPRS_U.Visible := true;
...
begin
  ...
  dmShowUserSt.spCalcBalanceRows.ParamByName('pBALANCE_DATE').Value :=
    dmShowUserSt.qContractInfo.FieldByName('CONTRACT_CANCEL_DATE').Value;
  if dmShowUserSt.qContractInfo.FieldByName('CONTRACT_CANCEL_DATE').Value > DATE then
     dmShowUserSt.spCalcBalanceRows.ParamByName('pBALANCE_DATE').Clear;
...
procedure TShowUserStatForm.TabSheet1Exit(Sender: TObject);
begin
  dmShowUserSt.qRepMP.Close;
end;

procedure TShowUserStatForm.tsNewDetailsShow(Sender: TObject);
begin
  dmShowUserSt.qDetailMonths.ParamByName('PHONE_NUMBER').AsString:=FPhoneNumber;
  dmShowUserSt.qDetailMonths.Close;
  dmShowUserSt.qDetailMonths.Open;
  dmShowUserSt.qDetailData.ParamByName('PHONE_NUMBER').AsString:=FPhoneNumber;
  dmShowUserSt.qDetailStat.ParamByName('PHONE_NUMBER').AsString:=FPhoneNumber;
  dtpBeginDate.DateTime:=Date;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          dmShowUserSt.dsDetail.Enabled := true;
          stt:=inttostr(FMonth);
          if length(stt)=1 then stt:='0'+stt;
...
procedure TShowUserStatForm.dsDetailMonthsDataChange(Sender: TObject; Field: TField);
var MonthStr:string;
    i:integer;
begin
  dmShowUserSt.qDetailData.Close;
  if dmShowUserSt.qDetailMonths.RecordCount> 0
    then MonthStr:=Copy(dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsString, 1, 4)
      + '_' + Copy(dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsString, 5, 2)
    else MonthStr:='2016_01';
  for i:=0 to dmShowUserSt.qDetailData.SQL.Count-1 do
    if Pos('FROM CALL_', dmShowUserSt.qDetailData.SQL[i]) > 0 then
      dmShowUserSt.qDetailData.SQL[i]:='  FROM CALL_' +  MonthStr + ' C,';
  dmShowUserSt.qDetailData.ParamByName('PHONE_NUMBER').AsString:=FPhoneNumber;
  dmShowUserSt.qDetailData.Open;
  dmShowUserSt.qDetailStat.Close;
  dmShowUserSt.qDetailStat.ParamByName('PHONE_NUMBER').AsString:=FPhoneNumber;
  dmShowUserSt.qDetailStat.ParamByName('YEAR_MONTH').AsString:=
    dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsString;
  dmShowUserSt.qDetailStat.Open;
end;
...
begin
  ...
  //  FormStyle:=fsMDIChild;
    ...
    Caption:='История смены доп. статусов по номеру '+FPhoneNumber;
  ...
  try
    ...
    tbh.Columns.Items[0].Title.caption:='Номер';
    tbh.Columns.Items[1].Title.caption:='Статус старый';
    tbh.Columns.Items[2].Title.caption:='Статус новый';
    tbh.Columns.Items[3].Title.caption:='Пользователь';
    tbh.Columns.Items[4].Title.caption:='Дата';
    ...
    if not qbh.Eof
      then fList.ShowModal
      else ShowMessage('Нет данных.');flist.Destroy;
  end;
end;
...
begin
  if FormatSQL=''
    then FormatSQL := dmShowUserSt.oqApi_log.SQL.Text;
  if dmShowUserSt.oqApi_log.Active
    then dmShowUserSt.oqApi_log.Close;
  if trunc(DpLogAPI.Date)=trunc(now)
    then TableName := 'BEELINE_SOAP_API_LOG'
    else TableName:='ext_'+Format('%.2d', [dayof(DpLogAPI.Date)])+'_BEELINE_SOAP_API_LOG';
...
procedure TShowUserStatForm.TDBBlueOn(Sender: TObject);
begin
  (sender as TDBText).Font.Color:=clblue;
  LblBalance.ShowHint:=TRUE;
end;
...
procedure TShowUserStatForm.TDBBlueOff(Sender: TObject);
begin
  (sender as TDBText).font.color:=clblack;
end;
...
procedure TShowUserStatForm.mcRefresh;
begin
  lSASBal.Caption := '';
  lMKBal.Caption := '';

  dmShowUserSt.qMCBalance.Close;
  dmShowUserSt.qMCBalance.ParamByName('PHONE_NUMBER').Value := FPhoneNumber;
  dmShowUserSt.qMCBalance.Open;

  lSASBal.Caption := dmShowUserSt.qMCBalance.FieldByName('BSASVALUE').AsString;
  lMKBal.Caption := dmShowUserSt.qMCBalance.FieldByName('BMKVALUE').AsString;
end;
...
procedure TShowUserStatForm.DBText13Click(Sender: TObject);
var
  qBH:TOraQuery;
  vTARIFF_URL :  String;
begin
try
  qBH:=TOraQuery.Create(nil);
    with qBH do begin
      params.CreateParam(ftString,'p_phone',ptInput);
      params.ParamByName('p_phone').Value:=FPhoneNumber;
        sql.Add('select t.TARIFF_URL from TARIFFS t,DB_LOADER_ACCOUNT_PHONES DB where db.phone_number=:p_phone and t.TARIFF_ID(+)=GET_PHONE_TARIFF_ID(DB.PHONE_NUMBER,DB.CELL_PLAN_CODE,DB.LAST_CHECK_DATE_TIME) and DB.YEAR_MONTH = to_number( to_char(sysdate, ''yyyymm'') )');
      Execute;
      vTARIFF_URL:=FieldByName('TARIFF_URL').AsString;
    end;
  finally
    qbh:=nil;
   end;
  if Trim(vTARIFF_URL) <> '' then
    ShellExecute(0, 'open',PWidechar(vTARIFF_URL), '', '', SW_SHOW)
  else
    ShowMessage('В справочнике тарифов не задана ссылка на описание тарифа!');
end;
...
procedure TShowUserStatForm.dpEndDateCloseUp(Sender: TObject);
begin
  GetAVGPay;
end;

procedure TShowUserStatForm.dpStartDateCloseUp(Sender: TObject);
begin
  GetAVGPay;
end;
...
begin
  ...
  if (Sender as TPageControl).ActivePage.Name='tsOptions' then
    pcLog.ActivePage := tbLogJournal;

  if (Sender as TPageControl).ActivePage.Name='tsPayments' then
  begin
    try
      sumpay:=0;
      if not gPayments.DataSource.DataSet.active then
        gPayments.DataSource.DataSet.open;
  ...
  begin
    ...
    begin
      ...
      if ((FieldByName('TYPE_VISIBILITY').AsInteger=1) and not (cbHideDetail.Checked))
          or ((FieldByName('TYPE_VISIBILITY').AsInteger=0) and (cbHideDetail.Checked)) then
      begin
        cbexcept1:=-1;
        if cbHideDetail.Checked
          then cbHideDetail.State:=cbUnChecked
          else cbHideDetail.State:=cbChecked;
        crdbgDetailData.Visible:=cbHideDetail.Checked;
        btUnloadCallsFromBill.Visible:=cbHideDetail.Checked;
        btDetailDataOut.Visible:=cbHideDetail.Checked;
        btDetailStatOut.Visible:=cbHideDetail.Checked;
        btSendToEMail.Visible:=cbHideDetail.Checked;
        btUnbilledCallsList.Visible:=cbHideDetail.Checked;
      end;
    end;
  end;
...
procedure TShowUserStatForm.PageControl2Change(Sender: TObject);
begin
  if (Sender as TPageControl).ActivePage.Name = 'tsMobPay' then
    mpRefresh;
end;

procedure TShowUserStatForm.pcLogChange(Sender: TObject);
begin
  if pcLog.ActivePage = tbLogJournal then
  begin
    dmShowUserSt.qAPIOptionHistory.Close;
    dmShowUserSt.qAPIOptionHistory.Open;
  end
  else if pcLog.ActivePage = tsDealedLog then
  begin
    dmShowUserSt.qDELAYED_ON_OFF_TARIFF_OPTIONS.Close;
    dmShowUserSt.qDELAYED_ON_OFF_TARIFF_OPTIONS.ParamByName('PHONE_NUMBER').Value := FPhoneNumber;
    dmShowUserSt.qDELAYED_ON_OFF_TARIFF_OPTIONS.Open;
  end;
end;
...
procedure TShowUserStatForm.aAddPromisedPaymentExecute(Sender: TObject);
var Itog: string;
    PromisedSum: double;
...
begin
  { Тип прав пользователей :
    1 - администратор,
    2 - просмотр,
    3 - только инф. по абоненту, иначе обычный пользователь';
  }
  if not TryStrToInt(ePromisedSum.Text, vSum) then
  begin
    ShowMessage('Введите правильно сумму платежа');
    Exit;
  end;
  PromisedSum := vSum;
  { Пользователям с правами "Просмотр" должна быть возможность
    добавления ОП только на 200р, не больше не меньше, строго
    такую сумму и желательно одной кнопкой.
  }
  if ((MainForm.UserRightsType=2) and (PromisedSum<>200)) then
  begin //  ADD_PROMISED_PAYMENT
    ePromisedSum.Text:='200';
    ShowMessage('Для пользователей с правами "Просмотр" '+#13+
                'доступно добавление ОП только на 200р.');
    Exit;
  end;
  ...
  try
    ...
    ShowMessage('Введите правильно срок платежа');
    Exit;
  end;

  MaxSumVar := GetMaxPROMISED_PAYMENT(dm.OraSession.Username);
  ...
  if ((MainForm.UserRightsType=3) and (UpperCase(dm.OraSession.Username) <> UpperCase(GetParamValue('USER_TELETIE_EXCEPTION')))) then
  begin
    if (PromisedSum > MaxSum) then
    begin
     ShowMessage('Сумма платежа не может быть больше ' + floattostr(MaxSum) + ' рублей. Текущее значение: '+FloatToStrF(PromisedSum, ffNumber, 15, 2));
     Exit;
    end;
    if (Trunc(PromisedDade)<>Trunc((Now+4))) then
    begin
     ShowMessage('Дата может быть только '+DateToStr(Now+4)+'. Текущая дата - '+DateToStr(PromisedDade));
     Exit;
    end;
  end;
  if (MainForm.UserRightsType=2) then
  begin
    if (PromisedSum > MaxSum) then
    begin
     ShowMessage('Сумма платежа не может быть больше ' + floattostr(MaxSum) + ' рублей. Текущее значение: '+FloatToStrF(PromisedSum, ffNumber, 15, 2));
    ...
    if (Trunc(PromisedDade)>Trunc((Now+4))) then
    begin
     ShowMessage('Дата может не быть больше '+DateToStr(Now+4)+'. Текущая дата - '+DateToStr(PromisedDade));
     Exit;
    end;
  end;
  try
    ...
    Itog:='Не удалось добавить обещанный платеж';
  end;
  ShowMessage(Itog);
...
begin
  ...
  begin
    dmShowUserSt.vtDetailFile.Filter:='SERVICE_COST<>"0" AND SERVICE_DATE >='''
      +FormatDateTime('DD.MM.YYYY',dtBegin.Date)+''' AND SERVICE_DATE <='''
      +FormatDateTime('DD.MM.YYYY',dtEnd.Date)+'''';
    dmShowUserSt.vtDetailFile.Filtered:=true;
    dmShowUserSt.qHBDetails.Filter:='SERVICE_COST<>"0" AND SERVICE_DATE >='''
      +FormatDateTime('DD.MM.YYYY',dtBegin.Date)+''' AND SERVICE_DATE <='''
      +FormatDateTime('DD.MM.YYYY',dtEnd.Date)+'''';
  ...
  begin
    dmShowUserSt.vtDetailFile.Filter:='SERVICE_DATE >='''
      +FormatDateTime('DD.MM.YYYY',dtBegin.Date)+''' AND SERVICE_DATE <='''
      +FormatDateTime('DD.MM.YYYY',dtEnd.Date)+'''';
    dmShowUserSt.vtDetailFile.Filtered:=false;
    dmShowUserSt.qHBDetails.Filter:='SERVICE_DATE >='''
      +FormatDateTime('DD.MM.YYYY',dtBegin.Date)+''' AND SERVICE_DATE <='''
      +FormatDateTime('DD.MM.YYYY',dtEnd.Date)+'''';
    dmShowUserSt.qHBDetails.Filtered:=true;
  end;
...
procedure TShowUserStatForm.aHide_detailExecute(Sender: TObject);
begin
  inc(cbexcept1);
  if cbexcept1=0 then
    exit;
  ...
  begin
    ...
    crdbgDetailData.Visible:=not(cbHideDetail.Checked);
    btUnloadCallsFromBill.Visible:=not(cbHideDetail.Checked);
    btDetailDataOut.Visible:=not(cbHideDetail.Checked);
    btDetailStatOut.Visible:=not(cbHideDetail.Checked);
    btSendToEMail.Visible:=not(cbHideDetail.Checked);
    btUnbilledCallsList.Visible:=not(cbHideDetail.Checked);
    if cbHideDetail.Checked
      then Post;
  end;
  qexcept1.Refresh;
...
begin
  ...
  begin
    ...
    then
//      ShowMessage('Платёж поступил позже даты закрытия контракта, прикрепить его нельзя. Для прикрепления необходимо изменить дату закрытия контракта.',mtInfo)
    MessageDlg('Платёж поступил позже даты закрытия контракта, прикрепить его нельзя. Для прикрепления необходимо изменить дату поступления оплаты (обратитесь к администратору).', mtWarning, [mbOk], 0)
...
procedure TShowUserStatForm.aMPAddPayExecute(Sender: TObject);
begin // MP add pay
  if Trim(FPhoneNumber) <> '' then
  begin
    if MessageDlg('Добавить платеж на телефон '+FPhoneNumber+' ?', mtConfirmation, mbYesNo, 0) = idYes then
    begin
      dmShowUserSt.pMOBPAY.ParamByName('PPHONEA').AsString:=FPhoneNumber;
      // FUser:=OraSession.Username;
      dmShowUserSt.pMOBPAY.ParamByName('USSD_USER').AsString:=MainForm.FUser;
      dmShowUserSt.pMOBPAY.ExecProc;
      ShowMessage(dmShowUserSt.pMOBPAY.ParamByName('RESULT').AsString);
    end;
  end;
end;

procedure TShowUserStatForm.aMPExcelExecute(Sender: TObject);
var cr : TCursor;
begin
  cr := Screen.Cursor;
  Screen.Cursor := crHourGlass;
  try
    ExportDBGridToExcel2(Caption+' платежи mobpay' , '',
...
procedure TShowUserStatForm.mpRefresh;
begin
  dmShowUserSt.qRepMP.Close;
//  dmShowUserSt.qRepMP.SQLRefresh.Text := 'select * from MOB_PAY t where t.phone=:p_phone order by t.date_pay';
  dmShowUserSt.qRepMP.ParamByName('p_phone').AsString := FPhoneNumber;
  try
    dmShowUserSt.qRepMP.Open;
    dmShowUserSt.qRepMP.First;
  except
   on E:Exception do
     ShowMessage('Произошла ошибка : '+#13#10+E.Message);
  end;
end;

procedure TShowUserStatForm.aMPRefreshExecute(Sender: TObject);
begin //
  mpRefresh;
end;
...
begin
  ...
  begin
    ...
    BalanceVirt := dmShowUserSt.qTariffInfo.FieldByName('VIRT_BALANCE').AsFloat; //виртуальный баланс
    //если на номере тп с признаком автоподключения пакета, то показываем виртуальный баланс, если он <> реальному
    if (dmShowUserSt.qTariffInfo.FieldByName('IS_AUTO_INTERNET').AsInteger = 1) and (Balance <> BalanceVirt) then
    begin
      lbVirtBalance.Top := lBalance.Top+2;
      lbVirtBalance.Caption := '('+FloatToStrF(BalanceVirt, ffNumber, 15, 2)+')';
      lBalance.Left := LblBalance.Left + LblBalance.Width + 3;
      lbVirtBalance.Left := lBalance.Left + lBalance.Width + 3;
    end
    else
      lbVirtBalance.Caption := '';
  ...
  begin
    ...
    // для коллеторских номеров показываем надпись коллектор
    ...
    btDetailFromBillCollector.Visible:=lbl_IS_COLLECTOR.Visible;
...
begin
  ...
  //and FormatDateTime('DD.MM.YYYY',dtBegin.Date)<=EndOfTheMonth(StrToDate('01.'+inttostr(AMonth)+'.'+inttostr(AYear))) then begin
    ShowMessage('Ошибка введения начальной даты.');
    result:=false;
  end;
  if not((dtEnd.Date>=Strtodate(tempS)) and (dtEnd.Date<=EndOfTheMonth(Strtodate(tempS)))) then
  begin
  //and FormatDateTime('DD.MM.YYYY',dtBegin.Date)<=EndOfTheMonth(StrToDate('01.'+inttostr(AMonth)+'.'+inttostr(AYear))) then begin
    ShowMessage('Ошибка введения конечной даты.');
    result:=false;
  end;
end;
...
begin
  TurnOnOffOption(FPhoneNumber, 'D', dmShowUserSt.qOptions.FieldByName('OPTION_CODE').AsString, 0, lbl_IS_COLLECTOR.Visible);
  dmShowUserSt.qAPIOptionHistory.Close;
  dmShowUserSt.qAPIOptionHistory.Open;
end;
...
procedure TShowUserStatForm.crdbgDetailStatDrawDataCell(Sender: TObject;
  const Rect: TRect; Field: TField; State: TGridDrawState);
begin
//
end;

procedure TShowUserStatForm.crdbgDetailStatGetCellParams(Sender: TObject;
  Field: TField; AFont: TFont; var Background: TColor; State: TGridDrawState;
  StateEx: TGridDrawStateEx);
begin
  if dmShowUserSt.qDetailStatIS_ROAMING.AsString = '666'
    then AFont.Color:=clRed;
  if dmShowUserSt.qDetailStatSERVICE_ID.AsString = '-1'
    then AFont.Style:=AFont.Style + [fsUnderLine];
  if dmShowUserSt.qDetailStatC_ACT_CD.AsString = '-1'
...
//подготовка к выгрузке Excel
  ...
  begin
    dmShowUserSt.spGetSIGNATURE_MAIL_ABOUT_DETAIL.ParamByName('PPHONENUMBER').Value := FPhoneNumber;
    dmShowUserSt.spGetSIGNATURE_MAIL_ABOUT_DETAIL.ExecProc;

    ANote :=  dmShowUserSt.spGetSIGNATURE_MAIL_ABOUT_DETAIL.ParamByName('RESULT').AsString;
  ...
  try
    ...
    ExportDBGrid(ACaption, ANote, ANameFile, dbgDetail, False, True);
  finally
    Screen.Cursor := cr;
  end;
...
//   VBODY.Add(ANote);
  ...
  //Передача в поток отправки почты
  ...
  begin
    ...
    SMTP_TITLE:='Детализация по номеру '+FPhoneNumber;
    SMTP_FROMTXT:=GetParamValue('SMTP_FROMTEXT');
    SMTP_BODY :=ANote;
//      SMTP_BODY:=TStringList.Create;
//      SMTP_BODY.Add(ANote);
    SMTP_ATT_FILE := FullFileName;
    Resume;
  end;
 //  FreeAndNil(VBODY);
  ...
  begin
    try
      ...
      sql.Add ('values(to_char(sysdate,''YYYYMM''),'+FPhoneNumber+', sysdate, '+'''Детализация по номеру '+FPhoneNumber+''','''+edEmailAdress.Text+'''||'' User:''||(user))');
      ExecSQL;
    except
      on e : exception do
...
procedure TShowUserStatForm.qSummaryPhoneSnapshotsBeforeOpen(DataSet: TDataSet);
begin
  dmShowUserSt.qSummaryPhone_Snapshots.ParamByName('PHONE_NUMBER').AsString := FPhoneNumber;
end;
...
begin
  ...
  begin
    //cbEXCLUDE_DETAIL.Visible:=true; -- таска 1350
...
begin
  ...
  begin
    dmShowUserSt.qContractInfo.Open;
    if GetConstantValue('SHOW_DEALERS') = '1' then
    begin
      MainForm.CheckAdminPrivs(AdminPriv);
      if not(AdminPriv) then
  ...
  if (UpperCase(DMUnit.Dm.OraSession.Username) = 'MAX')
    or (UpperCase(DMUnit.Dm.OraSession.Username) = 'CORP_MOBILE') then
  begin
    dmShowUserSt.qAllTariffs.Close;
    dmShowUserSt.qAllTariffs.Open;
  end else
    pChangePP.Hide;
end;
...
begin
  ...
  if (MainForm.UserRightsType=2) or (MainForm.UserRightsType=3) then
   ...
   //19610 - информация о МН роуминге
  MN_ROAMING;
...
begin
  if (Pos(LowerCase(MainForm.CheckUser),
          LowerCase(GetConstantValue('USER_ACCESS_HIDE_DETAIL'))) > 0) then
  begin
    cb_Hide_detail.Visible:=True;
    cb_Hide_detail.Enabled:=True;
    cbHideDetail.Visible:=True;
    cbHideDetail.Enabled:=True;
  end;
...
begin
  ...
  begin
    ...
    if rec then RowCount := RowCount + 1;

    inc(i);
    Cells[0, i] := 'Б/пл минут исх';
    ...
    inc(i);
    Cells[0, i] := 'Платных минут (по мин.)';
    ...
    inc(i);
    RowCount := RowCount + 1;
    Cells[0, i] := 'Платных минут (по сек.)';
    ...
    RowCount := RowCount + 1;
    inc(i);
    Cells[0, i] := 'Исх. Б/платных SMS и MMS';
...
begin
  ...
  dmShowUserSt.qDopPhoneInfo.Open;
    ...
    //MainForm.CheckAdminPrivs(AdminPriv);
    //if not(AdminPriv) then
  //показываем/скрываем кнопку обновления номера сим карты
  bRefreshSIM.Visible := True;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      dmShowUserSt.qHandBlock.ParamByName('CONTRACT_ID').AsString :=FPhoneNumber;
      dmShowUserSt.qHandBlock.ParamByName('CONTRACT_ID').AsString := VarToStr(FContractID); // +VartoStr(FContractID));
  ...
  if (FContractID <> 0)and(GetConstantValue('SHOW_CONTRACT_DOP_STATUS') = '1') then
  begin
    ...
    if not(AdminPriv or ROPriv or (MainForm.UserRightsType=3))
      then cbDopStatus.Enabled:=false;
  end
  else
  begin
    cbDopStatus.Hide;
  ...
  //Для K7-если ручная блокировка видна, то делаем ее доступной только для админов
  ...
  try
    qBH:=TOraQuery.Create(nil);
    qBH.Session := Dm.OraSession ;
    qBH.params.CreateParam(ftString,'p_phone',ptInput);
    qBH.params.ParamByName('p_phone').Value:=FPhoneNumber;
//        '  where trunc(pp.payment_date,''dd'') = '22.05.2015' -- trunc(sysdate) '+
//        '  AND PP.PROMISED_DATE >= '23.10.2012'-- trunc(sysdate) '+   // '9688987000' '+
       // ' AND PP.PROMISED_DATE >= trunc(sysdate) '+
  {  qBH.sql.Add('select * from ( '+
        '  select pp.promised_sum, pp.promised_date from PROMISED_PAYMENTS pp '+
        ' where PP.PAYMENT_DATE >= trunc(sysdate)  '+
        ' and ((pp.promised_date = PROMISED_DATE_END) or '+
        ' ((pp.promised_date <> PROMISED_DATE_END) and (pp.promised_date >= sysdate))) '+
        '  and pp.phone_number = :p_phone '+
        '  order by pp.promised_sum desc, pp.promised_date desc '+
        '  ) where rownum=1 ');   }

    qBH.sql.Add('select pp.promised_sum, pp.promised_date ' +
      '  from V_ACTIVE_PROMISED_PAYMENTS pp ' +
      '  where pp.phone_number = :p_phone');
    qBH.Execute;
    if not qBH.Eof then
    begin
      lbSumObPlat.Caption := FloatToStr(qBH.FieldByName('promised_sum').AsFloat);
      lbDatObPlat.Caption := DateToStr(DateOf(qBH.FieldByName('promised_date').AsDateTime));
      lbSumObPlat1.Caption := lbSumObPlat.Caption;
      lbDatObPlat1.Caption := lbDatObPlat.Caption;
      lbSumObPlat.Visible:=true;
      lbDatObPlat.Visible:=true;
      lbSumObPlat1.Visible:=true;
      lbDatObPlat1.Visible:=true;
      Label3.Visible:=true;
      Label5.Visible:=true;
      Label4.Visible:=true;
      Label7.Visible:=true;
    ...
    begin
      lbSumObPlat.Visible:=false;
      lbDatObPlat.Visible:=false;
      lbSumObPlat1.Visible:=false;
      lbDatObPlat1.Visible:=false;
...
begin
  ...
  begin
    if btPostDopStatus.Enabled then begin
      ShowMessage('Выбран, но не применён доп. статус!');
      exit;
    end;
    if dmShowUserSt.qContractInfo_DopStatus.FieldByName('DOP_STATUS').AsString<>'' then
    begin
      ShowMessage('Нельзя разблокировать телефон при наличии доп. статуса!');
      exit;
    end;
  end;
...
var
  ...
  buttonSelected : Integer;
begin
  buttonSelected:=MessageDlg('Вы действительно хотите отправить абоненту СМС сообщение с его балансом?', mtCustom, mbOKCancel, 0);
  if buttonSelected = mrCancel then
    exit;
  ...
  try
    ...
    if DetailText<>'' then
      DetailText := 'Ошибка отправки.'#13#10 + DetailText;
  except
    on E : Exception do
    begin
      DetailText := 'Ошибка отправки.'#13#10 + E.Message;
    end;
  end;
  if DetailText='' then
...
procedure TShowUserStatForm.btSendToEMailClick(Sender: TObject);
var cr : TCursor;
    ACaption, ANote, ANameFile, vRemark, tempStr, FullFileName : string;
    Qlog_writer: TOraQuery;
    vPaymentSum :integer;
    vFillialId:integer;
begin
//подготовка к выгрузке Excel
  ...
  ACaption := 'Детализация по номеру ' + FPhoneNumber
    + ' за ' + Copy(dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsString, 5, 2)
    + ' - ' + Copy(dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsString, 1, 4);
  ANameFile := FPhoneNumber+'_Detail_'
    + Copy(dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsString, 1, 4) + '_'
    + Copy(dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsString, 5, 2) + '_';
  ...
  try
    ...
    //          ExportDBGridToExcel(ACaption,ANameFile,
    //          dbgDetail, False, True,true);
    ExportDBGrid(ACaption, ANote, ANameFile, crdbgDetailData, False, True);
  finally
    Screen.Cursor := cr;
  end;
 ...
 //  FreeAndNil(VBODY);
  ...
  begin
    try
      ...
      sql.Add('insert into log_send_mail ');
      sql.Add('(year_month, phone_number, date_send, mail_subject,note)');
      sql.Add('values(to_char(sysdate,''YYYYMM''),' + FPhoneNumber+', sysdate, '
        + '''Детализация по номеру ' + FPhoneNumber + ''','''
        + edEmailAdress.Text + '''||'' User:''||(user))');
      ExecSQL;
    except
      on e : exception do
        MessageDlg('Ошибка вставки записи в таблицу логирования отправки сообщений(log_send_mail)', mtError, [mbOK], 0);
...
procedure TShowUserStatForm.AddDetailDataFilter;
begin
  if cbZeroCalls.Checked then
  begin
    dmShowUserSt.qDetailData.Filter:='CALL_COST <> 0 '
      + 'AND TRUNC(CH_SEIZ_DT) >=TO_DATE('''
      + FormatDateTime('DD.MM.YYYY HH24',dtpBeginDate.Date)
      + ''', ''DD.MM.YYYY'') AND TRUNC(CH_SEIZ_DT) <=TO_DATE('''
      + FormatDateTime('DD.MM.YYYY',dtpEndDate.Date)+''', ''DD.MM.YYYY'')';
    dmShowUserSt.qDetailData.Filtered:=true;
  end else
  begin
    dmShowUserSt.qDetailData.Filter:='CH_SEIZ_DT >='''
      + FormatDateTime('DD.MM.YYYY', dtpBeginDate.Date)
      + ' 00:00:00'' AND CH_SEIZ_DT <'''
      + FormatDateTime('DD.MM.YYYY',dtpEndDate.Date)+' 23:59:59''';
    dmShowUserSt.qDetailData.Filtered:=true;
  end;
end;

procedure TShowUserStatForm.btSetFilterClick(Sender: TObject);
begin
  AddDetailDataFilter;
end;
...
begin
  ...
  begin
    if emNewDateEnd.Text = '  .  .  ' then begin
      MessageDlg('Введите дату окончания ручной блокировки!',mtError, [mbOK], 0);
      exit;
    end;
...
begin
  MainForm.CheckAdminPrivs(vCheck);
  vRes := MessageDlg('Сменить группу услуг?', mtConfirmation, [mbYes, mbNo, mbCancel], 0);
...
begin
  ...
  if MainForm.UserRightsType=2 then   // Пользователям с правами "Просмотр" убрать возможность останавливать текущий ОП
  begin
    ShowMessage('Недоступно для пользователей с правами "Просмотр".');
    Exit;
  end;
  Itog:='';
  try
    ...
    Itog:='Не удалось удалить обещанный платеж';
  end;
  ShowMessage(Itog);
  dmShowUserSt.qPrPayments.Close;
  dmShowUserSt.qPrPayments.Open;
...
procedure TShowUserStatForm.btDetailFromBillCollectorClick(Sender: TObject);
begin
  DoReportCollectorBillDetails(FPhoneNumber,
      dmShowUserSt.qPeriods.FieldByName('YEAR').AsInteger*100
      + dmShowUserSt.qPeriods.FieldByName('MONTH').AsInteger);
end;

procedure TShowUserStatForm.btDetailFromBillsCollectorClick(Sender: TObject);
begin
  DoReportCollectorBillDetails(FPhoneNumber,
      dmShowUserSt.qDetailMonths.FieldByName('YEAR_MONTH').AsInteger);
end;
...
begin
  Try
    ...
    dmShowUserSt.qContractInfo_DopStatus.Open;
    btPostDopStatus.Enabled:=false;
   // Send_mail_dop_status;//19610 - отправка E-mail о смене доп.статуса
  Except
    MessageDlg('Изменение не произведено!',mtError, [mbOK], 0);
  End;
end;
...
procedure TShowUserStatForm.btDetailDataOutClick(Sender: TObject);
var cr : TCursor;
    ACaption : String;
    ANameFile : string;
    grid : TCRDBGrid;
...
begin
  cr := Screen.Cursor;
  Screen.Cursor := crHourGlass;
  grid := crdbgDetailData;
  ACaption := 'Детализация по номеру ' + FPhoneNumber
    + ' за ' + dmShowUserSt.qPeriods.FieldByName('MONTH').AsString
    + ' - ' + dmShowUserSt.qPeriods.FieldByName('YEAR').AsString;
  ANameFile:=FPhoneNumber + '_Detail_'
    + dmShowUserSt.qPeriods.FieldByName('YEAR').AsString + '_'
    + dmShowUserSt.qPeriods.FieldByName('MONTH').AsString + '_';
  try
    for i :=0 to grid.Columns.Count-1 do
      if Pos(grid.Columns[i].FieldName,
          'SUBSCR_NO;CH_SEIZ_DT;CH_SEIZ_TM;SERVICE_NAME_SHORT;'
            + 'C_ACT_DESC_RUS_SHORT;SRV_FT_DESC;CALLING_NO;DIALED_DIG;'
            + 'AT_CDRRD_M;AT_FT_DESC;ROAMING_TYPE_NAME;COUNTRY_CODE;') > 0
...
procedure TShowUserStatForm.btDetailStatOutClick(Sender: TObject);
var cr : TCursor;
    ACaption : String;
    ANameFile : string;
    grid : TCRDBGrid;
begin
  cr := Screen.Cursor;
  Screen.Cursor := crHourGlass;
  grid := crdbgDetailStat;
  ACaption := 'Статистика детализация по номеру ' + FPhoneNumber
    + ' за ' + dmShowUserSt.qPeriods.FieldByName('MONTH').AsString
    + ' - ' + dmShowUserSt.qPeriods.FieldByName('YEAR').AsString;
  ANameFile:=FPhoneNumber + '_Detail_statistic_'
    + dmShowUserSt.qPeriods.FieldByName('YEAR').AsString + '_'
    + dmShowUserSt.qPeriods.FieldByName('MONTH').AsString + '_';
  try
    ExportDBGrid(ACaption, '', ANameFile, grid, True,True);
...
procedure TShowUserStatForm.btChangePPClick(Sender: TObject);
begin
  dmShowUserSt.spChangePP.ParamByName('PPHONE_NUMBER').AsString:=FPhoneNumber;
  dmShowUserSt.spChangePP.ParamByName('PNEWTARIFFCODE').AsString:=
    cbAllTariffs.ListSource.DataSet.FieldByName('CELL_PLAN_CODE').AsString;
  dmShowUserSt.spChangePP.ParamByName('PFUTUREDATE').AsString:='N';
  if mrOk = MessageDlg('Изменить тариф на номере ' + FPhoneNumber + ' на тариф "'
      + cbAllTariffs.ListSource.DataSet.FieldByName('TARIFF_NAME').AsString + '" с кодом "'
      + cbAllTariffs.ListSource.DataSet.FieldByName('CELL_PLAN_CODE').AsString + '"?',
      mtConfirmation, [mbOK, mbCancel], 0) then
  begin
    dmShowUserSt.spChangePP.ExecProc;
    ShowMessage(dmShowUserSt.spChangePP.ParamByName('RESULT').AsString);
  end;
{ begin
  :RESULT := BEELINE_API_PCKG.CHANGEPP(:PPHONE_NUMBER, :PNEWTARIFFCODE, :PFUTUREDATE);
end;
}
end;
...
procedure TShowUserStatForm.BitBtn8Click(Sender: TObject);
begin // протокол обещанных платежей
  FrmLogProtokObPlat := TFrmLogProtokObPlat.Create(nil);
  try
    FrmLogProtokObPlat.FMnPhoneNumber := FPhoneNumber;
    FrmLogProtokObPlat.ShowModal;
  finally
    FreeAndNil(FrmLogProtokObPlat);
  end;
end;
...
begin
  ...
  begin
    vMN_ROAMING:=0;
    vMN_ROAMING_TEXT:='МН Роуминг запрещен';
  end;

  if dmShowUserSt.qContractInfo.FieldByName('MN_ROAMING').AsInteger<> 1 then
  begin
    vMN_ROAMING:=1;
    vMN_ROAMING_TEXT:='МН Роуминг разрешен';
  end;
  ...
  try
    ...
    on e : exception do
      MessageDlg('Ошибка вставки записи в таблицу контрактов '+sql_text, mtError, [mbOK], 0);
  end;
  ...
  sql_text:='INSERT INTO MANY_LOGS (LOG_SOURCE_ID,PHONE_NUMBER,TEXT_MESSAGES,LOGIN_SESSION, DATE_ON )'+
           ' VALUES ( 11,'+FPhoneNumber+','''+vMN_ROAMING_TEXT+''','''+ dm.OraSession.Username+''', sysdate )';
...
begin
  DoReportBillsAPI(FPhoneNumber);
end;

procedure TShowUserStatForm.bmcRefreshClick(Sender: TObject);
begin
  mcRefresh;
end;

procedure TShowUserStatForm.B_ecareClick(Sender: TObject);
var webform:TFormWeb;
begin
  ...
  try
    ...
    begin
      ...
      ShowMessage('Этот номер не имеет тех.данных для отображения.');
  finally
    FreeAndNil(webform);
  end;
end;
...
begin
  try
    ...
    qTemp.ExecSQL;
    qTemp.Free;
    Application.MessageBox(PChar('Добавлен в список проверки баланса.'),
...
begin
  ...
  begin
    ...
    begin
      ...
      if dmShowUserSt.qContractInfo_DopStatus.FieldByName('DOP_STATUS').AsString='' then
      begin
        ShowMessage('Нельзя блокировать телефон при отсутствии доп. статуса!');
        exit;
      end;
    end;
...
begin
  ...
  try
    ...
    on E : Exception do
    begin
      DetailText := 'Ошибка обновления номера SIM карты.'#13#10 + E.Message;
    end;
  end;

  if DetailText='OK!'  then
  begin
    ShowMessage('Номер сим карты обновлен!');
    dmShowUserSt.qDopPhoneInfo.Close;
    dmShowUserSt.qDopPhoneInfo.Open;
  end
  else
...
begin
  cr := Screen.Cursor;
  Screen.Cursor := crHourGlass;
  ACaption := 'Периоды тарификации абонентской платы по номеру ' + FPhoneNumber;
  try
    ExportDBGridToExcel(ACaption,'', grAbonPeriodList, False, True);
  finally
    Screen.Cursor := cr;
  end;
end;
...
begin
  TurnOnOffOption(FPhoneNumber, 'A', '', dmShowUserSt.qTariffInfo.FieldByName('TARIFF_ID').AsInteger, lbl_IS_COLLECTOR.Visible);
  dmShowUserSt.qAPIOptionHistory.Close;
  dmShowUserSt.qAPIOptionHistory.Open;
end;
...
begin
  cr := Screen.Cursor;
  Screen.Cursor := crHourGlass;
  ACaption := 'Детальные счета Билайна по номеру ' + FPhoneNumber;
  try
    ExportDBGridToExcel(ACaption, '', grBillBeeline, False, True);
  finally
    Screen.Cursor := cr;
  end;
end;
...
begin
  cr := Screen.Cursor;
  Screen.Cursor := crHourGlass;
  ACaption := 'Расшифровка счетов по номеру ' + FPhoneNumber;
  try
    ExportDBGridToExcel(ACaption, '', CRDBGridClientBills, False, True);
  finally
    Screen.Cursor := cr;
  end;
end;
...
begin
  cr := Screen.Cursor;
  Screen.Cursor := crHourGlass;
  ACaption := 'Детальная расшифровка счетов для клиента по номеру ' + FPhoneNumber;
  try
    ExportDBGridToExcel(ACaption, '', grBillClientNew, False, True);
  finally
    Screen.Cursor := cr;
  end;
end;
...
procedure TShowUserStatForm.cbSnapshotClick(Sender: TObject);
var
  ch : boolean;
begin
  ch := cbSnapshot.Checked;
  gSummaryPhone_Snapshots.Visible := ch;
  Splitter2.Visible := ch;
  if ch then
  begin
    gSummaryPhone.Align := alLeft;
    gSummaryPhone.Width := 726;
    Splitter2.Left := gSummaryPhone.Width;
    dmShowUserSt.qSummaryPhone_Snapshots.Close;
    dmShowUserSt.qSummaryPhone_Snapshots.BeforeOpen := qSummaryPhoneSnapshotsBeforeOpen;
    dmShowUserSt.qSummaryPhone_Snapshots.MasterSource := dmShowUserSt.dsSummaryPhone;
    dmShowUserSt.qSummaryPhone_Snapshots.MasterFields := 'EVENT_DATE';
    dmShowUserSt.qSummaryPhone_Snapshots.Open;
  end
  else
  begin
    dmShowUserSt.qSummaryPhone_Snapshots.Close;
    dmShowUserSt.qSummaryPhone_Snapshots.BeforeOpen := nil;
    dmShowUserSt.qSummaryPhone_Snapshots.MasterSource := nil;
    dmShowUserSt.qSummaryPhone_Snapshots.MasterFields := '';
    gSummaryPhone.Align := alClient;
  end;
end;

procedure TShowUserStatForm.cbZeroCallsClick(Sender: TObject);
begin
  if cbZeroCalls.Checked then
  begin
    dmShowUserSt.qDetailData.Filter:='CALL_COST <> 0';
  ...
  begin
    dmShowUserSt.qDetailData.Filter:='';
    dmShowUserSt.qDetailData.Filtered:=false;
  end;
end;
...
begin
  ...
  s := GetParamValue('MONTH_COUNT_AVG_SUM');
  decCount := 0;
  try
    if s <> '' then decCount := StrToInt(s);
  finally
    //
  end;

  DecodeDate(IncMonth(Date, -decCount), myYear, myMonth, myDay);

  dpStartDate.Date := EncodeDate(myYear, myMonth, 1);
  dpEndDate.Date := Date;
  ...
  if GetConstantValue('SHOW_SEND_MAIL_INFO_ABON')='1' then
  begin
    btSendMail.Visible:=true;
    ...
    //cbFree.Visible:=true;
    btSendToEMail.Visible:=true;
    btSendToEMail.Enabled:=true;
    eOldDetailMail.Visible:=true;
    lBeginDate.Visible:=true;
    lEndDate.Visible:=true;
    dtpBeginDate.Visible:=true;
    dtpEndDate.Visible:=true;
    btSetFilter.Visible:=true;
  end;
  ...
  if (GetConstantValue('BLOCK_TYPES_ENABLE')='1') then
  begin
  ...
  if not isadm then B_ecare.Visible:=False;

  if GetConstantValue('CALC_ABON_PAYMENT_TO_MONTH_END') = '1' then
  begin
    pDailyAbonPay.Show;
    if IsAdm
      then cbDailyAbonPay.Enabled:=true
      else cbDailyAbonPay.Enabled:=false;
  end
  else
    pDailyAbonPay.Hide;

  //PageControl6.Pages[0].TabVisible := false;
end;
...
begin
  with dm do
  begin
    dm.qformaccess.Close;
    dm.qformaccess.ParamByName('FORM_NAME').AsString := 'ShowUserStatForm';
    dm.qformaccess.ParamByName('USER_NAME').AsString := dm.OraSession.Username;
    dm.qformaccess.ParamByName('RIGHTS_TYPE').AsInteger := MainForm.UserRightsType;
    dm.qformaccess.ParamByName('CHECK_ALLOW_ACCOUNT').AsInteger:=MainForm.Allow_account;
    dm.qformaccess.execsql;
    for I := 0 to dm.qFormAccess.RecordCount-1 do
    begin
      try
        //C := FindComponentex('ShowUserStatForm.'+qformaccess.FieldByName('COMPONENT_NAME').AsString);
        nameComp := qformaccess.FieldByName('COMPONENT_NAME').AsString;
        if (nameComp = 'btAddPromisedPayment') then
         a := aAddPromisedPayment;
        if (nameComp = 'cb_Hide_detail') or (nameComp = 'cbHideDetail') then
         a := aHide_detail;
        if qformaccess.FieldByName('IS_ACTIVE').AsInteger=1
          then a.Enabled:=true
          else a.Enabled:=false;
        if qformaccess.FieldByName('IS_VISIBLE').AsInteger=1
          then a.Visible:=true
...
procedure TShowUserStatForm.GetAVGPay;
var msgText : string;
begin
  msgText := '';
  if dpStartDate.Date < StrToDate(DBText20.Caption) then
    msgText := '"Дата начала" периода не может быть больше "Даты начала" договора '
      + DBText20.Caption
  else if dpStartDate.Date > dpEndDate.Date then
    msgText := '"Дата начала" периода не может быть больше "Даты окончания" периода'
  else if DBText15.Caption <> '' then
  begin
    if dpEndDate.Date > StrToDate(DBText15.Caption) then
      msgText := '"Дата окончания" периода не может быть больше "Дата завершения" договора '
        + DBText15.Caption
  end;

  if msgText <> '' then
  begin
    MessageBox(Handle, PChar(msgText), 'ВНИМАНИЕ!!!', mb_IconWarning);
    Exit;
  end;

  dmShowUserSt.spGET_AVERAGE_PAYMENT.ParamByName('P_PHONE_NUMBER').Value :=  FPhoneNumber;
  dmShowUserSt.spGET_AVERAGE_PAYMENT.ParamByName('P_DATE_FROM').Value :=  dpStartDate.Date;
  dmShowUserSt.spGET_AVERAGE_PAYMENT.ParamByName('P_DATE_TO').Value :=  dpEndDate.Date;
  dmShowUserSt.spGET_AVERAGE_PAYMENT.ExecProc;

  if dmShowUserSt.spGET_AVERAGE_PAYMENT.ParamByName('RESULT').IsNull then
    lblEveragePayments.Caption := '????'
  else
    lblEveragePayments.Caption:=FloatToStrF(dmShowUserSt.spGET_AVERAGE_PAYMENT.ParamByName('RESULT').AsFloat, ffNumber,15,2);
end;
...
var FChangeContractForm : TRegisterPaymentForm;
...
begin
  ...
  try
    try
      ...
      on e : exception do
        Application.MessageBox(PChar('Ошибка. Сообщение не отправлено!'#13#10
          +e.Message), 'Предупреждение', MB_OK+MB_ICONWARNING);
    end;
  finally
    //очищаем из памяти
    IdMessage1.Clear;
...
var vSMTP_ADDR, vSMTP_TITLE, vS : String;
    //c:integer;
    idsmtp1:TIdSMTP;
    idMessage1:TIdMessage;
    SqlOra : TOraQuery;
...
begin
        //19610
  f_Send:=false; // отправка письма
  f_Mess:=false; // сообщение на экран: true - письмо не отправляется, а выводится сообщение на экран
   ...
   begin
    vS:=FPhoneNumber+' просьба заблокировать по сохранению ';
    vSMTP_TITLE:=dm.OraSession.Username+' : Смена доп.статуса по номеру '+FPhoneNumber;
    f_Send:=true;
    end;

    if cbDopStatus.DataSource.DataSet.FieldByName('Dop_Status').AsInteger=3 then
   begin
    //vS:=FPhoneNumber+' просьба отключить все платные услуги и заблокировать по сохранению';
    vS:=FPhoneNumber+' просьба, очистить номер от платных услуг и оставить блокированным';
    vSMTP_TITLE:=dm.OraSession.Username+' : ОТКАЗ от номера '+FPhoneNumber;
    f_Send:=true;
    end;
   ...
   if f_Mess=True then
   begin
   MessageBox(Handle, '"НЕ ЗАБУДЬТЕ ЗАБЛОКИРОВАТЬ ПО СОХРАНЕНИЮ ЧЕРЕЗ 0628!!!"',
        ...
        begin
         ...
         IdSMTP1.Connect;
             try
              ...
              IdSMTP1.Disconnect;
              ShowMessage(vSMTP_TITLE+': отправлено!');
              //вставка информации об отправке в таблицу логирования
              SqlOra:=TOraQuery.Create(self);
                    ...
                    begin
                      try
                        ...
                        sql.Add ('insert into log_send_mail ');
                        sql.Add('(year_month, phone_number, date_send, mail_subject,note)');
                        sql.Add (' values(to_char(sysdate,''YYYYMM''),'+FPhoneNumber+', sysdate, '''+vS+' по номеру '+FPhoneNumber+''','''+vSMTP_ADDR+'''||'' User:''||(user))');
                        ExecSQL;
                      except
                        on e : exception do
                          MessageDlg('Ошибка вставки записи в таблицу логирования отправки сообщений(log_send_mail)', mtError, [mbOK], 0);
 ...
 //ShowMessage(MainForm.OraSession.Username+': отправлено!');
...
begin
  ...
  dmShowUserSt.dsDetailMonths.OnDataChange:= dsDetailMonthsDataChange;
end;

procedure TShowUserStatForm.SetSmsDisableTimeClick(Sender: TObject);
var Qsms_time: TOraQuery;
begin
  ...
  begin
    ...
    params.ParamByName('sms_dis_time').AsString:=FloatToStrF(smstime.MinData,ffNumber,8,8)
      +';'+FloatToStrF(smstime.MaxData,ffNumber,8,8)+';';
    ...
    try
      ...
      ShowMessage('Запрос не выполнился.');
    end;
    Close;
    sql.Clear;
  end;
...
begin
  ...
  if dmShowUserSt.qContractInfo.FieldByName('MN_ROAMING').AsInteger=1 then
  begin
    BitBtn_MN_Roaming.Caption:='Запретить МН Роуминг';
    LB_MN_Roaming.Caption:='МН Роуминг: разрешен';
  end
  else
  begin
    BitBtn_MN_Roaming.Caption:='Разрешить МН Роуминг';
    LB_MN_Roaming.Caption:='МН Роуминг: запрещен';
  end;
end;
    ...
    begin
      ...
      begin
        try
          ...
          sql.Add (' values(12, to_char(sysdate,''YYYYMM''),'+FPhoneNumber+', sysdate, ''изменение даты платежа №'+gPayments.Fields[2].AsString+' с '+gPayments.Fields[0].AsString+' на '+datetostr(MonthCalendar1.Date)+''','''+Dm.OraSession.Username+''')');
          ExecSQL;
        except
          on e : exception do
...
begin
  ...
  //  FormStyle:=fsMDIChild;
    BorderStyle:=bsSingle;
    Caption:='История баланса с типами по номеру '+FPhoneNumber;
    Position:=poScreenCenter;
    Width:=650;
    Height:=450;
  end;
  ...
  try
    ...
    tbh.Columns.Items[0].Title.caption:='Обновлен';
    tbh.Columns.Items[1].Title.caption:='Тип';
    tbh.Columns.Items[2].Title.caption:='Баланс';
    tbh.Columns.Items[3].Title.caption:='Создание';
    if not qbh.Eof then
      fList.ShowModal
    else
    begin
