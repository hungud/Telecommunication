object ReportNumWithoutAbonPayFrm: TReportNumWithoutAbonPayFrm
  Left = 426
  Top = 230
  Caption = #1054#1090#1095#1077#1090' '#1086' '#1085#1086#1084#1077#1088#1072#1093' '#1073#1077#1079' '#1090#1088#1072#1092#1080#1082#1086#1074' '#1089' '#1072#1073#1086#1085#1077#1085#1090#1089#1082#1086#1081' '#1087#1083#1072#1090#1086#1081
  ClientHeight = 345
  ClientWidth = 1120
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  PopupMode = pmAuto
  Position = poScreenCenter
  OnClose = FormClose
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object grData: TCRDBGrid
    Left = 0
    Top = 90
    Width = 1120
    Height = 255
    Align = alClient
    DataSource = DataSource1
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    PopupMenu = PopupMenu1
    ReadOnly = True
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
    OnDblClick = aShowUserStatInfoExecute
    OnKeyDown = grDataKeyDown
    Columns = <
      item
        Alignment = taCenter
        Expanded = False
        FieldName = 'LOGIN'
        Title.Alignment = taCenter
        Title.Caption = #1051'/'#1089
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 90
        Visible = True
      end
      item
        Alignment = taRightJustify
        Expanded = False
        FieldName = 'PHONE_NUMBER_FEDERAL'
        Title.Alignment = taCenter
        Title.Caption = #1058#1077#1083#1077#1092#1086#1085
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CONTRACT_DATE'
        Title.Alignment = taCenter
        Title.Caption = #1044#1072#1090#1072' '#1082#1086#1085#1090#1088#1072#1082#1090#1072
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 120
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BALANCE'
        Title.Alignment = taCenter
        Title.Caption = #1041#1072#1083#1072#1085#1089
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 75
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PHONE_IS_ACTIVE'
        Title.Alignment = taCenter
        Title.Caption = #1057#1090#1072#1090#1091#1089
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 172
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DOP_STATUS_NAME'
        Title.Alignment = taCenter
        Title.Caption = #1044#1086#1087'. '#1089#1090#1072#1090#1091#1089
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 80
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'LAST_CHECK_DATE_STATUS'
        Title.Alignment = taCenter
        Title.Caption = #1044#1072#1090#1072' '#1087#1086#1089#1083#1077#1076#1085#1077#1075#1086' '#1089#1086#1089#1090#1086#1103#1085#1080#1103
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 180
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TARIFF_NAME'
        Title.Alignment = taCenter
        Title.Caption = #1058#1072#1088#1080#1092
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 193
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ERROR_LOAD_API'
        Title.Alignment = taCenter
        Title.Caption = #1054#1096#1080#1073#1082#1072' '#1079#1072#1075#1088#1091#1079#1082#1080' '#1087#1086' API'
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 180
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATE_CREATED'
        Title.Alignment = taCenter
        Title.Caption = #1044#1072#1090#1072' '#1079#1072#1075#1088#1091#1079#1082#1080' '#1076#1072#1085#1085#1099#1093
        Title.Font.Charset = DEFAULT_CHARSET
        Title.Font.Color = clWindowText
        Title.Font.Height = -11
        Title.Font.Name = 'MS Sans Serif'
        Title.Font.Style = [fsBold]
        Width = 86
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 1120
    Height = 90
    Align = alTop
    TabOrder = 1
    object lAccount: TLabel
      Left = 8
      Top = 8
      Width = 62
      Height = 13
      Caption = #1051#1080#1094'. '#1089#1095#1077#1090':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lReportText: TLabel
      Left = 345
      Top = 63
      Width = 212
      Height = 16
      Caption = #1044#1072#1090#1072' '#1092#1086#1088#1084#1080#1088#1086#1074#1072#1085#1080#1103' '#1086#1090#1095#1077#1090#1072':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbReportDate: TLabel
      Left = 563
      Top = 63
      Width = 94
      Height = 16
      Caption = 'lbReportDate'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -13
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object BitBtn1: TBitBtn
      Left = 344
      Top = 2
      Width = 151
      Height = 29
      Action = aExcel
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' MS Excel'
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
    end
    object BitBtn2: TBitBtn
      Left = 495
      Top = 2
      Width = 130
      Height = 29
      Action = aRefresh
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
    end
    object BitBtn3: TBitBtn
      Left = 625
      Top = 2
      Width = 111
      Height = 29
      Action = aShowUserStatInfo
      Caption = #1055#1086#1076#1088#1086#1073#1085#1086
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
    end
    object cbSearch: TCheckBox
      Left = 742
      Top = 7
      Width = 60
      Height = 17
      Caption = #1055#1086#1080#1089#1082
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 3
      OnClick = cbSearchClick
    end
    object CLB_Accounts: TsCheckListBox
      Left = 72
      Top = 2
      Width = 266
      Height = 85
      BorderStyle = bsSingle
      Color = clWhite
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlack
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      ParentShowHint = False
      ShowHint = True
      TabOrder = 4
      OnExit = CLB_AccountsExit
      OnMouseMove = CLB_AccountsMouseMove
      BoundLabel.Indent = 0
      BoundLabel.Font.Charset = DEFAULT_CHARSET
      BoundLabel.Font.Color = clWindowText
      BoundLabel.Font.Height = -11
      BoundLabel.Font.Name = 'Tahoma'
      BoundLabel.Font.Style = []
      BoundLabel.Layout = sclLeft
      BoundLabel.MaxWidth = 0
      BoundLabel.UseSkinColor = True
      SkinData.SkinSection = 'EDIT'
      OnClickCheck = CLB_AccountsClickCheck
    end
    object BitBtn4: TBitBtn
      Left = 344
      Top = 32
      Width = 151
      Height = 29
      Action = aExclusion
      Caption = #1048#1089#1082#1083#1102#1095#1077#1085#1080#1103
      TabOrder = 5
    end
    object btCreateReport: TBitBtn
      Left = 495
      Top = 32
      Width = 130
      Height = 29
      Hint = #1055#1088#1086#1089#1084#1086#1090#1088' '#1080#1085#1092#1086#1088#1084#1072#1094#1080#1080' '#1087#1086' '#1072#1073#1086#1085#1077#1085#1090#1091
      Caption = #1057#1092#1086#1088#1084#1080#1088#1086#1074#1072#1090#1100' '#1086#1090#1095#1077#1090
      ParentShowHint = False
      ShowHint = True
      TabOrder = 6
      OnClick = btCreateReportClick
    end
  end
  object qReport: TOraQuery
    SQL.Strings = (
      'select '
      '    v.login, '
      '    v.phone_number_federal, '
      '    v.contract_date, '
      '    v.balance, '
      '    v.PHONE_IS_ACTIVE, '
      '    v.Dop_status_name, '
      '    v.LAST_CHECK_DATE_STATUS, '
      '    v.DATE_CREATED, '
      '   -- v.CURR_TARIFF_ID, '
      '    v.TARIFF_NAME,'
      '    v.ERROR_LOAD_API'
      'from PHONE_WITHOUT_TRAFFIC v'
      'where not exists ('
      '                  select 1'
      '                  FROM NUM_WITHOUT_TRAFFIC_EXCLUSION '
      '                  WHERE PHONE_NUMBER = V.PHONE_NUMBER_FEDERAL'
      '           )')
    FetchRows = 250
    ReadOnly = True
    Left = 96
    Top = 200
  end
  object DataSource1: TDataSource
    DataSet = qReport
    Left = 160
    Top = 200
  end
  object ActionList1: TActionList
    Images = MainForm.ImageList24
    Left = 32
    Top = 200
    object aExcel: TAction
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' MS Excel'
      ImageIndex = 12
      OnExecute = aExcelExecute
    end
    object aRefresh: TAction
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      ImageIndex = 9
      OnExecute = aRefreshExecute
    end
    object aShowUserStatInfo: TAction
      Caption = #1055#1086#1076#1088#1086#1073#1085#1086
      Hint = #1055#1088#1086#1089#1084#1086#1090#1088' '#1080#1085#1092#1086#1088#1084#1072#1094#1080#1080' '#1087#1086' '#1072#1073#1086#1085#1077#1085#1090#1091
      ImageIndex = 22
      ShortCut = 13
      OnExecute = aShowUserStatInfoExecute
    end
    object aExclusion: TAction
      Caption = #1048#1089#1082#1083#1102#1095#1077#1085#1080#1103
      OnExecute = aExclusionExecute
    end
  end
  object qAccounts: TOraQuery
    SQL.Strings = (
      
        'SELECT ACCOUNT_ID, decode(COMPANY_NAME,null,login, login||'#39' : '#39'|' +
        '|COMPANY_NAME) as login, COMPANY_NAME  FROM ACCOUNTS '
      'ORDER BY 2 ASC')
    Left = 200
    Top = 144
  end
  object qUserCheckAllow: TOraQuery
    SQL.Strings = (
      'SELECT check_allow_account FROM user_names'
      'WHERE UPPER(user_name) = UPPER(:user_name)')
    Left = 32
    Top = 144
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'user_name'
      end>
  end
  object qAccounts_Allow: TOraQuery
    SQL.Strings = (
      
        'SELECT a.ACCOUNT_ID, decode(COMPANY_NAME,null,login, login||'#39' : ' +
        #39'||COMPANY_NAME) as login, COMPANY_NAME'
      'FROM ACCOUNTS a, user_names u, rights_type_account_allow r'
      'WHERE UPPER(u.user_name) = UPPER(:user_name) '
      
        '  AND u.rights_type = r.rights_type AND r.account_id = a.account' +
        '_id'
      'ORDER BY 2 ASC')
    Left = 120
    Top = 144
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'user_name'
      end>
  end
  object qGET_PHONE_WITHOUT_TRAFFIC_TAB: TOraQuery
    SQL.Strings = (
      'select '
      '    v.login, '
      '    v.phone_number_federal, '
      '    v.contract_date, '
      '    v.balance, '
      '    v.PHONE_IS_ACTIVE, '
      '    v.Dop_status_name, '
      '    v.LAST_CHECK_DATE_STATUS, '
      '    v.DATE_CREATED, '
      '   -- v.CURR_TARIFF_ID, '
      '    v.TARIFF_NAME,'
      '    v.ERROR_LOAD_API'
      'from PHONE_WITHOUT_TRAFFIC v'
      'where not exists ('
      '                  select 1'
      '                  FROM NUM_WITHOUT_TRAFFIC_EXCLUSION '
      '                  WHERE PHONE_NUMBER = V.PHONE_NUMBER_FEDERAL'
      '           )')
    Left = 96
    Top = 248
  end
  object PopupMenu1: TPopupMenu
    Left = 336
    Top = 192
    object N1: TMenuItem
      Caption = #1048#1089#1082#1083#1102#1095#1080#1090#1100' '#1090#1077#1083#1077#1092#1086#1085
      OnClick = N1Click
    end
  end
  object Timer1: TTimer
    Enabled = False
    OnTimer = Timer1Timer
    Left = 744
    Top = 168
  end
  object qGetReportData: TOraQuery
    Session = Dm.OraSession
    SQL.Strings = (
      'select'
      '  max(date_created) max_date_created,'
      '  count(*) preview_count'
      '   '
      'from'
      '  PHONE_WITHOUT_TRAFFIC act')
    Left = 292
    Top = 263
  end
  object qGetJobStatus: TOraQuery
    Session = Dm.OraSession
    SQL.Strings = (
      'select'
      '  count(*) ct'
      'from'
      '  dba_scheduler_jobs t '
      'where'
      '  t.job_name like '#39'J_PHONE_WITHOUT_TRAFFIC_ST_0%'#39
      '  and state = '#39'RUNNING'#39)
    Left = 371
    Top = 260
  end
  object sProcStartJob: TOraStoredProc
    StoredProcName = 'START_JOB_WITHOUT_TRAFFIC'
    Session = Dm.OraSession
    SQL.Strings = (
      'begin'
      '  START_JOB_WITHOUT_TRAFFIC;'
      'end;')
    Left = 455
    Top = 210
    CommandStoredProcName = 'START_JOB_WITHOUT_TRAFFIC'
  end
end
