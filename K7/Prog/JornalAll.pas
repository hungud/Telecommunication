...
type
  TJornalAllForm = class(TForm)
    ...
    procedure aShowUserGroupExecute(Sender: TObject);
  private
...
uses CommCtrl, ContractsRegistration_Utils,SelectContract_, SelectContract,  Main,
  ShowUserStat, LoadData, DMUnit, SelectContractGroup, ShowGroupStat;
...
begin
  ...
  try
    ...
    //для смены тарифа по USSD редактирование запрещается
    if (VarIsNull(qMain.FieldByName('PARENT_PAYMENT_ID').Value))
       AND (qMain.FieldByName('DOCUM_TYPE_ID').Value <> TYPE_DOC_CHANGE_TARIFF_BY_USSD) then
      pRunMode := 'EDIT'
    else
      pRunMode := 'DISABLE';
...
//                             VarArrayOf([RegisterPaymentForm.FReceivedPaymentId, TYPE_DOC_PAYMENT]), []);
      ...
      TYPE_DOC_CHANGE_TARIFF_BY_USSD    : EditContractChange;
    else
...
begin
{
  ...
  begin
   ...
   SelectPhoneNumber_(vPhoneNumber, vContractID, MainForm.FUseFilialBlockAccess, Dm.OraSession, MainForm.FFilial);
...
begin
  ...
  begin
    ...
    if qMain.FieldByName('DOCUM_TYPE_ID').IsNull then
      ...
      TYPE_DOC_CHANGE_TARIFF_BY_USSD : vImageIndex := 3;
    else
      vImageIndex := -1;
    end;
...
//  MainForm.CheckAdminPrivileges;
 Try
  ...
  if (users.IndexOf(UpperCase(Dm.OraSession.Username)) <> -1) or (GetConstantValue('SERVER_NAME') <> 'GSM_CORP') then
    AddPayment
  else
...
procedure TJornalAllForm.aShowUserGroupExecute(Sender: TObject);
var
  vGROUP_NAME : String;
  vGROUP_ID : Integer;
begin
  //SelectPhoneNumber (vPhoneNumber, vContractID) ;
  SelectPhoneNumberGroup(vGROUP_NAME, vGROUP_ID, MainForm.FUseFilialBlockAccess, Dm.OraSession, MainForm.FFilial);
  if vGROUP_NAME <> '' then
  DoShowGroupStat(vGROUP_ID,vGROUP_NAME);
