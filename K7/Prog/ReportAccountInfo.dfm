object ReportAccountInfoForm: TReportAccountInfoForm
  Left = 0
  Top = 0
  Caption = #1048#1079#1084#1077#1085#1077#1085#1080#1103' '#1073#1072#1083#1072#1085#1089#1086#1074
  ClientHeight = 561
  ClientWidth = 802
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object spl1: TSplitter
    Left = 0
    Top = 355
    Width = 802
    Height = 5
    Cursor = crVSplit
    Align = alBottom
    ExplicitTop = 357
    ExplicitWidth = 776
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 802
    Height = 28
    Align = alTop
    TabOrder = 0
    ExplicitWidth = 776
    object lAccount: TLabel
      Left = 8
      Top = 8
      Width = 62
      Height = 13
      Caption = #1051#1080#1094'. '#1089#1095#1077#1090':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lbl2: TLabel
      Left = 612
      Top = 9
      Width = 74
      Height = 13
      Caption = #1057#1095#1077#1090#1072' '#1085#1072' '#1076#1072#1090#1091
    end
    object btLoadInExcel: TBitBtn
      Left = 291
      Top = -1
      Width = 135
      Height = 29
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' Excel'
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
      OnClick = aExcelExecute
    end
    object btRefresh: TBitBtn
      Left = 188
      Top = -1
      Width = 105
      Height = 29
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
      OnClick = aRefreshExecute
    end
    object cbAccounts: TComboBox
      Left = 76
      Top = 3
      Width = 98
      Height = 21
      Style = csDropDownList
      TabOrder = 2
      OnChange = cbAccountsChange
    end
    object bLoadHistoryInExcel: TBitBtn
      Left = 424
      Top = -1
      Width = 182
      Height = 29
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1080#1089#1090#1086#1088#1080#1102' '#1074' Excel'
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 3
      OnClick = aExcelExecute
    end
    object dtDate: TDateTimePicker
      Left = 692
      Top = 1
      Width = 101
      Height = 21
      Date = 41899.658416597220000000
      Time = 41899.658416597220000000
      ShowCheckbox = True
      TabOrder = 4
      OnChange = dtDateChange
    end
  end
  object grData: TCRDBGrid
    Left = 0
    Top = 28
    Width = 802
    Height = 327
    Align = alClient
    DataSource = dsReport
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    ParentFont = False
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -12
    TitleFont.Name = 'Tahoma'
    TitleFont.Style = [fsBold]
    Columns = <
      item
        Expanded = False
        FieldName = 'COMPANY_NAME'
        Title.Alignment = taCenter
        Width = 244
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ACCOUNT_NUMBER'
        Title.Alignment = taCenter
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'LOGIN'
        Title.Alignment = taCenter
        Width = 124
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'BALANCE'
        Title.Alignment = taCenter
        Width = 100
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'COUNT_PHONE'
        Title.Alignment = taCenter
        Width = 107
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'DATE_LAST_UPDATED'
        Title.Alignment = taCenter
        Width = 148
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PAYMENT_SUM_PREV'
        Title.Alignment = taCenter
        Width = 149
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'PAYMENT_SUM_NOW'
        Title.Alignment = taCenter
        Width = 138
        Visible = True
      end>
  end
  object pAccount_History: TPanel
    Left = 0
    Top = 360
    Width = 802
    Height = 201
    Align = alBottom
    TabOrder = 2
    ExplicitWidth = 776
    object lbl1: TLabel
      Left = 1
      Top = 1
      Width = 800
      Height = 13
      Align = alTop
      Caption = '  '#1048#1089#1090#1086#1088#1080#1103' '#1080#1079#1084#1077#1085#1077#1085#1080#1103' '#1089#1095#1077#1090#1072
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      ExplicitWidth = 155
    end
    object gAccount_History: TCRDBGrid
      Left = 1
      Top = 14
      Width = 800
      Height = 186
      Align = alClient
      DataSource = dsAccount_History
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      TabOrder = 0
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -11
      TitleFont.Name = 'Tahoma'
      TitleFont.Style = []
      Columns = <
        item
          Expanded = False
          FieldName = 'LOGIN'
          Font.Charset = DEFAULT_CHARSET
          Font.Color = clWindowText
          Font.Height = -11
          Font.Name = 'Tahoma'
          Font.Style = []
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Tahoma'
          Title.Font.Style = [fsBold]
          Width = 99
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'BALANCE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Tahoma'
          Title.Font.Style = [fsBold]
          Width = 167
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'COUNT_PHONE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Tahoma'
          Title.Font.Style = [fsBold]
          Width = 154
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'USER_UPDATE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Tahoma'
          Title.Font.Style = [fsBold]
          Width = 128
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'DATE_UPDATE'
          Title.Alignment = taCenter
          Title.Font.Charset = DEFAULT_CHARSET
          Title.Font.Color = clWindowText
          Title.Font.Height = -12
          Title.Font.Name = 'Tahoma'
          Title.Font.Style = [fsBold]
          Width = 199
          Visible = True
        end>
    end
  end
  object qAccounts: TOraQuery
    DataTypeMap = <>
    Session = MainForm.OraSession
    SQL.Strings = (
      'SELECT ACCOUNT_ID, LOGIN FROM ACCOUNTS ORDER BY LOGIN ASC')
    Left = 120
    Top = 96
  end
  object dsReport: TDataSource
    DataSet = qreport
    Left = 56
    Top = 96
  end
  object qreport: TOraQuery
    SQLInsert.Strings = (
      'SELECT A.ACCOUNT_ID, '
      '       A.COMPANY_NAME,'
      '       A.ACCOUNT_NUMBER,'
      '       A.LOGIN,'
      '       I.BALANCE, '
      '       I.COUNT_PHONE, '
      '       I.DATE_LAST_UPDATED, '
      '       BPP.PAYMENT_SUM_PREV,'
      '       BPN.PAYMENT_SUM_NOW'
      '  FROM ACCOUNT_INFO I, ACCOUNTS A, '
      '       (select b.ACCOUNT_ID, sum(b.PAYMENT_SUM) PAYMENT_SUM_PREV'
      '          from db_loader_payments b'
      
        '          where b.YEAR_MONTH = to_number(to_char(add_months(sysd' +
        'ate, -1), '#39'yyyymm'#39'))'
      '          group by b.YEAR_MONTH, b.ACCOUNT_ID) BPP, '
      '       (SELECT B.ACCOUNT_ID, SUM(B.PAYMENT_SUM) PAYMENT_SUM_NOW'
      '          from db_loader_payments b'
      
        '          where b.YEAR_MONTH = to_number(to_char(sysdate, '#39'yyyym' +
        'm'#39'))'
      '          group by b.YEAR_MONTH, b.ACCOUNT_ID) BPN'
      '  WHERE A.ACCOUNT_ID = I.ACCOUNT_ID(+) '
      '    and a.ACCOUNT_ID = BPP.ACCOUNT_ID(+)'
      '    and a.ACCOUNT_ID = BPN.ACCOUNT_ID(+)'
      '    AND ((:ACCOUNT_ID IS NULL) OR (I.ACCOUNT_ID = :ACCOUNT_ID))'
      '  ORDER BY A.COMPANY_NAME')
    SQLUpdate.Strings = (
      'SELECT A.ACCOUNT_ID, '
      '       A.COMPANY_NAME,'
      '       A.ACCOUNT_NUMBER,'
      '       A.LOGIN,'
      '       NVL(his.BALANCE, null) BALANCE, '
      '       NVL(his.COUNT_PHONE, null) COUNT_PHONE, '
      '       I.DATE_LAST_UPDATED, '
      '       BPP.PAYMENT_SUM_PREV,'
      '       BPN.PAYMENT_SUM_NOW'
      '  FROM ACCOUNT_INFO I, ACCOUNTS A, '
      '       (select b.ACCOUNT_ID, sum(b.PAYMENT_SUM) PAYMENT_SUM_PREV'
      '          from db_loader_payments b'
      
        '          where b.YEAR_MONTH = to_number(to_char(add_months(sysd' +
        'ate, -1), '#39'yyyymm'#39'))'
      '          group by b.YEAR_MONTH, b.ACCOUNT_ID) BPP, '
      '       (SELECT B.ACCOUNT_ID, SUM(B.PAYMENT_SUM) PAYMENT_SUM_NOW'
      '          from db_loader_payments b'
      
        '          where b.YEAR_MONTH = to_number(to_char(sysdate, '#39'yyyym' +
        'm'#39'))'
      '          group by b.YEAR_MONTH, b.ACCOUNT_ID) BPN,'
      '       --'#1087#1086#1083#1091#1095#1072#1077#1084' '#1080#1089#1090#1086#1088#1080#1102' '#1080#1079#1084#1077#1085#1077#1085#1080#1081' '#1089#1095#1077#1090#1086#1074
      
        '       (select ACCHIS.ID_ACCOUNT_INFO_HISTORY, ACCHIS.ACCOUNT_ID' +
        ', BALANCE, '
      
        '         ACCHIS.COUNT_PHONE, ACCHIS.USER_UPDATE, ACCHIS.DATE_UPD' +
        'ATE DATE_LAST_UPDATED'
      '        from Account_info_history acchis,'
      '             (select ACCOUNT_ID, MAX(date_update) date_update'
      '              from Account_info_history '
      '              where trunc(date_update) = trunc(:pDate_Update)'
      '              group by ACCOUNT_ID) acchisdet'
      '        where ACCHIS.ACCOUNT_ID = acchisdet.ACCOUNT_ID'
      '        and ACCHIS.DATE_UPDATE = acchisdet.date_update) His'
      '  WHERE A.ACCOUNT_ID = I.ACCOUNT_ID(+) '
      '    and a.ACCOUNT_ID = BPP.ACCOUNT_ID(+)'
      '    and a.ACCOUNT_ID = BPN.ACCOUNT_ID(+)'
      '    and a.ACCOUNT_ID = His.ACCOUNT_ID(+)'
      '    AND ((:ACCOUNT_ID IS NULL) OR (I.ACCOUNT_ID = :ACCOUNT_ID))'
      '  ORDER BY A.COMPANY_NAME')
    DataTypeMap = <>
    Session = MainForm.OraSession
    SQL.Strings = (
      'SELECT A.ACCOUNT_ID, '
      '       A.COMPANY_NAME,'
      '       A.ACCOUNT_NUMBER,'
      '       A.LOGIN,'
      '       I.BALANCE, '
      '       I.COUNT_PHONE, '
      '       I.DATE_LAST_UPDATED, '
      '       BPP.PAYMENT_SUM_PREV,'
      '       BPN.PAYMENT_SUM_NOW'
      '  FROM ACCOUNT_INFO I, ACCOUNTS A, '
      '       (select b.ACCOUNT_ID, sum(b.PAYMENT_SUM) PAYMENT_SUM_PREV'
      '          from db_loader_payments b'
      
        '          where b.YEAR_MONTH = to_number(to_char(add_months(sysd' +
        'ate, -1), '#39'yyyymm'#39'))'
      '          group by b.YEAR_MONTH, b.ACCOUNT_ID) BPP, '
      '       (SELECT B.ACCOUNT_ID, SUM(B.PAYMENT_SUM) PAYMENT_SUM_NOW'
      '          from db_loader_payments b'
      
        '          where b.YEAR_MONTH = to_number(to_char(sysdate, '#39'yyyym' +
        'm'#39'))'
      '          group by b.YEAR_MONTH, b.ACCOUNT_ID) BPN'
      '  WHERE A.ACCOUNT_ID = I.ACCOUNT_ID(+) '
      '    and a.ACCOUNT_ID = BPP.ACCOUNT_ID(+)'
      '    and a.ACCOUNT_ID = BPN.ACCOUNT_ID(+)'
      '    AND ((:ACCOUNT_ID IS NULL) OR (I.ACCOUNT_ID = :ACCOUNT_ID))'
      '  ORDER BY A.COMPANY_NAME')
    FetchRows = 250
    FetchAll = True
    AfterScroll = qreportAfterScroll
    Left = 184
    Top = 96
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'ACCOUNT_ID'
      end>
    object qreportCOMPANY_NAME: TStringField
      DisplayLabel = #1055#1089#1077#1074#1076#1086#1085#1080#1084
      DisplayWidth = 30
      FieldName = 'COMPANY_NAME'
      Size = 120
    end
    object qreportACCOUNT_NUMBER: TFloatField
      DisplayLabel = #1051#1080#1094'. '#1089#1095#1077#1090
      DisplayWidth = 12
      FieldName = 'ACCOUNT_NUMBER'
    end
    object qreportLOGIN: TStringField
      DisplayLabel = #1051#1086#1075#1080#1085
      DisplayWidth = 15
      FieldName = 'LOGIN'
      Size = 120
    end
    object qreportBALANCE: TFloatField
      DisplayLabel = #1041#1072#1083#1072#1085#1089
      DisplayWidth = 12
      FieldName = 'BALANCE'
    end
    object qreportCOUNT_PHONE: TFloatField
      DisplayLabel = #1050#1086#1083'-'#1074#1086' '#1085#1086#1084#1077#1088#1086#1074
      DisplayWidth = 8
      FieldName = 'COUNT_PHONE'
    end
    object qreportDATE_LAST_UPDATED: TDateTimeField
      DisplayLabel = #1044#1072#1090#1072' '#1086#1073#1085#1086#1074#1083#1077#1085#1080#1103
      DisplayWidth = 18
      FieldName = 'DATE_LAST_UPDATED'
    end
    object qreportPAYMENT_SUM_PREV: TFloatField
      DisplayLabel = #1055#1083#1072#1090#1077#1078#1080' '#1087#1088#1077#1076'. '#1084#1077#1089#1103#1094#1072
      FieldName = 'PAYMENT_SUM_PREV'
    end
    object qreportPAYMENT_SUM_NOW: TFloatField
      DisplayLabel = #1055#1083#1072#1090#1077#1078#1080' '#1090#1077#1082'. '#1084#1077#1089#1103#1094#1072
      FieldName = 'PAYMENT_SUM_NOW'
    end
    object qReportACCOUNT_ID: TFloatField
      FieldName = 'ACCOUNT_ID'
    end
  end
  object qAccounts_History: TOraQuery
    DataTypeMap = <>
    Session = MainForm.OraSession
    SQL.Strings = (
      'SELECT '
      '  accihfhis.ID_ACCOUNT_INFO_HISTORY, '
      '  accihfhis.ACCOUNT_ID, '
      '  ACCS.LOGIN,'
      '  accihfhis.BALANCE, '
      '  accihfhis.COUNT_PHONE, '
      '  accihfhis.USER_UPDATE, '
      '  accihfhis.DATE_UPDATE'
      'FROM ACCOUNT_INFO_HISTORY accihfhis'
      
        'LEFT JOIN ACCOUNTS accs ON ACCIHFHIS.ACCOUNT_ID = ACCS.ACCOUNT_I' +
        'D'
      'WHERE accihfhis.ACCOUNT_ID = :pACCOUNT_ID')
    BeforeOpen = qAccounts_HistoryBeforeOpen
    Left = 392
    Top = 440
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'pACCOUNT_ID'
      end>
    object qAccounts_HistoryID_ACCOUNT_INFO_HISTORY: TFloatField
      FieldName = 'ID_ACCOUNT_INFO_HISTORY'
      Required = True
    end
    object qAccounts_HistoryACCOUNT_ID: TFloatField
      DisplayLabel = #1050#1086#1076' '
      FieldName = 'ACCOUNT_ID'
      Required = True
    end
    object qAccounts_HistoryBALANCE: TFloatField
      DisplayLabel = #1041#1072#1083#1072#1085#1089
      FieldName = 'BALANCE'
    end
    object qAccounts_HistoryCOUNT_PHONE: TFloatField
      DisplayLabel = #1050#1086#1083'-'#1074#1086' '#1085#1086#1084#1077#1088#1086#1074
      FieldName = 'COUNT_PHONE'
    end
    object qAccounts_HistoryUSER_UPDATE: TStringField
      DisplayLabel = #1055#1086#1083#1100#1079#1086#1074#1072#1090#1077#1083#1100
      FieldName = 'USER_UPDATE'
      Size = 120
    end
    object qAccounts_HistoryDATE_UPDATE: TDateTimeField
      DisplayLabel = #1044#1072#1090#1072' '#1080#1079#1084#1077#1085#1077#1085#1080#1103
      FieldName = 'DATE_UPDATE'
    end
    object qAccounts_HistoryLOGIN: TStringField
      DisplayLabel = #1051#1086#1075#1080#1085
      FieldName = 'LOGIN'
      Size = 120
    end
  end
  object dsAccount_History: TDataSource
    DataSet = qAccounts_History
    Left = 512
    Top = 440
  end
end
