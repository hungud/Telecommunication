...
const
  ...
  TYPE_DOC_CHANGE_TARIFF_BY_USSD = 6;
  ...
  TYPE_DOC_CHANGE_TARIFF_BY_USSD_TXT = 'USSD: Заявление на смену тарифного плана';
...
//                               var pPhoneNumber : String; var pAbonentId : Variant) : boolean;
  ...
  //в качестве передаваемых параметров: путь сохранения, имя файла, query, field
  procedure SaveBlobFile(pPath, pFileName: string; qS: TOraQuery; pField: TField);

implementation

uses
  Dialogs, Variants, Main, DMUnit;
...
begin
  ...
  try
    qS.SQL.Text:= 'SELECT MAX_PROMISED_PAYMENT FROM USER_NAMES WHERE UPPER(USER_NAME) = UPPER(:pUSER_NAME)';
...
begin
  ...
  begin
    ...
    try
      ...
      qS.ParamByName('pUSER_NAME').AsString := dm.OraSession.Username;
      qS.Open;
