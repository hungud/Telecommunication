object ReportHotBillingFilesForm: TReportHotBillingFilesForm
  Left = 0
  Top = 0
  Caption = #1047#1072#1075#1088#1091#1079#1082#1080' '#1075#1086#1088#1103#1095#1077#1075#1086' '#1073#1080#1083#1083#1080#1085#1075#1072
  ClientHeight = 433
  ClientWidth = 803
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  Position = poScreenCenter
  PixelsPerInch = 96
  TextHeight = 13
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 803
    Height = 30
    Align = alTop
    TabOrder = 0
    object lPeriod: TLabel
      Left = 8
      Top = 8
      Width = 49
      Height = 13
      Caption = #1055#1077#1088#1080#1086#1076':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object btLoadInExcel: TBitBtn
      Left = 167
      Top = 0
      Width = 135
      Height = 29
      Action = aExcel
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' Excel'
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
    end
    object btRefresh: TBitBtn
      Left = 303
      Top = 0
      Width = 105
      Height = 29
      Action = aRefresh
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
    end
    object cbPeriod: TComboBox
      Left = 62
      Top = 3
      Width = 99
      Height = 21
      Style = csDropDownList
      TabOrder = 2
    end
  end
  object PageControl1: TPageControl
    Left = 0
    Top = 30
    Width = 803
    Height = 403
    ActivePage = tsFiles
    Align = alClient
    TabOrder = 1
    object tsFiles: TTabSheet
      Caption = #1047#1072#1075#1088#1091#1079#1082#1072' '#1092#1072#1081#1083#1086#1074' '#1075#1086#1088'.'#1073#1080#1083#1083#1080#1085#1075#1072
      object grData_Files: TCRDBGrid
        Left = 0
        Top = 0
        Width = 795
        Height = 375
        Align = alClient
        DataSource = dsReport_Files
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'FILE_NAME'
            Title.Alignment = taCenter
            Title.Caption = #1048#1084#1103' '#1092#1072#1081#1083#1072
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 307
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'LOAD_SDATE'
            Title.Alignment = taCenter
            Title.Caption = #1044#1072#1090#1072' '#1085#1072#1095#1072#1083#1072' '#1079#1072#1075#1088#1091#1079#1082#1080
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 135
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'LOAD_EDATE'
            Title.Alignment = taCenter
            Title.Caption = #1044#1072#1090#1072' '#1086#1082#1086#1085#1095#1072#1085#1080#1103' '#1079#1072#1075#1088#1091#1079#1082#1080
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 165
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'CNT'
            Title.Alignment = taCenter
            Title.Caption = #1050#1086#1083'-'#1074#1086' '#1079#1072#1087#1080#1089#1077#1081
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 116
            Visible = True
          end>
      end
    end
    object tsPrev: TTabSheet
      Caption = #1047#1072#1075#1088#1091#1079#1082#1072' '#1075#1086#1088'.'#1073#1080#1083#1083#1080#1085#1075#1072' '#1087#1086' '#1095#1072#1089#1072#1084
      ImageIndex = 1
      object grData_Prev: TCRDBGrid
        Left = 0
        Top = 0
        Width = 795
        Height = 375
        Align = alClient
        DataSource = dsReport_Prev
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
        ReadOnly = True
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'Tahoma'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'START_DATE'
            Title.Alignment = taCenter
            Title.Caption = #1044#1072#1090#1072' '#1085#1072#1095#1072#1083#1072' '#1074#1099#1079#1086#1074#1072
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 154
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'INTERVAL_TIME'
            Title.Alignment = taCenter
            Title.Caption = #1042#1088#1077#1084#1103' '#1085#1072#1095#1072#1083#1072' '#1074#1099#1079#1086#1074#1072
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 144
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'CNT'
            Title.Alignment = taCenter
            Title.Caption = #1050#1086#1083'-'#1074#1086' '#1079#1072#1087#1080#1089#1077#1081
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 98
            Visible = True
          end
          item
            Alignment = taCenter
            Expanded = False
            FieldName = 'CNT_PREV'
            Title.Alignment = taCenter
            Title.Caption = #1050#1086#1083'-'#1074#1086' '#1074' '#1087#1088#1086#1096#1083#1099#1081' '#1076#1077#1085#1100
            Title.Font.Charset = DEFAULT_CHARSET
            Title.Font.Color = clWindowText
            Title.Font.Height = -11
            Title.Font.Name = 'Tahoma'
            Title.Font.Style = [fsBold]
            Width = 147
            Visible = True
          end>
      end
    end
  end
  object ActionList: TActionList
    Images = MainForm.ImageList24
    Left = 64
    Top = 88
    object aExcel: TAction
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' Excel'
      ImageIndex = 12
      OnExecute = aExcelExecute
    end
    object aRefresh: TAction
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      ImageIndex = 9
      OnExecute = aRefreshExecute
    end
  end
  object qPeriods: TOraQuery
    DataTypeMap = <>
    SQL.Strings = (
      'SELECT substr(object_name,9,4)||substr(object_name,6,2),'
      '       substr(object_name,6,7) period,'
      
        '       substr(object_name,6,2)||'#39'-'#39'||substr(object_name,9,4) per' +
        'iod_name'
      
        'FROM all_objects o WHERE o.object_name like '#39'CALL_%'#39' AND o.objec' +
        't_type='#39'TABLE'#39
      
        'AND substr(object_name,9,4)||substr(object_name,6,2) <= TO_CHAR(' +
        'SYSDATE,'#39'yyyymm'#39')'
      'ORDER BY 1 DESC')
    Left = 160
    Top = 136
  end
  object qReport_Files: TOraQuery
    DataTypeMap = <>
    SQL.Strings = (
      
        'select f.FILE_NAME, f.LOAD_SDATE, f.LOAD_EDATE, f.HBF_ID, count(' +
        '*) cnt'
      'from hot_billing_files f,'
      'call_09_2014 c'
      'where f.hbf_id=c.dbf_id'
      'group by f.FILE_NAME, f.LOAD_SDATE, f.LOAD_EDATE, f.HBF_ID'
      'order by hbf_id desc')
    FetchRows = 500
    FetchAll = True
    ReadOnly = True
    Left = 64
    Top = 136
    object qReport_FilesFILE_NAME: TStringField
      FieldName = 'FILE_NAME'
      Size = 50
    end
    object qReport_FilesLOAD_SDATE: TDateTimeField
      FieldName = 'LOAD_SDATE'
    end
    object qReport_FilesLOAD_EDATE: TDateTimeField
      FieldName = 'LOAD_EDATE'
    end
    object qReport_FilesCNT: TFloatField
      FieldName = 'CNT'
    end
  end
  object dsReport_Files: TDataSource
    DataSet = qReport_Files
    Left = 112
    Top = 152
  end
  object qReport_Prev: TOraQuery
    DataTypeMap = <>
    Session = MainForm.OraSession
    SQL.Strings = (
      'WITH'
      
        'T AS (SELECT trunc(start_time, '#39'hh'#39') t_current, trunc(start_time' +
        '-1, '#39'hh'#39') t_prev, count(*) CNT'
      'from call_07_2013 c'
      'group by trunc(start_time, '#39'hh'#39'), trunc(start_time-1, '#39'hh'#39')'
      ')'
      
        'select to_char(t1.t_current,'#39'dd.mm.yyyy'#39') start_date, to_char(t1' +
        '.t_current,'#39'hh24.mi'#39')||'#39'-'#39'||to_char(t1.t_current+1/24,'#39'hh24.mi'#39')' +
        ' interval_time,'
      '       t1.cnt, t2.cnt cnt_prev'
      'from t t1, t t2'
      'where t2.t_current(+) = t1.t_prev'
      'order by 1 desc, 2 asc')
    FetchAll = True
    ReadOnly = True
    Left = 56
    Top = 184
    object qReport_PrevSTART_DATE: TStringField
      FieldName = 'START_DATE'
      Size = 75
    end
    object qReport_PrevINTERVAL_TIME: TStringField
      FieldName = 'INTERVAL_TIME'
      Size = 182
    end
    object qReport_PrevCNT: TFloatField
      FieldName = 'CNT'
    end
    object qReport_PrevCNT_PREV: TFloatField
      FieldName = 'CNT_PREV'
    end
  end
  object dsReport_Prev: TDataSource
    DataSet = qReport_Prev
    Left = 120
    Top = 216
  end
end
