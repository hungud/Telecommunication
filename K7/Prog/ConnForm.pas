...
uses
  ...
  Vcl.ExtCtrls,
  MemUtils, Ora, OraError, OdacVCL, DMUnit, Func_Const, sEdit, TimedMsgBox,
...
begin
  btConnect.NumGlyphs := 1;
  btCancel.NumGlyphs := 1;
end;

procedure TfmConn.DoConnect;
begin
  if (SameText(edUsername.Text, eSchema.Text)) or (not cbEncrypt_pwd.checked) then
    FConnectDialog.Session.Password := edPassword.Text
  else
    FConnectDialog.Session.Password := md5_30(edPassword.Text);
