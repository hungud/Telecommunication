...
type
  TSelectContractForm = class(TForm)
    ...
    sfMNP_MAIN_TO_TEMP: TOraStoredProc;
...
uses ContractsRegistration_Utils,ShowUserStat, DMUnit;
...
begin
  ...
  qRightAccountAllow.ParamByName('USER_NAME').AsString := Dm.OraSession.Username;
...
begin
  ...
  qMain.ParamByName('USER_NAME').AsString := Dm.OraSession.Username;
  end;
  ...
  begin
   ...
   begin
     if (g.DataSource.DataSet.FieldByName('HAND_BLOCK').AsInteger = 1) then
       MainForm.ImageList16.Draw(g.Canvas,Rect.CenterPoint.X, Rect.Top, 11)
     else
       MainForm.ImageList16.Draw(g.Canvas,Rect.CenterPoint.X, Rect.Top, 12);
...
procedure TSelectContractForm.qMainHAND_BLOCKGetText(Sender: TField;
