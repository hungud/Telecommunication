...
type
  TReportFieldDetailsForm = class(TForm)
    ...
    dsMain: TDataSource;
    qReportCMFIELD_VALUE: TStringField;
...
begin
  ...
  try
    ...
    if GetConstantValue('SERVER_NAME') = 'CORP_MOBILE' then begin
      ReportFrm.Caption:='Неописанные БС';
      ReportFrm.tsMain.Caption:='Неописанные БС';
      ReportFrm.tsSearch.tabVisible:=false;
      ReportFrm.dsMain.DataSet:=ReportFrm.qReportCM;
      ReportFrm.dsMain.DataSet.Open;
    end;
    if GetConstantValue('SERVER_NAME') = 'GSM_CORP' then begin
      ReportFrm.Caption:='Номера собеседников';
      ReportFrm.tsMain.Caption:='Номера собеседников';
...
begin
  ...
  if GetConstantValue('SERVER_NAME') = 'GSM_CORP' then begin
    if cbPeriod.Text = '' then begin
      MessageDlg('Не задан период!', mtError, [mbOK], 0);
      exit;
    end;
    qReportJSM.Close;
...
begin
 ...
 //проверка существования таблиц в заданном интервале дат
...
begin
  if (eCalling_no.Text = '') then begin
    MessageDlg('Не задан номер собеседника!', mtError, [mbOK], 0);
    exit;
  end;
  if (eBeginDate.Text = '  .  .    ') then begin
    MessageDlg('Не задана начальная дата поиска!', mtError, [mbOK], 0);
    exit;
  end;
  if (eEndDate.Text = '  .  .    ') then begin
    MessageDlg('Не задана конечная дата поиска!', mtError, [mbOK], 0);
    exit;
  end;
  if StrToDate(eBeginDate.Text) > StrToDate(eEndDate.Text) then begin
    MessageDlg('Начальная дата больше конечной!', mtError, [mbOK], 0);
    exit;
  end;
  qSearch.Close;
  ...
  if (m = mm) and (y = yy)  then begin
    if not checkAllTables('CALL_'+FormatFloat('00',m)+'_'+IntToStr(y)) then begin
      MessageDlg('Отсутствует таблица детализации для заданного временного интервала!', mtError, [mbOK], 0);
      exit;
    end;
