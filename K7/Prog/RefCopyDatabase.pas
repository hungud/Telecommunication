...
type
  TRefCopyDataBaseForm = class(TForm)
    ...
    aExcel: TAction;
    qCompareBalanceCONTRACT_DATE: TDateTimeField;
    qCompareBalanceIS_CREDIT_CONTRACT: TIntegerField;
    qCompareBalanceIS_COLLECTOR: TFloatField;
...
procedure TRefCopyDataBaseForm.aInsertExecute(Sender: TObject);
 var
  SchemaName:string;
begin
 SchemaName:=Dm.OraSession.Schema+'_COPY';
 if InputQuery('Введите имени схемы','Введите имя новой схемы', SchemaName) then
 begin
   spCopySchema.ParamByName('SOURCE_SCHEME').AsString:=Dm.OraSession.Schema;
