...
VirtualTable,SyncObjs, Math,oraclasses, idSMTP, idMessage, IdAttachmentFile, Vcl.Oleauto,
  Vcl.DBCtrls,wslider, Vcl.Mask, IdIOHandlerSocket, IdIOHandlerStack, IdSSL, IdSSLOpenSSL, IdExplicitTLSClientServerBase;
...
type
 ...
 ThreadSMTP = class(TThread)
  ...
  TShowGroupStatForm = class(TForm)
    ...
    qBillsDetailBILL_SUMM: TFloatField;
    qBillsDetailABON_TP: TFloatField;
    qBillsDetailABON_ADD: TFloatField;
    qBillsDetailDISCOUNT: TFloatField;
    qBillsDetailSINGLE_PAYMENTS: TFloatField;
    qBillsDetailCALLS: TFloatField;
    qBillsDetailSINGLE_CORRECTION: TFloatField;
    IS_BILL_CHECK: TCheckBox;
    CRDBGrid2: TCRDBGrid;
    qHBDetails: TOraQuery;
    spGetHBMonth: TOraStoredProc;
    spGetPhoneDetail: TOraStoredProc;
    qHBDetailsPhoneList: TOraQuery;
    qHBDetailsPhone: TOraQuery;
    chkSendDetailAuto: TCheckBox;
    eSendDetailDay: TEdit;
    Label8: TLabel;
    Label11: TLabel;
    email_balance_period_Edit: TEdit;
    Label10: TLabel;
    email_balance_edit: TEdit;
    Label9: TLabel;
    btDetailAPISendMail: TBitBtn;
    Label12: TLabel;
    spDetailAPISendMail: TOraStoredProc;
    qContractGroupBEN: TFloatField;
    qContractGroupIS_COLLECTOR: TFloatField;
    ...
    procedure chkSendDetailAutoClick(Sender: TObject);
    procedure btDetailAPISendMailClick(Sender: TObject);
  ...
  private
    ...
    procedure LoadDetailText(const pPhone_Number:string; const pYear,pMonth:integer; var pDetailText:string);
    ...
    procedure LoadDetailInExcel(const pPhone_Number:string; const pYear, pMonth, sheet:integer; var Excel: OleVariant);
  public
...
const
  ...
  qHBDetailsPhoneStrTableNameNum = 1;
...
begin
  ...
  if chkHand_block.Checked then
   ...
   if chkSendDetailAuto.Checked then begin
     EditGroupParams.ParamByName('p_send_detail_auto').AsInteger:=1;
     if eParamsDetMail.Text = '' then begin
       MessageDlg('Для автоматической отправки детализаций укажите адрес почты!', mtError, [mbOK], 0);
       exit;
     end;
     if eSendDetailDay.Text <> '' then
       EditGroupParams.ParamByName('p_send_detail_day').AsInteger:=StrToInt(eSendDetailDay.Text)
     else begin
       MessageDlg('Заполните день месяца, после которого отправлять детализацию!', mtError, [mbOK], 0);
       exit;
     end;
   end
   else begin
     EditGroupParams.ParamByName('p_send_detail_auto').Clear;
     EditGroupParams.ParamByName('p_send_detail_day').Clear;
   end;

   EditGroupParams.ParamByName('p_email_balance').AsString:=email_balance_edit.text;
   EditGroupParams.ParamByName('p_email_balance_period').AsInteger:=StrToInt(email_balance_period_Edit.Text);
...
const
  ...
  ColCount:integer=14;
var
 i,j,org_name:integer;
 s, DetailText, Detail_mail : string;
 Excel : OleVariant;
 DetailFromDB: boolean;
begin
  ...
  //Определяем откуда брать детализации
  if IS_BILL_CHECK.Checked then
    DetailFromDB:=false
  else begin
      DetailFromDB:=true;
     //Определяем грузятся ли детализации в базу
     if GetConstantValue('DB_DETAILS')='0' then
       DetailFromDB:=false
     else begin
       spGetHBMonth.ParamByName('pYEAR').AsInteger := FYear;
       spGetHBMonth.ParamByName('pMONTH').AsInteger := FMonth;
       spGetHBMonth.ExecProc;
       if spGetHBMonth.ParamByName('RESULT').AsInteger=0 then
         DetailFromDB:=false;
     end;
  end;
  ...
  Try
    ...
    try
      ...
      begin
        ...
        begin
          ...
          if not DetailFromDB then begin
            ...
            if IS_BILL_CHECK.Checked then
              ...
              LoadDetailText(qContractGroup.FieldByName('phone_number_federal').AsString, fyear, fmonth, DetailText);
            //Обработка детализации
            PrepareDetailExcel(DetailText, i, Excel);
          end
          else
            //Получение детализации номера, заполнение Excel
            LoadDetailInExcel(qContractGroup.FieldByName('phone_number_federal').AsString, fyear, fmonth, i, Excel);
      ...
      Excel.WorkBooks[1].SaveAs(ExtractFilePath(application.ExeName)+'Detail_'+s+'.xlsx');
    ...
    if FCOLLECTOR = 1 then
      org_name := 93
    else
      org_name := 1;
    qSendMailParams.ParamByName('pfrom').AsInteger := org_name;
    ...
    try
     ...
     begin
      ...
      SMTP_PORT:= qSendMailParams.FieldByName('SMTP_PORT').AsInteger;
      SMTP_ADDR := Detail_mail;
      SMTP_TITLE := 'Детализация группы договоров "'+FGroupName+'" за '+IntToStr(FYear)+' - '+IntToStr(FMonth);
      SMTP_FROMTXT := qSendMailParams.FieldByName('SMTP_FROM').AsString;
//      SMTP_FROMTXT := GetParamValue('SMTP_FROMTEXT');
      SMTP_ATT_FILE := ExtractFilePath(application.ExeName)+'Detail_'+s+'.xlsx';
      Resume;
    end;
    finally
...
procedure TShowGroupStatForm.LoadDetailText(const pPhone_Number:string; const pYear,pMonth:integer; var pDetailText:string);
var DetailText : String;
begin
    spGetPhoneDetail.ParamByName('pYEAR').AsInteger := FYear;
    spGetPhoneDetail.ParamByName('pMONTH').AsInteger := FMonth;
    spGetPhoneDetail.ParamByName('pPHONE_NUMBER').AsString := pPhone_Number;
    try
...
procedure TShowGroupStatForm.LoadDetailInExcel(const pPhone_Number:string; const pYear,pMonth,sheet:integer; var Excel: OleVariant);
var
  i : Integer;
  sl : TStringList;
  s : String;
...
begin
  Try
    try
      ...
      if length(s)=1 then
      s:='0'+s;
      qHBDetailsPhone.Close;
      qHBDetailsPhone.SQL.Strings[qHBDetailsStrTableNameNum]:=' from call_'+s+'_'+inttostr(pYear)+' c10';
      qHBDetailsPhone.ParamByName('GROUP_ID').AsInteger := FGroupId;
      qHBDetailsPhone.ParamByName('YEAR_MONTH').AsString :=inttostr(FYear)+s;
      qHBDetailsPhone.ParamByName('PHONE_NUMBER').AsString :=pPhone_Number;
      qHBDetailsPhone.Open;
      i:=0;
      qHBDetailsPhone.First;
      while not qHBDetailsPhone.Eof do
      begin
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,1].value:=qHBDetailsPhone.FieldByName('SELF_NUMBER').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,2].value:=qHBDetailsPhone.FieldByName('SERVICE_DATE').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,3].value:=qHBDetailsPhone.FieldByName('SERVICE_TIME').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,4].value:=qHBDetailsPhone.FieldByName('SERVICE_TYPE').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,5].value:=qHBDetailsPhone.FieldByName('SERVICE_DIRECTION').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,6].value:=qHBDetailsPhone.FieldByName('OTHER_NUMBER').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,7].value:=qHBDetailsPhone.FieldByName('DURATION').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,8].value:=qHBDetailsPhone.FieldByName('DURATION_MIN').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,9].value:=qHBDetailsPhone.FieldByName('SERVICE_COST').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,10].value:=qHBDetailsPhone.FieldByName('IS_ROAMING').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,11].value:=qHBDetailsPhone.FieldByName('ROAMING_ZONE').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,12].value:=qHBDetailsPhone.FieldByName('ADD_INFO').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,13].value:=qHBDetailsPhone.FieldByName('BASE_STATION_CODE').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,14].value:=qHBDetailsPhone.FieldByName('COST_NO_VAT').AsString;
        Excel.WorkBooks[1].WorkSheets[sheet].cells[i+2,15].value:=qHBDetailsPhone.FieldByName('BS_PLACE').AsString;

        inc(i);
        qHBDetailsPhone.Next;
      end;
    finally
     qHBDetailsPhone.Close;
    end;
    ...
    on E : Exception do
      MessageDlg('Ошибка при получении и записи в Excel детализации.'#13#10 + E.Message , mtError, [mbOK], 0);
  end;
end;
...
begin
  ...
  begin
   ...
   try
     ...
     FStringStream.Position := 0;
     send_mail_detail.ParamByName('MAIL_TEXT').AsOraClob.OCISvcCtx := Dm.OraSession.OCISvcCtx;
...
procedure TShowGroupStatForm.btDetailAPISendMailClick(Sender: TObject);
begin
//L(:P_GROUP_ID, :P_MAIL_PARAM_ACCOUNT_ID);
  spDetailAPISendMail.ParamByName('P_GROUP_ID').AsInteger:=FGroupId;
  if FCOLLECTOR = 1 then
    spDetailAPISendMail.ParamByName('P_MAIL_PARAM_ACCOUNT_ID').Clear
  else
    spDetailAPISendMail.ParamByName('P_MAIL_PARAM_ACCOUNT_ID').AsInteger := 0;
  spDetailAPISendMail.ExecSQL;
        MessageDlg('Детализации высланы.', mtInformation, [mbOK], 0);
end;
  ...
  begin
   if IS_BILL_CHECK.Checked then
     try
       ...
       dsDetail.Enabled:=false;
       dsDetail.DataSet:=vtDetailFile;
       dsDetail.Enabled:=true;
...
//         cbPhoneNumber.ItemIndex := 0;
     ...
     begin
       ...
       if length(s)=1 then
         s:='0'+s;
       qHBDetails.SQL.Strings[qHBDetailsStrTableNameNum]:=' from call_'+s+'_'+inttostr(FYear)+' c10';
       qHBDetails.ParamByName('GROUP_ID').AsInteger := FGroupId;
       qHBDetails.ParamByName('YEAR_MONTH').AsString :=inttostr(FYear)+s;
       qHBDetails.Open;
       //Добавление номеров в список
       //Вариант: показываем в списке только номера, по которым есть детализации
       try
         cbPhoneNumber.Clear;
         qHBDetailsPhoneList.Close;
         qHBDetailsPhoneList.SQL.Strings[qHBDetailsPhoneStrTableNameNum]:=' from call_'+s+'_'+inttostr(FYear)+' c10';
         qHBDetailsPhoneList.ParamByName('GROUP_ID').AsInteger := FGroupId;
         qHBDetailsPhoneList.ParamByName('YEAR_MONTH').AsString :=inttostr(FYear)+s;
         qHBDetailsPhoneList.Open;
         qHBDetailsPhoneList.First;
         while not qHBDetailsPhoneList.Eof do
         begin
           cbPhoneNumber.Items.Add(qHBDetailsPhoneList.FieldByName('phone_number').AsString);
           qHBDetailsPhoneList.Next;
         end;

       finally
         qHBDetailsPhoneList.Close;
       end;
       //Вариант: показываем в списке все номера из группы
{       cbPhoneNumber.Clear;
       qContractGroup.First;
       while not qContractGroup.Eof do
       begin
...
begin
 if dsDetail.DataSet = vtDetailFile then begin
   vtDetailFile.Filtered:=false;
   if cbPhoneNumber.Text <> '' then begin
     vtDetailFile.Filter:='self_number='+QuotedStr(cbPhoneNumber.Text);
     vtDetailFile.Filtered:=true;
   end;
 end
 else begin
   qHBDetails.Filtered:=false;
   if cbPhoneNumber.Text <> '' then begin
     qHBDetails.Filter:='self_number='+QuotedStr(cbPhoneNumber.Text);
     qHBDetails.Filtered:=true;
   end;
 end;
end;

procedure TShowGroupStatForm.chkSendDetailAutoClick(Sender: TObject);
begin
  eSendDetailDay.Enabled:=chkSendDetailAuto.Checked;
  if not chkSendDetailAuto.Checked then
    eSendDetailDay.Text:='';
end;
...
begin
 ...
 try
  ...
  if savePdf.Execute then begin
    qBillsFrx.ParamByName('GROUP_ID').AsInteger:=FGroupId;
    qBillsFrx.ParamByName('YEAR_MONTH').AsInteger:=Integer(period);
    qAbonPeriodList.ParamByName('GROUP_ID').AsInteger:=FGroupId;
    qAbonPeriodList.ParamByName('YEAR_MONTH').AsInteger:=Integer(period);
    qOptsHistory.ParamByName('GROUP_ID').AsInteger:=FGroupId;
    qOptsHistory.ParamByName('YEAR_MONTH').AsInteger:=Integer(period);
    qBillMnRouming.ParamByName('GROUP_ID').AsInteger:=FGroupId;
    qBillMnRouming.ParamByName('YEAR_MONTH').AsInteger:=Integer(period);
    qTitle.ParamByName('GROUP_ID').AsInteger:=FGroupId;
    qTitle.ParamByName('YEAR_MONTH').AsString:=inttostr(Integer(period));
    ...
    if not qGroupParams.FieldByName('ParamPDF_Contacts').IsNull then begin
    ...
    (frxReport1.FindObject('Memo_Bill1') as TfrxMemoView).Text:='Счет № '+ Edit_bill.Text + ' ' + inttostr(FGroupId) + '/' + inttostr(Integer(period));
...
begin
  ...
  qContractGroup.Open;
  FCOLLECTOR := qContractGroup.FieldByName('IS_COLLECTOR').AsInteger;
...
begin
 try
   ...
   chkSendDetailAuto.Checked:=(qGroupParams.FieldByName('send_detail_auto').AsInteger = 1);
   email_balance_edit.text        := qGroupParams.FieldByName('email_balance').AsString;
   email_balance_period_Edit.Text := qGroupParams.FieldByName('email_balance_period').AsString;

   if qGroupParams.FieldByName('send_detail_auto').AsInteger = 1 then begin
     eSendDetailDay.Enabled:=true;
     eSendDetailDay.Text:=qGroupParams.FieldByName('send_detail_day').AsString;
   end
   else begin
     eSendDetailDay.Enabled:=false;
     eSendDetailDay.Text:='';
   end;
