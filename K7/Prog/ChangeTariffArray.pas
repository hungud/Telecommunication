...
uses
  ...
  CRGrid, MemDS, VirtualTable, DBAccess, Ora, ComObj, OraTransaction, DMUnit,
  IntecExportGrid, Main, ContractsRegistration_Utils;
...
type
  TChangeTariffArrayForm = class(TForm)
    ...
    procedure tbActiveClick(Sender: TObject);
...
procedure TChangeTariffArrayForm.tbActiveClick(Sender: TObject);
var i: integer;
begin
  vtChanges.First;
  i:=0;
  ...
  begin
    SetProgress(i);
    qCheckActive.Close;
    qCheckActive.ParamByName('PHONE_NUMBER').AsString:=vtChangesPhoneNumber.AsString;
    qCheckActive.Open;
    if (qCheckActive.RecordCount = 1) and (copy(vtChangesItog.AsString,1,19)='ТП изменен успешно.') then
    begin
      //Showmessage('SMS send');
      spSendSms.Close;
      spSendSms.ParamByName('PPHONENUMBER').AsString := vtChangesPhoneNumber.AsString;
      spSendSms.ExecSQL;
    end;
    vtChanges.Next;
    Inc(i);
  ...
  begin
    ...
    begin
     ...
     // VALUES(:CONTRACT_ID, :FILIAL_ID, TRUNC(:DATE_CHANGE), :SUM_GET, :NEW_TARIFF_ID, 4)
        ...
        qCheckChanges.Close;
        qCheckChanges.Open;
        if (GetConstantValue('USE_TYPE_PAYMENTS')='1') and (vtChangesChangePaymentReverseschet.AsInteger<>0) then begin
         qAddReversePayment.SQL.Text:= 'INSERT INTO RECEIVED_PAYMENTS(PHONE_NUMBER, PAYMENT_SUM, PAYMENT_DATE_TIME, ' +
                                      ' CONTRACT_ID, IS_CONTRACT_PAYMENT, FILIAL_ID, ' +
                                      ' PAYMENT_ANNUL_FLAG, REMARK, RECEIVED_PAYMENT_TYPE_ID, REVERSESCHET) ' +
                                      ' VALUES('''+vtChangesPhoneNumber.AsString+''', '+vtChangesChangePaymentReverseschet.AsString+', sysdate, ' +
                                      vtChangesContractId.AsString+', 0, '+inttostr (Integer(cbFilial.Items.objects[cbFilial.ItemIndex]))+', ' +
                                      ' 0, ''Оплата за смену ТП'', 21,1)';
         qAddReversePayment.Close;
         qAddReversePayment.ExecSQL;
        end;
...
procedure TChangeTariffArrayForm.FormCreate(Sender: TObject);
begin
