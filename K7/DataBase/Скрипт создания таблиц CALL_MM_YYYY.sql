...
begin
  ...
  for i  IN  month_start .. month_end LOOP
    ...
    EXECUTE IMMEDIATE 'CREATE TABLESPACE '||D||' DATAFILE   ''D:\ORACLE\ORADATA\K7\'||d||'_001.DBF'' SIZE 70M AUTOEXTEND ON NEXT 10M MAXSIZE UNLIMITED';
    ...
    EXECUTE IMMEDIATE 'COMMENT ON COLUMN '||D||'.ROAMING_PROVIDER_ID IS ''ID ÐÎÓÌÈÍÃ-ÏÐÎÂÀÉÄÅÐÀ ÈÇ ÒÀÁËÈÖÛ ROAMING_PROVIDERS''';
    ...
    EXECUTE IMMEDIATE 'CREATE INDEX '||D||'$COST$SSCR$ST$DBF ON '||D||'
      (CALL_COST, SUBSCR_NO, START_TIME, DBF_ID)
      LOGGING
      TABLESPACE '||D||'
      COMPRESS 2';
    ...
    EXECUTE IMMEDIATE 'GRANT DELETE, SELECT, UPDATE ON '||D||' TO CORP_MOBILE_ROLE';

    EXECUTE IMMEDIATE 'GRANT SELECT ON '||D||' TO CORP_MOBILE_ROLE_RO';
