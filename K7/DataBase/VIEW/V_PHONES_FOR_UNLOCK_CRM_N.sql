...
-- Version = 4
--
-- 4. 12.10.2015 Кочнев. Добавлены 141 код доп статуса
-- 3. 31.10.2014 Крайнов. Добавлены "Первоначальная"
-- 2. 21.10.2014 Крайнов. Исправление кода доп статуса.
--
  SELECT v.phone_number_federal,
    ...
    WHERE loader_script_name IS NOT NULL
     ...
     -- AND v.phone_is_active_code = 0
      AND v.hand_block = 0
      AND (NVL(v.DOP_STATUS_ID, -1) = -1   -- Без доп. статуса
            OR V.DOP_STATUS_ID = 222 )     -- "Первоначальная"
