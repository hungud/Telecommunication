...
-- Version = 11
-- 11. 2015.10.01. Крайнов. Отмена системы callback(не использовалась более 1.5года)
-- 10. 06.08.2015. Крайнов. Поправка фильтра по счетам.
-- 9. 15.07.2015. Крайнов. Поправка под раздельные кредиты по разным л/с. 
  ...
  SELECT TBFC.PHONE_NUMBER,
         ...
         CASE
           WHEN TRUNC(SYSDATE) <= ACCOUNT_LOADED_BILLS.DATE_CREDIT_END
                ...
                AND TBFC.YEAR_MONTH >= ACCOUNT_LOADED_BILLS.YEAR_MONTH
             THEN 'Счёт оператора без аб. пл. с ' || TO_CHAR(TO_DATE(TBFC.YEAR_MONTH, 'yyyymm'), 'DD.MM.YYYY')
                   || ' по ' || TO_CHAR(LAST_DAY(TO_DATE(TBFC.YEAR_MONTH, 'yyyymm')), 'DD.MM.YYYY')
           ELSE 'Счёт оператора с ' || TO_CHAR(TO_DATE(TBFC.YEAR_MONTH, 'yyyymm'), 'DD.MM.YYYY')
                || ' по ' || TO_CHAR(LAST_DAY(TO_DATE(TBFC.YEAR_MONTH, 'yyyymm')), 'DD.MM.YYYY')
         END AS REMARK,
         1 AS BILL_CHECKED, 
