...
--#Version=6
--
-- v.6 Афросин 15.07.2015 Добавил в комментарий Смена ТП по USSD при заказе смены через USSD команду 
...
CASE WHEN ticket_type = 15 THEN -- Подключение/отключение услуг
  ...
  ELSE
    'Команда ' || lower(NVL((select ticket_type_name from beeline_ticket_types where beeline_ticket_types.ticket_type_id=t.ticket_type), '???')) 
  END action,
  TO_CHAR (T.USER_CREATED)
    ||decode
          (
            nvl(
                 (
                   select 1 from USSD_CHANGE_PP_LOG us where us.TICKET_ID = t.TICKET_ID
                 )
                ,0
               ),
            1,
    ...
    || 'заявка '|| TO_CHAR (t.TICKET_ID)|| ' '|| DECODE (NVL (t.answer, '-1'), '-1', 'Не проверено', '0', 'Отклонена', '1', 'Исполнено', 'Иное')|| '. '
    || t.comments note
