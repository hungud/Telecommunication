...
--
--#Version=5
-- 2015.11.06.  райнов. ƒобавил GET_CONTRACT_ID_BY_YEAR_MONTH
...
--
  FUNCTION GET_CONTRACT_ID_BY_YEAR_MONTH(  
    pYEAR_MONTH IN NUMBER,
    pPHONE_NUMBER IN VARCHAR2
    ) RETURN INTEGER;
--
    ...
    CURSOR C IS
      SELECT V.CONTRACT_ID
        ...
        WHERE V.PHONE_NUMBER_FEDERAL = pPHONE_NUMBER
          AND TO_NUMBER(TO_CHAR(V.CONTRACT_DATE, 'YYYYMM')) <= pYEAR_MONTH
          AND TO_NUMBER(TO_CHAR(NVL(V.CONTRACT_CANCEL_DATE, SYSDATE), 'YYYYMM')) >= pYEAR_MONTH
        ORDER BY V.CONTRACT_DATE DESC;    
  BEGIN
    OPEN C;
    FETCH C INTO ITOG;
    IF C%NOTFOUND THEN
