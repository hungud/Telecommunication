...
-- 
--#Version=8
-- 8. Матюнин 29.07.2015 - изменил фуyкцию AUTH. Обновление токено крутиться в отдельном процессе. 
                        -- Фуyкция AUTH использует фунцкию AUTH2, которая берет валидный токен из таблицы с BEELINE_API_TOKEN_LIST
                        -- Если валидный токен отсутвует, генерится исключение и запрос по АПИ не идет, чтобы спамить Билайн и не быть заблокированным                           
-- 7. Бакунов (Изменения вносил Алексеев. Были доработки). Добавлены новые функции
...
function getSIMList(tok in varchar2, hashs in varchar2, pban in varchar2, phone in varchar2 default null) return SOAP_API_ANSWER_TYPE;

--получение информации о корректировках BAN
function getAdjustmentList(tok in varchar2, hashs in varchar2, pban in varchar2, startDate in date, endDate in date) return SOAP_API_ANSWER_TYPE;

--заказ отчета по детализации счета для списка CTN.
function createBillCallsRequest(tok in varchar2, hashs in varchar2, pban in varchar2, pAcc_id in number, billDate in date, CTNList IN VARCHAR2 DEFAULT NULL) return SOAP_API_ANSWER_TYPE;
...
-- Новая функция для получения валидного токена. В отличии от AUTH не запрашивает токен с сайта, чтобы не спамить билайн при "испортившемся" токене.  
--function auth2(login in varchar2,password in varchar2 ) return VARCHAR2;
...
--#Version=7
-- 7. Бакунов (Изменения вносил Алексеев. Были доработки). Добавлены новые функции
--v6. 23.01.2015 Афросин изменил получение статусов балансов
...
function auth2(login in varchar2) return VARCHAR2
is
  token  varchar2(32767); 
-- Получаем токен для заданного логина, если он валидный, либо выбрасываем исклоючение, чтобы не спамить билайн  
begin
  token := '-';
  select t.token
    into token
    from beeline_api_token_list_2 t 
   where t.acc_log = login
     and t.is_valid = 1 
   ; 
  IF TRIM(token) IS NULL THEN
    --RAISE_ERROR;
    raise_application_error(-20000, 'В таблице beeline_api_token_list токен пустой!');
  END IF;
  return (token);
exception    
  WHEN OTHERS THEN 
    IF token IS NOT NULL THEN 
      raise_application_error(-20000, 'В таблице beeline_api_token_list отсутвует валидный токен!');
    ELSE
      raise_application_error(-20000, 'В таблице beeline_api_token_list токен пустой!');
    END IF;   
end;
...
*/
begin
  -- токен обновляется теперь в GET_BEELINE_TOKEN
  token := auth2(login);
  return token; 
end; 
...
--получение номера SIM карты для BAN
FUNCTION getSIMList(tok IN VARCHAR2, hashs IN VARCHAR2, pban IN VARCHAR2, phone IN VARCHAR2 DEFAULT NULL) RETURN  SOAP_API_ANSWER_TYPE AS
    soap_text VARCHAR2(32767);
    sURL VARCHAR2(100);
    sINTERFACE VARCHAR2(100);
    Err_text VARCHAR2(1000);
...
BEGIN
    ...
    exception -- определяет головной BAN коллекторского счёта.
      when no_data_found then 
        select distinct w.account_id into Acc_id from BEELINE_LOADER_INF w where w.ban = pban;
    end;   
    --Запрос
    Err_text := create_conn(soap_text, sURL, sINTERFACE, phone, acc_id, 23);
    --sys_log
    save_sys_log (phone, acc_id, pban, 23, ret);

    RETURN  ret;
END;

--получение информации о корректировках BAN
FUNCTION getAdjustmentList(tok IN VARCHAR2, hashs IN VARCHAR2, pban IN VARCHAR2, startDate IN DATE, endDate IN DATE) RETURN SOAP_API_ANSWER_TYPE AS
    soap_text VARCHAR2(32767);
    sURL VARCHAR2(100);
    sINTERFACE VARCHAR2(100);
    Err_text VARCHAR2(1000);
...
BEGIN
    ...
    SELECT t.account_id INTO acc_id FROM accounts t WHERE t.account_number = pban;
    --Запрос
    Err_text := create_conn(soap_text, sURL, sINTERFACE, null, acc_id, 24);
    --sys_log
    save_sys_log (null, acc_id, pban, 24, ret);

    RETURN  ret;
END;

--заказ отчета по детализации счета для списка CTN. (список номеров указывается через запятую)
FUNCTION createBillCallsRequest(tok IN VARCHAR2, hashs IN VARCHAR2, pban IN VARCHAR2, pAcc_id in number, billDate IN DATE, CTNList IN VARCHAR2 DEFAULT NULL) RETURN SOAP_API_ANSWER_TYPE  AS
    soap_text VARCHAR2(32767);
    sURL VARCHAR2(100);
    sINTERFACE VARCHAR2(100);
    Err_text VARCHAR2(1000);
    pCTNListTag VARCHAR2(10000);
    iCTNList VARCHAR2(10000);
BEGIN
    ret := SOAP_API_ANSWER_TYPE(NULL, NULL, NULL);
    sURL := '/SubscriberService';
    ...
    pCTNListTag := '';
    iCTNList := CTNList;
    WHILE INSTR(iCTNList,',') > 0 LOOP
      --берем номер и создаем с ним строку CTN
      pCTNListTag := '<CTNList>'||SUBSTR(iCTNList, INSTR(iCTNList,',') - 10, 10)||'</CTNList>';
      --обрезаем строку
      iCTNList := SUBSTR(iCTNList, INSTR(iCTNList,',') + 1);
    END LOOP;
