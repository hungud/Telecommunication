...
--
--v6. 03.11.2015 Алексеев. Для номеров:
--                                      - с дискретным списанием абонки по тп,
--                                      - у которых в текущем месяце идет дискретное списание (учитываем сдвиг дискретного списания)
--                                      - у которых было зафиксирование сеписание абон. платы
--                                     устанавливаем в качестве величины разблока 0
--v5. 29.06.2015 Алексеев. Заменил таблицу contracts на views v_contracts
--v4. 11.02.2015 Алексеев. Добавил активные обещаные платежи
...
--v1. Алексеев. Добавил скрипт (отсутствовал)      
  ...
  BEGIN
    for C in (
      ...
      ,v_contracts dg,tariffs tr,abonents ab
            where
            --связи
            ...
            and DG.CONTRACT_CANCEL_DATE IS NULL
            and exists (select 1 
...
--            and p.payment_date>sysdate-1
            ...
            --
            group by p.phone_number,cb.balance,dg.disconnect_limit,dg.connect_limit,tr.balance_unblock, DG.CONTRACT_DATE, tr.DISCR_SPISANIE, tr.SDVIG_DISCR_SPISANIE
            --,cb.last_update
            ...
            having (cb.balance)- NVL (dg.disconnect_limit, 0)>
              NVL 
              (
                  dg.connect_limit, 
                  NVL 
                  (
                      CASE
                        WHEN (NVL (tr.DISCR_SPISANIE, 0) = 1) AND --если дискретное списание абон. платы по тарифу
                                  (TO_NUMBER (TO_CHAR (SYSDATE, 'YYYYMM')) >=TO_NUMBER (TO_CHAR (ADD_MONTHS(DG.CONTRACT_DATE, NVL(tr.SDVIG_DISCR_SPISANIE, 0)), 'YYYYMM'))) AND --учитываем сдвиг дискретного списания
                                  (CHECK_DISCR_SPISANIE_ABON (p.phone_number) = 1) THEN --проверяем наличие списания абонки
                          0
                        ELSE
                          tr.balance_unblock
                      END
