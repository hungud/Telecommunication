...
pack_size integer:=5;--25;-- размер для пачки запросов
...
function Get_account_bill(acc in number, ticket_id in number) return number is
  ...
  begin
    ...
    if length(regexp_substr(load_err,'\d+'))>5 then 
      return(regexp_substr(load_err,'\d+'));--если в ответе число больше 5 знаков - это LOG_ID
...
begin
  BillPeriod:=to_char(ADD_MONTHS(sysdate - 2,-1),'YYYYMM');--предыдущий месяц
-- BillPeriod:=201502;
...
--Начисления есть - грузим детальные 
  ...
  chek_account_bills;  
  BILLS_ACCOUNT_CHECK; -- проверка наличия счетов
  null;
