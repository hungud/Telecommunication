...
AS
  --v2. јлексеев. ƒобавлено условие отсутстви€ номера в табл. AUTO_BLOCKED_PHONE с номером за€вки и отсутстви€ номера в BEELINE_TICKETS
  --v1. јлексеев.
     ...
     WHERE 
            NOT EXISTS
              (
                ...
                AND (AUTO_BLOCKED_PHONE.BLOCK_DATE_TIME > SYSDATE - 30/24/60) -- Ќе более 30 минут паузы
                AND AUTO_BLOCKED_PHONE.Note like '%«а€вка на блок є%'
              )
     AND 
            NOT EXISTS
               (
  ...
  LOOP
    BEGIN
      ...
      IF NVL(vREC.PHONE_IS_ACTIVE, 0) <> 1 THEN
        --отправл€ем номер на переблок на сохранение
    ...
    EXCEPTION
      WHEN OTHERS THEN
        ...
        VALUES(vREC.PHONE_NUMBER, '—бой блокировки номера с доп. статусом на сохранение '||TO_CHAR(SYSDATE, 'DD.MM.YYYY HH24:MI:SS')||'. '||vSTR);
        COMMIT;
    END;
  END LOOP;
end;
