CREATE OR REPLACE PROCEDURE TARIFER_BILL_CREATE(
...
BEGIN
  ...
  IF vCOUNT = 0 THEN
    ...
    UPDATE TARIFER_BILL_FOR_CLIENTS B
      SET B.ABON_TP = NVL((SELECT SUM(-x.ROW_COST)
                             ...
                             WHERE X.PHONE_NUMBER = B.PHONE_NUMBER
                               AND TO_NUMBER(TO_CHAR(x.ROW_DATE, 'YYYYMM')) = B.YEAR_MONTH
          ...
          B.SINGLE_CHANGE_TARIFF = NVL((SELECT SUM(-x.PAYMENT_SUM)
                                          ...
                                          WHERE X.PHONE_NUMBER = B.PHONE_NUMBER
                                            AND TO_NUMBER(TO_CHAR(x.PAYMENT_DATE_TIME , 'YYYYMM')) = B.YEAR_MONTH
                                            AND X.RECEIVED_PAYMENT_TYPE_ID in (21, 41)
                                            AND X.IS_CONTRACT_PAYMENT = 0
                                            AND X.REVERSESCHET = 1), 0),
          B.SINGLE_CORRECTION = NVL((SELECT SUM(-x.PAYMENT_SUM)
                                        FROM RECEIVED_PAYMENTS X
                                        WHERE X.PHONE_NUMBER = B.PHONE_NUMBER
                                          AND TO_NUMBER(TO_CHAR(x.PAYMENT_DATE_TIME , 'YYYYMM')) = B.YEAR_MONTH
    ...
    -- Сбор Итого.       
    UPDATE TARIFER_BILL_FOR_CLIENTS B
      SET B.BILL_SUMM = B.BILL_SUMM + B.ABON_TP + B.ABON_ADD + B.CALLS + B.SINGLE_PAYMENTS + B.SINGLE_CHANGE_TARIFF + B.SINGLE_CORRECTION,
          B.SINGLE_PAYMENTS = B.SINGLE_PAYMENTS + B.SINGLE_CHANGE_TARIFF + B.SINGLE_CORRECTION
      WHERE B.YEAR_MONTH = pYEAR_MONTH
        AND B.PHONE_NUMBER = pPHONE_NUMBER;
    -- Присоединение звонков по детализации
    UPDATE TARIFER_BILL_FOR_CLIENTS B
      SET B.CALLS_SMS_MMS = nvl((select sum(d.MMS_COST + d.SMS_COST)
                                   from db_loader_phone_stat d
                                   where d.YEAR_MONTH = b.YEAR_MONTH
                                     and d.PHONE_NUMBER = b.PHONE_NUMBER), 0),
          B.CALLS_GPRS = nvl((select sum(d.INTERNET_COST)
                                from db_loader_phone_stat d
                                where d.YEAR_MONTH = b.YEAR_MONTH
                                  and d.PHONE_NUMBER = b.PHONE_NUMBER), 0),
          B.CALLS_LOCAL = nvl((select sum(d.CALLS_COST)
                                 from db_loader_phone_stat d
                                 where d.YEAR_MONTH = b.YEAR_MONTH
                                   and d.PHONE_NUMBER = b.PHONE_NUMBER), 0),
          B.CALLS_RUS_RPP = b.CALLS - nvl((select sum(d.CALLS_COST + d.INTERNET_COST + d.MMS_COST + d.SMS_COST)
                                             from db_loader_phone_stat d
                                             where d.YEAR_MONTH = b.YEAR_MONTH
                                               and d.PHONE_NUMBER = b.PHONE_NUMBER), 0) 
      WHERE B.YEAR_MONTH = pYEAR_MONTH
