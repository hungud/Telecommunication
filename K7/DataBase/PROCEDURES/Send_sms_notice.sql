...
--#Version=24
--
--24 30.10.2015 Алексеев А.П. Ограничил отправку смс если одновременно выполняются след. условия:
...
--                            Выполнение данных условий проверяет функция CHECK_SEND_SMS_NOTICE
--23 15.08.2015 Афросин А.Ю. Добавил параметр %contract%
...
--  
  ...
  vCALL_CENTER_CONTACTS_STR varchar2(150 char);
--
BEGIN
 --Проверка на время отправки - по Будням с 9 до 18 в выходные с 12 до 18.
  IF ((TO_NUMBER(TO_CHAR(SYSDATE,'D','NLS_DATE_LANGUAGE=RUSSIAN'))<6) AND (SYSDATE-TRUNC(SYSDATE)>9/24) AND (SYSDATE-TRUNC(SYSDATE)<18/24))
    ...
    vCALL_CENTER_CONTACTS_STR := MS_PARAMS.GET_PARAM_VALUE('CALL_CENTER_CONTACTS_STR');
    ...
    LOOP--0:00.61*n
      --проверяем возможность отправки смс 
      IF CHECK_SEND_SMS_NOTICE(vREC.PHONE_NUMBER_FEDERAL) = 1 THEN
        ...
        IF acc_notice_is_on(vREC.ACCOUNT_ID)=1 and 
          ...
          IF (SQL%NOTFOUND) THEN
            ...
            SMS_TXT:=replace(SMS_TXT,'%contact%', vCALL_CENTER_CONTACTS_STR);
