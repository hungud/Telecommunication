GRANT SELECT ON DB_LOADER_PHONE_STAT TO K7_LK;

CREATE OR REPLACE FUNCTION K7_LK.GET_PHONE_CURRENT_STATS(
  pPHONE_NUMBER IN VARCHAR2,
  pYEAR_MONTH IN NUMBER DEFAULT NULL
  ) RETURN K7_LK.T_PHONE_STATS_TAB PIPELINED AS
--#Version=6
--
-- Возвращается статистику использования номера за период
-- Используем как
--   SELECT * FROM TABLE(GET_PHONE_CURRENT_STATS('9689681650'))
-- Для компиляции делаем GRANT SELECT ON DB_LOADER_PHONE_STAT TO K7_LK;
--
...
-- select * from table(K7_LK.GET_PHONE_CURRENT_STATS(:pPHONE_NUMBER))
