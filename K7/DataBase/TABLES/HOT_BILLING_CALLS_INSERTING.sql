...
COMMENT ON COLUMN HOT_BILLING_CALLS_INSERTING.AT_C_DR_SC IS 'Длительность (сек.)';
COMMENT ON COLUMN HOT_BILLING_CALLS_INSERTING.AT_CDRRD_M IS 'Длительность (мин., с округлением)';
...
CREATE UNIQUE INDEX PK_HOT_BILLING_CALLS_INSERTING ON HOT_BILLING_CALLS_INSERTING(SUBSCR_NO, CH_SEIZ_DT, US_SEQ_N) COMPRESS 1;

CREATE INDEX I_HB_INS_CALLS$SUB_AT ON HOT_BILLING_CALLS_INSERTING(DBF_ID, SUBSCR_NO, AT_FT_CODE) COMPRESS 3;

CREATE INDEX I_HB_INS_CALLS$SUBSCR_NO ON HOT_BILLING_CALLS_INSERTING(SUBSCR_NO);

CREATE INDEX I_HB_INS_CALLS$DBF_ID$IDX ON HOT_BILLING_CALLS_INSERTING(DBF_ID);

CREATE INDEX I_HB_INS_CALLS$SUBSCR_NO$CH ON HOT_BILLING_CALLS_INSERTING(SUBSCR_NO, CH_SEIZ_DT);

GRANT DELETE, INSERT, SELECT, UPDATE ON HOT_BILLING_CALLS_INSERTING TO CORP_MOBILE_ROLE;

GRANT SELECT ON HOT_BILLING_CALLS_INSERTING TO CORP_MOBILE_ROLE_RO;