...
--#if not RecordExists("SELECT * FROM constants WHERE NAME='HOUR_START_SEND_PROMIZED_MESS'") then
INSERT INTO CONSTANTS (NAME, DESCR, TYPE, VALUE)
  VALUES ('OPTION_CAN_BE_TURNED_BY_ABON', 'Показывать в Справочнике "Тарифные опции" поля:CAN_BE_TURNED_BY_ABONENT,USSD_TURN_ON_COMMAND,USSD_TURN_OFF_COMMAND ', 'B', 1);    
--#end if



alter table contracts add (with_auto_internet_turn_on number(1) default 0);

COMMENT ON COLUMN CONTRACTS.with_auto_internet_turn_on IS 'Признак автоподключения internet доп.пакета(опции)';

Insert into CONSTANTS
   (NAME, DESCR, TYPE, VALUE,  
    ACCESS_VERSTION_STRING)
 Values
   ('GET_ABON_BALANCE_COLUMN_NAME', 'список интеграционный веб-сервиса K7, доступного по ссылке http://109.95.210.44:81/balance?phone_number=номер телефона', 'S', 
    'balance, is_active as "activity", dop_status as "dopstatus", tariff_name as "tarif", updated, possible_to_unlock as "is_possible_to_unlock", ban, promised_payment_status, possible_voluntary_block', 
    '1.8.26.81');
commit;

--#if not RecordExists("SELECT * FROM constants WHERE NAME='SHOW_BLOCKPHONE_ONEPAYMENT'") then
INSERT INTO CONSTANTS (NAME, DESCR, TYPE, VALUE, ACCESS_VERSTION_STRING)
  VALUES ('USER_ACCESS_HIDE_DETAIL', 'Пользователи с разрешением скрывать детализацию' , 'S', 'NIKOLAY', '1.8.26.89');
--#end if

Insert into CONSTANTS
   (NAME, DESCR, TYPE, VALUE, ACCESS_VERSTION_STRING)
 Values
   ('EXPORT_OBJECTS_TO_SVN_PATH', 'Путь для выгрузки файлов скриптов Базы данных', 'S', 'D:\Tarifer\DB_SCRIPTS',  '1.8.26.91');
COMMIT;
...
--#if not RecordExists("SELECT * FROM constants WHERE NAME='CALC_OPTIONS_DAILY_ACTIV'") then
INSERT INTO CONSTANTS (NAME, DESCR, TYPE, VALUE)
  VALUES ('CALC_OPTIONS_DAILY_ACTIV', 'Списание посуточных услуг с учетом активности', 'B', 1);    
--#end if

--#if not RecordExists("SELECT * FROM constants WHERE NAME='CALC_ABON_TP_UNLIM_ON'") then
INSERT INTO CONSTANTS (NAME, DESCR, TYPE, VALUE)
  VALUES ('CALC_ABON_TP_UNLIM_ON', 'Включение механизма по списанию абон платы по тп безлимит (тп с автоподключением) с отсрочкой', 'B', 0);
--#end if

--#if not RecordExists("SELECT * FROM constants WHERE NAME='SHIFT_SERVER_TIME'") then
INSERT INTO CONSTANTS (NAME, DESCR, TYPE, VALUE)
  VALUES ('SHIFT_SERVER_TIME', 'Сдвиг серверного времни для списаний абон плат заранее(мин)', 'N', 60);  
--#end if

--#if not RecordExists("SELECT * FROM constants WHERE NAME='COUNT_DAYS_AUTO_BLOCK_FRAUD'") then
INSERT INTO CONSTANTS (NAME, DESCR, TYPE, VALUE)
  VALUES ('COUNT_DAYS_AUTO_BLOCK_FRAUD', 'Кол-во дней после активации (для условия авто.блокировки со статусом фрод)', 'N', 5);    
--#end if

--#if not RecordExists("SELECT * FROM constants WHERE NAME='COUNT_DAYS_AUTO_BLOCK_FRAUD'") then
INSERT INTO CONSTANTS (NAME, DESCR, TYPE, VALUE)
  VALUES ('COUNT_STREAM_HOT_BILLING', 'Кол-во потоков для загрузки гор.биллинга', 'N', 1);    
--#end if