...
-- #end if  
 ...
 --#if not ColumnExists("CONTRACT_GROUPS.send_detail_auto") then
 -- Add/modify columns 
alter table CONTRACT_GROUPS add send_detail_auto NUMBER(1);
-- Add comments to the columns 
comment on column CONTRACT_GROUPS.send_detail_auto
  is 'Параметр: Автоматически отправлять детализацию';
--#end if 

 --#if not ColumnExists("CONTRACT_GROUPS.send_detail_day") then
 -- Add/modify columns 
alter table CONTRACT_GROUPS add send_detail_day NUMBER(2);
-- Add comments to the columns 
comment on column CONTRACT_GROUPS.send_detail_day
  is 'Параметр: День месяца, после которого необходимо отправлять детализацию';
--#end if 


alter table CONTRACT_GROUPS add (email_balance varchar2(128) default '');
COMMENT ON COLUMN CONTRACT_GROUPS.email_balance IS 'e-mail для рассылки баланса группы';

alter table CONTRACT_GROUPS add (email_balance_period number(4) default 0);
COMMENT ON COLUMN CONTRACT_GROUPS.email_balance_period IS 'Период рассылки баланса группы по e-mail';

alter table CONTRACT_GROUPS add (email_balance_last date default null);
COMMENT ON COLUMN CONTRACT_GROUPS.email_balance_last IS 'Дата последней отправки баланса группы по e-mail';
