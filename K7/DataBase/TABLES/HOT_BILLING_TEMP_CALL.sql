...
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.PROV_ID IS 'ID РОУМИНГ-ПРОВАЙДЕРА ИЗ ТАБЛИЦЫ ROAMING_PROVIDERS';

Alter table HOT_BILLING_TEMP_CALL add(AT_SOC VARCHAR2(9 BYTE));
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.AT_SOC IS 'Услуга, по которой определяется тарификация';

Alter table HOT_BILLING_TEMP_CALL add(PC_PLAN_CD VARCHAR2(9 BYTE));
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.PC_PLAN_CD IS 'Тарифный план';

Alter table HOT_BILLING_TEMP_CALL add(C_ACT_CD VARCHAR2(1 BYTE));
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.C_ACT_CD IS 'Тип вызова';

Alter table HOT_BILLING_TEMP_CALL add(MESSAGE_TP VARCHAR2(1 BYTE));
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.MESSAGE_TP IS 'Тип записи: домашний или роуминг';

Alter table HOT_BILLING_TEMP_CALL add(SRV_FT_CD VARCHAR2(6 BYTE));
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.SRV_FT_CD IS 'Сервис (код)';

Alter table HOT_BILLING_TEMP_CALL add(UOM VARCHAR2(2 BYTE));
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.UOM IS 'Единица измерения';

Alter table HOT_BILLING_TEMP_CALL add(DISCT_SOCS VARCHAR2(100 BYTE));
COMMENT ON COLUMN HOT_BILLING_TEMP_CALL.DISCT_SOCS IS 'Услуга-скидка';
