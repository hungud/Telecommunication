...
uses ShowUserStat, IntecExportGrid, Main;
...
//  FAccid:='-2';
  ...
  try
    ...
    if ReportFrm.UserCheckAllow then begin
      ReportFrm.qAccounts_Allow.ParamByName('USER_NAME').AsString:= mainform.OraSession.Username;
...
//    ReportFrm.aRefresh.Execute;
    if ReportFrm.UserCheckAllow then begin
      ...
      ReportFrm.qReport.ParamByName('USER_NAME').AsString := mainform.OraSession.Username;
    end;
...
begin
  ...
  begin
   ...
   // MessageDlg('Текст запроса: '+sql_Text, mtError, [mbOK], 0);
       sql.Add(sql_Text);
       Prepare;
       if FAccountAll and UserCheckAllow then ParamByName('USER_NAME').AsString := mainform.OraSession.Username;
       ExecSQL;
       Open;
