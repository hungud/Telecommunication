object ReportSummaryMinutesFrm: TReportSummaryMinutesFrm
  Left = 203
  Top = 134
  Caption = #1054#1090#1095#1105#1090' '#1086' '#1073#1077#1089#1087#1083#1072#1090#1085#1099#1093' '#1084#1080#1085#1091#1090#1072#1093
  ClientHeight = 371
  ClientWidth = 894
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  Icon.Data = {
    }
  OldCreateOrder = False
  PopupMode = pmAuto
  Position = poScreenCenter
  OnCreate = FormCreate
  PixelsPerInch = 96
  TextHeight = 13
  object grData: TCRDBGrid
    Left = 0
    Top = 29
    Width = 894
    Height = 342
    Align = alClient
    DataSource = DataSource1
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete, dgCancelOnExit]
    ParentFont = False
    ReadOnly = True
    TabOrder = 0
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = [fsBold]
    OnDblClick = aShowUserStatInfoExecute
    OnKeyDown = grDataKeyDown
    Columns = <
      item
        Expanded = False
        FieldName = 'PHONE_NUMBER'
        Title.Alignment = taCenter
        Title.Caption = #1053#1086#1084#1077#1088
        Width = 92
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'TARIFF_NAME'
        Title.Alignment = taCenter
        Title.Caption = #1058#1072#1088#1080#1092
        Width = 228
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'YEAR_MONTH'
        Title.Alignment = taCenter
        Title.Caption = #1043#1086#1076'/'#1084#1077#1089#1103#1094
        Width = -1
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'IS_CONTRACT'
        Title.Alignment = taCenter
        Title.Caption = #1050#1086#1085#1090#1088#1072#1082#1090'('#1076#1072'/'#1085#1077#1090')'
        Width = 119
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'IS_ACTIVE'
        Title.Alignment = taCenter
        Title.Caption = #1040#1082#1090#1080#1074#1077#1085'/'#1041#1083#1086#1082#1080#1088#1086#1074#1072#1085
        Width = 147
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ZEROCOST_OUTCOME_COUNT'
        Title.Alignment = taCenter
        Title.Caption = #1041#1077#1089#1087#1083#1072#1090#1085#1099#1093'|'#1079#1074#1086#1085#1082#1086#1074
        Width = 85
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'ZEROCOST_OUTCOME_MINUTES'
        Title.Alignment = taCenter
        Title.Caption = #1041#1077#1089#1087#1083#1072#1090#1085#1099#1093'|'#1084#1080#1085#1091#1090
        Width = 78
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CALLS_COUNT'
        Title.Alignment = taCenter
        Title.Caption = #1055#1083#1072#1090#1085#1099#1093'|'#1079#1074#1086#1085#1082#1086#1074
        Width = 63
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CALLS_MINUTES'
        Title.Alignment = taCenter
        Title.Caption = #1055#1083#1072#1090#1085#1099#1093'|'#1084#1080#1085#1091#1090
        Width = 65
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'CALLS_COST'
        Title.Alignment = taCenter
        Title.Caption = #1055#1083#1072#1090#1085#1099#1093'|'#1089#1090#1086#1080#1084#1086#1089#1090#1100
        Width = 68
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 0
    Width = 894
    Height = 29
    Align = alTop
    TabOrder = 1
    object Label1: TLabel
      Left = 179
      Top = 8
      Width = 49
      Height = 13
      Caption = #1055#1077#1088#1080#1086#1076':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object lAccount: TLabel
      Left = 8
      Top = 8
      Width = 62
      Height = 13
      Caption = #1051#1080#1094'. '#1089#1095#1077#1090':'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
    end
    object BitBtn1: TBitBtn
      Left = 339
      Top = 0
      Width = 151
      Height = 29
      Action = aExcel
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' MS Excel'
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 0
    end
    object BitBtn2: TBitBtn
      Left = 490
      Top = 0
      Width = 105
      Height = 29
      Action = aRefresh
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 1
    end
    object BitBtn3: TBitBtn
      Left = 595
      Top = 0
      Width = 111
      Height = 29
      Action = aShowUserStatInfo
      Caption = #1055#1086#1076#1088#1086#1073#1085#1086
      Glyph.Data = {
        
        FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00FF00}
      ParentShowHint = False
      ShowHint = True
      TabOrder = 2
    end
    object cbPeriod: TComboBox
      Left = 233
      Top = 3
      Width = 99
      Height = 21
      Style = csDropDownList
      TabOrder = 3
      OnChange = cbPeriodChange
    end
    object cbSearch: TCheckBox
      Left = 712
      Top = 6
      Width = 60
      Height = 17
      Caption = #1055#1086#1080#1089#1082
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 4
      OnClick = cbSearchClick
    end
    object cbAccounts: TComboBox
      Left = 76
      Top = 3
      Width = 98
      Height = 21
      Style = csDropDownList
      TabOrder = 5
      OnChange = cbAccountsChange
    end
    object cbAllPhones: TCheckBox
      Left = 772
      Top = 6
      Width = 93
      Height = 17
      Caption = #1042#1089#1077' '#1085#1086#1084#1077#1088#1072
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'MS Sans Serif'
      Font.Style = [fsBold]
      ParentFont = False
      TabOrder = 6
      OnClick = cbAllPhonesClick
    end
  end
  object qReport: TOraQuery
    SQL.Strings = (
      'SELECT * FROM V_REPORT_SUMMARY_MINUTES V'
      '  WHERE v.YEAR_MONTH=:YEAR_MONTH'
      '    AND ((:ACCOUNT_ID IS NULL)OR(v.ACCOUNT_ID=:ACCOUNT_ID))'
      '  ORDER BY v.ZEROCOST_OUTCOME_MINUTES')
    FetchRows = 250
    ReadOnly = True
    IndexFieldNames = 'ZEROCOST_OUTCOME_MINUTES DESC'
    Left = 56
    Top = 120
    ParamData = <
      item
        DataType = ftUnknown
        Name = 'YEAR_MONTH'
      end
      item
        DataType = ftUnknown
        Name = 'ACCOUNT_ID'
      end>
  end
  object DataSource1: TDataSource
    DataSet = qReport
    Left = 88
    Top = 128
  end
  object ActionList1: TActionList
    Images = MainForm.ImageList24
    Left = 56
    Top = 72
    object aExcel: TAction
      Caption = #1042#1099#1075#1088#1091#1079#1080#1090#1100' '#1074' MS Excel'
      ImageIndex = 12
      OnExecute = aExcelExecute
    end
    object aRefresh: TAction
      Caption = #1054#1073#1085#1086#1074#1080#1090#1100
      ImageIndex = 9
      OnExecute = aRefreshExecute
    end
    object aShowUserStatInfo: TAction
      Caption = #1055#1086#1076#1088#1086#1073#1085#1086
      Hint = #1055#1088#1086#1089#1084#1086#1090#1088' '#1080#1085#1092#1086#1088#1084#1072#1094#1080#1080' '#1087#1086' '#1072#1073#1086#1085#1077#1085#1090#1091
      ImageIndex = 22
      ShortCut = 13
      OnExecute = aShowUserStatInfoExecute
    end
  end
  object qPeriods: TOraQuery
    SQL.Strings = (
      'SELECT '
      '  YEAR_MONTH,'
      '  '#39'  '#39' || TO_CHAR(YEAR_MONTH - (TRUNC(YEAR_MONTH / 100) * 100)) '
      '    || '#39' - '#39
      '    || TRUNC(YEAR_MONTH / 100) YEAR_MONTH_NAME'
      'FROM V_REPORT_SUMMARY_MINUTES'
      'GROUP BY YEAR_MONTH'
      'ORDER BY YEAR_MONTH DESC')
    ReadOnly = True
    Left = 152
    Top = 120
  end
  object dsPeriods: TDataSource
    AutoEdit = False
    DataSet = qPeriods
    Left = 184
    Top = 128
  end
  object qAccounts: TOraQuery
    Session = MainForm.OraSession
    SQL.Strings = (
      'SELECT ACCOUNT_ID, LOGIN '
      'FROM ACCOUNTS '
      'ORDER BY LOGIN ASC')
    Left = 152
    Top = 72
  end
  object qFastPeriods: TOraQuery
    Session = MainForm.OraSession
    SQL.Strings = (
      'SELECT '
      '  YEAR_MONTH,'
      '  '#39'  '#39' || TO_CHAR(YEAR_MONTH - (TRUNC(YEAR_MONTH / 100) * 100)) '
      '    || '#39' - '#39
      '    || TRUNC(YEAR_MONTH / 100) YEAR_MONTH_NAME'
      'FROM DB_LOADER_PHONE_STAT'
      'GROUP BY YEAR_MONTH'
      'ORDER BY YEAR_MONTH DESC')
    ReadOnly = True
    Left = 264
    Top = 120
  end
end
