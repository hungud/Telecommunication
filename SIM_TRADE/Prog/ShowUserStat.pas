...
begin
 ...
 // BitBtn5.Visible:=(GetConstantValue('SERVER_NAME')='CORP_MOBILE');
  ...
  try
    ...
    begin
      ...
      if GetConstantValue('SERVER_NAME') = 'CORP_MOBILE' then
        if dmShowUserSt.qContractInfo.FieldByName('DISCONNECT_LIMIT').AsInteger = 0 then
          ...
          lCreditLimit.Caption:= 'Кредитный лимит:    '
    ...
    //Показываем только для GSM_CORP
...
qBH:TOraQuery;
begin
//if not (GetConstantValue('SERVER_NAME')='CORP_MOBILE') then exit;  //только для K7
 try
...
var fList:TForm;
    ...
    tBH:TDBGrid;
begin
  //if not ((GetConstantValue('SERVER_NAME')='CORP_MOBILE') OR (GetConstantValue('SERVER_NAME')='GSM_CORP')) then exit;  //только для K7 и ЖСМ
  flist:=tform.Create(ShowBillDetailsForm);
  with flist do begin
    //  FormStyle:=fsMDIChild;
    BorderStyle:=bsSingle;
...
//подготовка к выгрузке Excel
  ...
  if IS_BILL_CHECK.Checked then
    ...
    ANote := '*Предварительные начисления могут отличаться от детализации за закрытый биллинговый период.';
  ACaption := 'Детализация по номеру '
      + FPhoneNumber
      + ' за '
      + dmShowUserSt.qPeriods.FieldByName('MONTH').AsString
 ...
 //  FreeAndNil(VBODY);
  ...
  begin
    ...
    begin
      ...
      vRemark:='Детализация '+DateToStr(dtBegin.Date)+'-'+DateToStr(dtEnd.Date)+' '+edEmailAdress.Text;
      ...
      try
        ...
        on e : exception do
          MessageDlg('Ошибка вставки записи в таблицу ручных платажей', mtError, [mbOK], 0);
      end;
    end;
    Destroy;
  end;
...
procedure TShowUserStatForm.BUnbilledCallsListClick(Sender: TObject);
  ...
  begin
    cr := Screen.Cursor;
    Screen.Cursor := crHourGlass;
    ACaption := 'Необилинная детализация по номеру '+(sender as TCRDBGrid).Hint;
...
begin
  ...
  begin
    //  FormStyle:=fsMDIChild;
    BorderStyle := bsSingle;
    Caption := 'Необилинная детализация по номеру '+FPhoneNumber;
    Position := poScreenCenter;
    Width := 760;
    Height := 540;
  end;
  ...
  try
    ...
    with tbh.Columns do
    begin
      Items[0].Title.caption := 'Дата\Время';
      Items[1].Title.caption := 'От';
      Items[1].Width := 70;
      Items[2].Title.caption := 'Кому';
      Items[2].Width := 70;
      Items[3].Title.caption := 'Услуга';
      Items[3].Width := 100;
      Items[4].Title.caption := 'Тип вызова';
      Items[4].Width := 100;
      Items[5].Title.caption := 'Объем данных';
      Items[5].Width := 90;
      Items[6].Title.caption := 'Стоимость';
      Items[6].Width := 60;
      Items[7].Title.caption := 'Длительность';
      Items[7].Width := 90;
    end;
...
begin
  ...
  f_Mess:=false; // сообщение на экран: true - письмо не отправляется, а выводится сообщение на экран
   ...
   begin
    ...
    vSMTP_TITLE:=MainForm.OraSession.Username+' : Смена доп.статуса по номеру '+FPhoneNumber;
    f_Send:=true;
    end;
   ...
   begin
    ...
    vSMTP_TITLE:=MainForm.OraSession.Username+' : ОТКАЗ от номера '+FPhoneNumber;
    f_Send:=true;
    end;
