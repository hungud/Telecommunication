...
type
  TReportBalanceFrm = class(TForm)
    ...
    procedure cbCreditClick(Sender: TObject);
...
//          or(ReportFrm.grData.Columns[i].FieldName='ACCOUNT_NUMBER')
    ...
    if ReportFrm.cbOperatorNames.Items.Count > 2 then
      ...
      if ReportFrm.cbOperatorNames.Items.Count > 0 then
       ...
       // ShowMessage(GetConstantValue('REPORTS_USE_ACCOUNT_THREADS'));
...
begin
 ...
 { if cbrefresh.Checked=False then
  ...
  //label2.Caption:=inttostr(session_id);
  SHOW_ProgressBar ;
...
procedure TReportBalanceFrm.cbCreditClick(Sender: TObject);
begin
 ...
 with qReport do
              begin
                try
                  ...
                  sql_text:='SELECT * FROM V_ABONENT_BALANCES_WITHOUTSORT ';
                  sql_text:=sql_text+' WHERE   (:OPERATOR_ID IS NULL ';
                  sql_text:=sql_text+' OR V_ABONENT_BALANCES_WITHOUTSORT.OPERATOR_NAME = :OPERATOR_NAME or (:OPERATOR_NAME =''Все''))';
                    if FAccid<>'-1'  then
      ...
      begin
        ...
        begin
          try
            ...
            if cbCredit.Checked then
              sql.Text:='begin temp_balphone('+IntToStr(session_id)+','''+FAccid+''',1); end;'
            else
