...
AS
    --#Version=2
    --
	--v2  03.06.2015 - Алексеев А.П. - добавил учет исключающих job
...
BEGIN
    BEGIN
        ...
        --стопаем все job
        execute immediate 'SELECT owner||''.''||job_name 
		                   FROM DBA_SCHEDULER_JOBS 
						   WHERE STATE=''RUNNING'' 
						   AND OWNER = (SELECT SYS_CONTEXT (''userenv'', ''CURRENT_SCHEMA'') FROM DUAL)
						   AND job_name NOT IN (SELECT NAME_STOP_JOB FROM STOP_JOB_EXCEPTION)' BULK COLLECT INTO vJOB;
