...
--
--
-- 20. 09,10,2014 Афросин - в LOAD_ACCOUNT_DETAILS выскакивала ошибка если запрос не возвращал данных
-- добавил проверку  if vPHONES.count>0 then
...
--    AND DB_LOADER_ACCOUNT_PHONES.PHONE_IS_ACTIVE = 1
    ...
    FOR rec IN (
      SELECT DB_LOADER_ACCOUNT_PHONES.PHONE_NUMBER
       ...
       /* ORDER BY -- Сначала идут давно не загружаемые.
          ...
          ORDER BY -- Сначала те, кому не надо идти на сайт.
          DB_LOADER_PHONE_STAT.LAST_CHECK_DATE_TIME desc
   ...
   /*     SELECT strInLike(21,nvl(ac.n_method,'0;'),';','()')
    ...
    if  pmethod = 0 then
    --RAISE_APPLICATION_ERROR(-20000, vPHONES2(3));
        if s.ACCOUNT_ID <> 105 then --исправление загрузки деталки с сайта для 105 лс
