...
t.DATA_VOL,t.CELL_ID,t.MN_UNLIM,sysdate)';
  ...
  BEGIN
    ...
    */
    ...
    open curnm for 'select /*+index(c CALL_' || to_char(smonth, 'mm_yyyy') || '$subscr#stime$IDX)*/ c.servicetype,c.servicedirection,decode(c.call_cost,0,0,1),count(*),sum(c.call_cost),
            sum(c.COST_CHNG),
sum(decode(c.servicetype,''C'',ceil(c.dur/60),''G'',c.dur/1024,''W'',c.dur/1024/1024,c.dur)),
sum(decode(c.servicetype,''C'',c.dur/60,''G'',c.dur/1024,''W'',c.dur/1024/1024,c.dur))
 from (select distinct subscr_no, start_time, dbf_id, call_cost, costnovat, dur, imei, servicetype, servicedirection,
call_date, call_time, duration, dialed_dig,calling_no, at_chg_amt, data_vol,
cell_id, mn_unlim, insert_date,COST_CHNG from call_' || to_char(smonth,
                                                          'mm_yyyy') || ' ) c
where c.start_time>nvl(DB_LOADER_PCKG.GET_PHONE_BALANCE_DATE(c.subscr_no),trunc(sysdate,''mm'')-1/86400)
and (c.servicetype=''C''
or (c.servicetype=''S'')
...
group by c.servicetype,c.servicedirection,decode(c.call_cost,0,0,1)';
    ...
    loop
      FETCH curnm
        into sst,sd, zf, coun, scost, pCOST_CHNG, sdurcm, sdurm;
      EXIT WHEN curnm%NOTFOUND;
      case sst
        when 'C' then
          if zf = 0 then
...
c.servtype,c.servicedirection,decode(c.call_cost,0,0,1),count(*),sum(c.call_cost),
        sum(c.COST_CHNG),
...
sum(decode(c.servtype,''C'',c.dur/60,''B'',c.dur/60,''G'',c.dur/1024,''W'',c.dur/1024/1024,c.dur))
 from (select distinct subscr_no, start_time, at_ft_code,dbf_id, call_cost, costnovat, dur, imei, case at_ft_code
when ''CBIN'' then ''B''
when ''CBOUT'' then ''B''
else
servicetype
end servtype, servicedirection,
call_date, call_time, duration, dialed_dig,calling_no, at_chg_amt, data_vol,
cell_id, mn_unlim, insert_date,COST_CHNG from call_' || to_char(smonth,
                                                          'mm_yyyy') || ' ) c
where c.start_time>(SELECT nvl(DB_LOADER_PCKG.GET_PHONE_BALANCE_DATE(:phone_number),trunc(sysdate,''mm'')-1/86400) FROM DUAL)
