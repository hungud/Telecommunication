...
PROCEDURE CalcDetailSum(ssubscr in varchar2,
                        smonth  in date);
PROCEDURE CalcDetailSumOpt(ssubscr in varchar2,
                        smonth  in date);
...
/
/* Formatted on 19.01.2015 13:41:08 (QP5 v5.252.13127.32867) */
CREATE OR REPLACE PACKAGE BODY HOT_BILLING_PCKG AS
  --
  --#Version=6
  -- Крайнов изменения для ввода прогрузки деталок по АПИ.
  ...
  PROCEDURE SAVE_CALL(modn in integer, resm in integer, sitef in integer) IS
    ...
    cursor curf is
      select /*+index(htc HB_TEMP_CALL$DBF_ID$IDX)*/
       ...
       trunc(to_date(htc.ch_seiz_dt, 'yyyymmddhh24miss'), 'mm'),
       count(*)
        from hot_billing_temp_call htc, hot_billing_files hbf
       where mod(htc.dbf_id, modn) = resm
         AND HBF.HBF_ID = HTC.DBF_ID
         AND HBF.LOAD_EDATE IS NOT NULL
         and htc.ch_seiz_dt is not null
         and ((htc.dbf_id <> 4339 and sitef = 0) or
              (htc.dbf_id = 4339 and sitef = 1))
       group by htc.dbf_id,
                trunc(to_date(htc.ch_seiz_dt, 'yyyymmddhh24miss'), 'mm')
       order by 1;
    cursor curnm is
...
open curnm for 'select /*+ index(c CALL_' || to_char(smonth,
                                                          'mm_yyyy') || '$SUBSCR#STIME$IDX) */
c.servicetype,decode(c.call_cost,0,0,1),count(*),sum(c.call_cost),
    sum(c.COST_CHNG),
sum(decode(c.servicetype,''C'',ceil(c.dur/60),''G'',c.dur/1024,''W'',c.dur/1024/1024,c.dur)),
sum(decode(c.servicetype,''C'',c.dur/60,''G'',c.dur/1024,''W'',c.dur/1024/1024,c.dur))
 from call_' || to_char(smonth, 'mm_yyyy') || ' c,
  ((SELECT nvl(DB_LOADER_PCKG.GET_PHONE_BALANCE_DATE(''' || ssubscr || '''),trunc(sysdate,''mm'')-1/86400) date_start FROM DUAL)) b
where c.subscr_no=''' || ssubscr || '''
