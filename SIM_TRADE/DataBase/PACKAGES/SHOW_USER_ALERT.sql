CREATE OR REPLACE PACKAGE SHOW_USER_ALERT IS
--Version=1
--  
...
CREATE OR REPLACE PACKAGE BODY SHOW_USER_ALERT IS
--
FUNCTION CHECK_EXISTS_ALERT(
  pCONTRACT_ID IN INTEGER
  ) RETURN INTEGER IS
  ...
  IF ITOG = 0 THEN  -- Проверим рассрочки и скидки на ТП
    ...
    IF C%FOUND THEN
      IF (NVL(DUMMY.INSTALLMENT_PAYMENT_MONTHS, 0)) > 0 
          OR (NVL(DUMMY.ABON_TP_DISCOUNT, 0) <> 0) THEN 
        ITOG:=1;
      END IF;
    ELSE
      ITOG:=0;
...
--GRANT EXECUTE ON SHOW_USER_ALERT TO LONTANA_ROLE;

--GRANT EXECUTE ON SHOW_USER_ALERT TO CORP_MOBILE_ROLE;

--GRANT EXECUTE ON SHOW_USER_ALERT TO CORP_MOBILE_ROLE_RO;