CREATE OR REPLACE PACKAGE BALANCE
AS
--
--Version=3
--
--
--v3 30.01.2015 Арфосин Переделал  GETTARIFFOPTIONADDBill
--v2 28.01.2015 Арфосин Переделал  GETTARIFFOPTIONADDBill
--
   ...
   FUNCTION GET_CUR_TARIF_by_PHONE (pPHONE_NUM VARCHAR2 DEFAULT NULL)
      RETURN tblrowGetCurtarif
      PIPELINED;

   FUNCTION get_calc_koeff_detal (pPHONE_NUM IN VARCHAR2 DEFAULT NULL)
      RETURN NUMBER;

   FUNCTION CALCBALANCEROWS2 (pPHONE_NUMBER        VARCHAR2,
                              pBALANCE_DATE        DATE DEFAULT NULL,
                              pFILL_DESCRIPTION    BOOLEAN DEFAULT TRUE)
      RETURN tblGETABONENTBALANCE
      PIPELINED;

   FUNCTION get_tarifid (pPHONE_NUM IN VARCHAR2 DEFAULT NULL)
      RETURN INTEGER;
   ...
   FUNCTION get_abonent_balance (pPHONE_NUM      IN VARCHAR2 DEFAULT NULL,
                                 pdate_balance   IN DATE DEFAULT NULL)
      RETURN NUMBER;
   ...
   FUNCTION CALC_BALANCE_STAT_BY_YM (pPHONE_NUMBER    VARCHAR2,
                                     pBALANCE_DATE    DATE DEFAULT NULL,
                                     pYEAR_MONTH      INTEGER)
   ...
   FUNCTION GETTARIFFOPTIONADDBill (pACCOUNT_ID     IN INTEGER,
                                    pYEAR_MONTH     IN INTEGER,
                                    pPHONE_NUMBER   IN VARCHAR2,
                                    pBAN            IN NUMBER DEFAULT 0)
   ...
   PROCEDURE virtual_balphone (pyear_month    NUMBER,
                               sessionid      NUMBER,
                               pacc_id        VARCHAR2,
                               pis_credit     NUMBER);

   PROCEDURE VIRTUAL_BALPHONE2 (pYEAR_MONTH    NUMBER,
                                SESSIONID      NUMBER,
                                pACC_ID        VARCHAR2,
                                pIS_CREDIT     NUMBER);
   ...
   PROCEDURE virtual_rep_bal2 (nnum NUMBER, sessionid NUMBER);
END;
/
...
pipelined is
  --Version=4 Дискретные услуги при смене тарифного плана в течении месяца НЕ учитываются теперь дважды как в версии 3      
    CURSOR C (pDATE_BEGIN IN DATE, pDATE_END IN DATE/*, pTARIFF_ID IN INTEGER, TARIFF_CODE IN INTEGER*/) IS
...
/*        AND V.ACCOUNT_ID = PO.ACCOUNT_ID
        ...
        AND V.DATE_BEGIN = pDATE_BEGIN
        AND V.DATE_END = pDATE_END
        AND V.TARIFF_CODE IS NOT NULL
*/        
        AND pDATE_END > PO.TURN_ON_DATE
        AND PO.OPTION_CODE = OP.OPTION_CODE
        AND OP.TARIFF_OPTION_ID = TC.TARIFF_OPTION_ID
        AND TC.BEGIN_DATE <= pDATE_BEGIN
        AND TC.END_DATE >= pDATE_END
...
--  cursorCrow:=-1;
  ...
  for v_ab_per in (select DATE_BEGIN,
                          DATE_END,
                          TARIFF_ID,
                          TARIFF_CODE
                           
                          from V_BILL_ABON_PER_FOR_CLIENT 
                          where 
                              ACCOUNT_ID = pACCOUNT_ID and 
                              YEAR_MONTH = pYEAR_MONTH and 
                              PHONE_NUMBER = pPHONE_NUMBER and
                              TARIFF_CODE is not null  
                  ) loop
 ITOG1 := 0;
--    cursorCrow := -1;
    FOR rec IN C(v_ab_per.DATE_BEGIN, v_ab_per.DATE_END/*, v_ab_per.TARIFF_ID, v_ab_per.TARIFF_CODE*/ ) LOOP
--     ITOG1 := 0;
      --Открываем новые стоимости
--      OPEN NC(rec.TARIFF_ID, rec.TARIFF_OPTION_COST_ID);
      OPEN NC(v_ab_per.TARIFF_ID, rec.TARIFF_OPTION_COST_ID);
...
--      dbms_output.put_line('v_ab_per.DATE_BEGIN = '||v_ab_per.DATE_BEGIN||' v_ab_per.DATE_END = '||v_ab_per.DATE_END);
      
      if GC%found then
        vTURN_ON_COST:=NVL(vGC.bill_turn_on_cost, rec.TURN_ON_COST);
        vMONTHLY_COST:=NVL(vGC.bill_monthly_cost, rec.MONTHLY_COST);
--         dbms_output.put_line('GC%found = '||vMONTHLY_COST||' rec.OPERATOR_MONTHLY_COST = '||rec.OPERATOR_MONTHLY_COST);       
      elsIF NC%FOUND THEN
...
--        if  rec.OPERATOR_MONTHLY_COST <> 0 then
--        dbms_output.put_line('NC%found = '||vMONTHLY_COST||' rec.OPERATOR_MONTHLY_COST = '||rec.OPERATOR_MONTHLY_COST);
--        end if;   
      ELSE
        vTURN_ON_COST:=rec.TURN_ON_COST;
        vMONTHLY_COST:=rec.MONTHLY_COST;
  --      dbms_output.put_line('not found = '||vMONTHLY_COST||' rec.OPERATOR_MONTHLY_COST = '||rec.OPERATOR_MONTHLY_COST); 
      END IF;
      CLOSE NC;
      CLOSE GC;
      vBEGIN := rec.TURN_ON_DATE;
      IF vBEGIN < v_ab_per.DATE_BEGIN THEN
        vBEGIN := v_ab_per.DATE_BEGIN;
      END IF;
      
      vEND := NVL(rec.TURN_OFF_DATE, TO_DATE('20501231', 'YYYYMMDD'));
      IF vEND > v_ab_per.DATE_END THEN
        vEND := v_ab_per.DATE_END;
      END IF;
--      dbms_out.put
  --      if vMONTHLY_COST <> 0 or nvl(rec.OPERATOR_MONTHLY_COST, 0) <> 0 then
  --      dbms_output.put_line('vMONTHLY_COST = '||vMONTHLY_COST||' rec.OPERATOR_MONTHLY_COST = '||rec.OPERATOR_MONTHLY_COST);
  --     end if;
...
/*     
          for q in (select pPHONE_NUMBER, pYEAR_MONTH, ITOG1, vABON_TP_FULL_NEW, vBEGIN,vEND,v_ab_per.tariff_code  from dual) loop
            pipe row (q);
          end loop;
*/        
--          ITOG1:=0;    
...
--        cursorCrow:=cursorCrow+1; 
    ...
    --if cursorCrow=-1 then    
      for q in (select pPHONE_NUMBER, pYEAR_MONTH, nvl(ITOG1, 0), nvl(vABON_TP_FULL_NEW, 0), v_ab_per.DATE_BEGIN, v_ab_per.DATE_END,v_ab_per.tariff_code  from dual) loop
        pipe row (q);
      end loop;
    --end if;
...
PROCEDURE CALC_VIRT_BALANCE2(
  ...
  pBILL_CALLS IN OUT NUMBER,
  pDETAIL_CALLS IN OUT NUMBER,
  pPAYMENT_TILL_BILL IN OUT NUMBER
  ) IS
--#Version=1
  DATE_ROWS DBMS_SQL.DATE_TABLE; 
  COST_ROWS DBMS_SQL.NUMBER_TABLE; 
  DESCRIPTION_ROWS DBMS_SQL.VARCHAR2_TABLE;
  CURSOR cPHONE_ACTIV IS
    SELECT NVL(FB.PHONE_IS_ACTIVE, 0) AS IS_ACTIVE -- INTO pIS_ACTIVE
      FROM DB_LOADER_ACCOUNT_PHONES FB
      WHERE FB.PHONE_NUMBER = pPHONE_NUMBER
        AND FB.YEAR_MONTH = TO_CHAR(SYSDATE, 'YYYYMM')
        AND ROWNUM <=1;
  vPHONE_ACTIV_DUMMY cPHONE_ACTIV%ROWTYPE;
  CURSOR cPHONE_CONTRACT IS
    SELECT '+' AS IS_CONTRACT -- INTO pIS_CONTRACT
      ...
      WHERE C.PHONE_NUMBER_FEDERAL = pPHONE_NUMBER
        ...
        AND ROWNUM <=1;
  vPHONE_CONTRACT_DUMMY cPHONE_CONTRACT%ROWTYPE;
  vRESULT NUMBER(15, 2);
  L BINARY_INTEGER;
  vBALANCE_DATE DATE;
  vDAYS integer;
...
BEGIN
  ...
  IF L IS NOT NULL THEN
    FOR I IN COST_ROWS.FIRST..L LOOP
      ...
      IF (TO_CHAR(TRUNC(DATE_ROWS(I)),'YYYYMM')=TO_CHAR(pYEAR_MONTH)) AND (COST_ROWS(I) < 0) THEN
        ...
        END IF;
        IF INSTR(DESCRIPTION_ROWS(I), 'Детализация звонков') > 0 THEN
          pDETAIL_CALLS := pDETAIL_CALLS + COST_ROWS(I);
        END IF;        
      END IF;   
      -- Посчитаем корректировки до счета
      IF (TO_CHAR(TRUNC(DATE_ROWS(I)),'YYYYMM')=TO_CHAR(pYEAR_MONTH)) THEN     
        IF (INSTR(UPPER(DESCRIPTION_ROWS(I)), 'ЗАРЕГИСТРИРОВАН В ОФИСЕ') > 0)
            AND (INSTR(UPPER(DESCRIPTION_ROWS(I)), '(ДО ПОЯВЛЕНИЯ СЧЕТ.)') > 0) THEN
          pPAYMENT_TILL_BILL:=pPAYMENT_TILL_BILL + COST_ROWS(I);
        END IF;     
      END IF;   
    END LOOP;
  END IF;
  SELECT -SUM(V.BILL_SUM_NEW), -SUM(V.ABON_TP_NEW), -SUM(V.ABON_TP_OLD), -SUM(V.ABON_ADD_NEW) INTO pBILLS, pBILL_AB, pBILL_AB_BE, pBILL_USL
    ...
    WHERE V.PHONE_NUMBER = pPHONE_NUMBER
      AND V.YEAR_MONTH = pYEAR_MONTH;
  SELECT -SUM(FB.SINGLE_CHANGE_TARIFF), -SUM(FB.SINGLE_OTHER), -SUM(FB.CALLS) INTO pBILL_SINGLE_CHANGE_TP, pBILL_SINGLE_OTHER, pBILL_CALLS 
    FROM DB_LOADER_full_FINANCE_BILL FB
    WHERE FB.PHONE_NUMBER = pPHONE_NUMBER
      AND FB.YEAR_MONTH = pYEAR_MONTH;
  /*SELECT NVL(FB.PHONE_IS_ACTIVE, 0) INTO pIS_ACTIVE
    FROM DB_LOADER_ACCOUNT_PHONES FB
    WHERE FB.PHONE_NUMBER = pPHONE_NUMBER
      AND FB.YEAR_MONTH = TO_CHAR(SYSDATE, 'YYYYMM')
      AND ROWNUM <=1;*/ -- Выдавало ошибки нуллы
  OPEN cPHONE_ACTIV;
  FETCH cPHONE_ACTIV INTO vPHONE_ACTIV_DUMMY;
  IF cPHONE_ACTIV%FOUND THEN
    pIS_ACTIVE:=vPHONE_ACTIV_DUMMY.IS_ACTIVE;
  ELSE
    pIS_ACTIVE:=0;
  END IF;
  CLOSE cPHONE_ACTIV;
  /*SELECT NVL('+', '-') INTO pIS_CONTRACT
    FROM CONTRACTS C, CONTRACT_CANCELS CC
    WHERE C.PHONE_NUMBER_FEDERAL = pPHONE_NUMBER
      AND C.CONTRACT_ID = CC.CONTRACT_ID(+)
      AND C.CONTRACT_DATE <= ADD_MONTHS(TO_DATE(pYEAR_MONTH, 'YYYYMM'), 1) - 1
      AND (CC.CONTRACT_CANCEL_DATE IS NULL
          OR CC.CONTRACT_CANCEL_DATE >=TO_DATE(pYEAR_MONTH, 'YYYYMM'))
      AND ROWNUM <=1;*/ -- Выдавало ошибки нуллы
  OPEN cPHONE_CONTRACT;
  FETCH cPHONE_CONTRACT INTO vPHONE_CONTRACT_DUMMY;
  IF cPHONE_CONTRACT%FOUND THEN
    pIS_CONTRACT:=vPHONE_CONTRACT_DUMMY.IS_CONTRACT;
  ELSE
    pIS_CONTRACT:='-';
  END IF;
  CLOSE cPHONE_CONTRACT;
  FOR rec IN (SELECT DISTINCT TR.TARIFF_NAME
                FROM DB_LOADER_ACCOUNT_PHONE_HISTS H, TARIFFS TR
                WHERE H.PHONE_NUMBER = pPHONE_NUMBER
                  AND TRUNC(H.BEGIN_DATE) <= LAST_DAY(TO_DATE(pYEAR_MONTH, 'YYYYMM')) 
                  AND TRUNC(H.END_DATE) >=TO_DATE(pYEAR_MONTH, 'YYYYMM')
                  AND TR.TARIFF_ID = GET_PHONE_TARIFF_ID(pPHONE_NUMBER, H.CELL_PLAN_CODE, H.BEGIN_DATE + (H.END_DATE - H.BEGIN_DATE)/2 ) )
  LOOP 
    pTARIFF_NAMES:= pTARIFF_NAMES || ' ; ' || rec.TARIFF_NAME;
  END LOOP;
  pBALANCE:=ROUND(pBALANCE, 4);
  pCHARGES:=ROUND(pCHARGES, 4);
  pABON:=ROUND(pABON, 4);
...
procedure virtual_rep_bal2(
  ...
  pBILL_CALLS NUMBER;
  pDETAIL_CALLS NUMBER;
  pPAYMENT_TILL_BILL NUMBER;
begin
  for rec in (select *  from virtual_balance_phone vbp where num=nnum and session_id=sessionid) 
  loop
    begin
      ...
      CALC_VIRT_BALANCE2(REC.PHONE_NUMBER, SYSDATE, rec.year_month, pBALANCE, 
                         ...
                         pBILL_CALLS, pDETAIL_CALLS, pPAYMENT_TILL_BILL);     
      insert into  virtual_balance (phone_number,kr_av,pcharges,pabon,pusl,pbills,pbill_ab, pbill_usl,pbalance, 
                                    ...
                                    BILL_CALLS, DETAIL_CALLS, PAYMENT_TILL_BILL
                                    )
