...
--#Version=05
--
--v5 26.02.2015 Бакунов. Доработки:  добавлена процедура SET_OBJ_ID_BY_PHONE
--v4 19.02.2015 Бакунов. Доработки:  LOAD_OBJ_ID и CREATE_CONN (добавлен необязательный параметр pPHONE_NUMBER).
--v3  16.02.2015 -Афросин А.Ю. - добавил параметр по умолчанию в LOCK_PHONE
...
account_sum number:=0;--сумма по л\с для проверки перехода на сл. месяц
 ...
 --Парсинг строки и Вставка в таблицу через db_loader_pckg.SET_REPORT_DATA
          ...
          case nIndexField
            ...
            when 3 then begin
                          rcReportData.Detail_Sum:=to_number(replace(rtrim(sfld,chr(10)||chr(13)||chr(9)),',','.'), '9999999999.99')*nds;--сумма*НДС
                        exception
                          when others then
                            rcReportData.Detail_Sum:=to_number(replace(rtrim(sfld,chr(10)||chr(13)||chr(9)),'.',','), '9999999999.99')*nds;--сумма*НДС
                        end;
    ...
    IF ( sql%rowcount = 0 )
     THEN
      insert into iot_acc_report_data_temp
          ...
          values
            (pACCOUNT_ID, const_year_month, account_sum,sysdate);
     END IF;
     commit;
     end;
...
begin
 ...
 if strInLike(91,nvl(method,'0;'),';','()')=1 
   then 
     EXECUTE IMMEDIATE  'begin :err :=beeline_api_pckg.LOCK_PHONE(:pPHONE_NUMBER,0,:pLOCK_TYPE); end;' USING OUT err, IN pPHONE_NUMBER, pLOCK_TYPE;
