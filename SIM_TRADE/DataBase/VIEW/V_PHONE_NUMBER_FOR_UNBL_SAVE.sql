CREATE OR REPLACE VIEW V_PHONE_NUMBER_FOR_UNBL_SAVE AS
--Version=5
  SELECT v.PHONE_NUMBER_FEDERAL PHONE_NUMBER,
    ...
    WHERE phone_is_active_code=0 
      ...
      AND EXISTS (SELECT 1
                    ...
                    WHERE DB_LOADER_PAYMENTS.PHONE_NUMBER = V.PHONE_NUMBER_FEDERAL
                      and DB_LOADER_PAYMENTS.YEAR_MONTH >= TO_NUMBER(TO_CHAR(SYSDATE-3, 'yyyymm'))
                      AND DB_LOADER_PAYMENTS.PAYMENT_DATE >= TRUNC(SYSDATE - 3)
                      AND ROWNUM = 1)
...
GRANT SELECT ON V_PHONE_NUMBER_FOR_UNBL_SAVE TO CORP_MOBILE_ROLE;   

GRANT SELECT ON V_PHONE_NUMBER_FOR_UNBL_SAVE TO CORP_MOBILE_ROLE_RO;  