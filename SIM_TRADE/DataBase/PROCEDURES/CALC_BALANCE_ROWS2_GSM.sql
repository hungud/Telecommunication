/* Formatted on 04.02.2015 11:53:24 (QP5 v5.252.13127.32867) */
...
IS
        ...
        SELECT CONTRACTS.CONTRACT_DATE,
         ...
         WHERE     CONTRACTS.PHONE_NUMBER_FEDERAL = pPHONE_NUMBER
               AND (   pBALANCE_DATE IS NULL
                    OR CONTRACTS.CONTRACT_DATE <= pBALANCE_DATE)
   ...
   CURSOR DEP (pCONTRACT_ID IN INTEGER)
   IS
      SELECT CURRENT_DEPOSITE_VALUE, DATE_DEPOSITE_CHANGE
        FROM CONTRACT_DEPOSITES
       WHERE CONTRACT_ID = pCONTRACT_ID;

   CURSOR OPER (pCONTRACT_ID IN INTEGER, pDATE DATE)
   IS
      SELECT NOTE
        FROM CONTRACT_DEPOSITE_OPER
       WHERE CONTRACT_ID = pCONTRACT_ID AND OPERATION_DATE_TIME = pDATE;
   ...
   IS
      SELECT DATE_LAST_UPDATE,
             ...
             TRUNC (RECALC_CHARGE_COST (pPHONE_NUMBER, -DETAIL_SUM), 2)
                AS BILL_SUM,
   ...
   IS
      SELECT FIRST_VALUE (DB_LOADER_ACCOUNT_PHONE_HISTS.BEGIN_DATE)
                OVER (ORDER BY DB_LOADER_ACCOUNT_PHONE_HISTS.END_DATE ASC)
        FROM DB_LOADER_ACCOUNT_PHONE_HISTS
       WHERE     DB_LOADER_ACCOUNT_PHONE_HISTS.PHONE_NUMBER = pPHONE_NUMBER
             AND DB_LOADER_ACCOUNT_PHONE_HISTS.PHONE_IS_ACTIVE = 1
             AND DB_LOADER_ACCOUNT_PHONE_HISTS.END_DATE >=
                    pSTART_BALANCE_DATE - pCORRECT_ACTIVATION_DATE_DAYS
             AND DB_LOADER_ACCOUNT_PHONE_HISTS.BEGIN_DATE <=
                    pSTART_BALANCE_DATE + pCORRECT_ACTIVATION_DATE_DAYS;
...
BEGIN
   ...
   IF MS_CONSTANTS.GET_CONSTANT_VALUE ('USE_INSTALLMENT_PAYMENT') = '1'
   THEN
      IF vINST_PAYMENT_MONTH IS NOT NULL
      THEN
         FOR I IN 0 .. vINST_PAYMENT_MONTH - 1
         LOOP
            IF ADD_MONTHS (vINST_PAYMENT_DATE, I) < SYSDATE
            THEN
               IF ADD_MONTHS (vINST_PAYMENT_DATE - 1, I + 1) < SYSDATE
               THEN
                  vINST_TEMP_DATE :=
                     ADD_MONTHS (vINST_PAYMENT_DATE - 1, I + 1);
                  vINST_TEMP_SUM :=
                     TRUNC (vINST_PAYMENT_SUM / vINST_PAYMENT_MONTH, 2);
               ELSE
                     ...
                     TRUNC (
                        ...
                        * (  1
                           -   (  ADD_MONTHS (vINST_PAYMENT_DATE - 1, I + 1)
                                - TRUNC (SYSDATE))
                             / (  ADD_MONTHS (vINST_PAYMENT_DATE - 1, I + 1)
                                - ADD_MONTHS (vINST_PAYMENT_DATE - 1, I))),
                        2);
               END IF;
               ...
               THEN
                  vINST_TEMP_DESCR :=
                     ...
                     || TO_CHAR (I + 1)
                     || 'й месяц. С '
                     || TO_CHAR (ADD_MONTHS (vINST_PAYMENT_DATE - 1, I) + 1,
                                 'DD.MM.YYYY')
                     || ' по '
                     || TO_CHAR (ADD_MONTHS (vINST_PAYMENT_DATE - 1, I + 1),
                                 'DD.MM.YYYY')
   ...
   IF     (vOPTION_GROUP_ID IS NOT NULL)
      AND (MS_CONSTANTS.GET_CONSTANT_VALUE ('USE_TARIFF_OPTION_GROUP') = '1')
   THEN
      ...
      FOR recTOGC
         IN (SELECT TOGC.*
               FROM TARIFF_OPTION_GROUP TOG, TARIFF_OPTION_GROUP_COSTS TOGC
              WHERE     TOG.OPTION_GROUP_ID = vOPTION_GROUP_ID
                    AND TOG.IS_ACTIVE = 1
                    AND TOG.OPTION_GROUP_ID = TOGC.OPTION_GROUP_ID)
      LOOP
         vOPTION_GROUP_LIST :=
            vOPTION_GROUP_LIST || ';' || recTOGC.OPTION_CODE;
         vOPTION_GROUP_COSTS (vOPTION_GROUP_COSTS.COUNT + 1) :=
               recTOGC.OPTION_CODE
            || ';'
            || recTOGC.TURN_ON_COST
            || ';'
            || recTOGC.MONTHLY_COST
            || ';'
            || recTOGC.BILL_TURN_ON_COST
            || ';'
            || recTOGC.BILL_MONTHLY_COST;
      END LOOP;
   END IF;
   ...
   THEN
      ...
      APPEND_ROW (recDEP.DATE_DEPOSITE_CHANGE,
                  -recDEP.CURRENT_DEPOSITE_VALUE,
                  'Депозит клиента. ' || recOPER.NOTE);
   END IF;
      ...
      THEN
         ...
         THEN         -- Если 1, то договор кредитный, тогда абонку не считаем
            ...
            ELSE
               ...
               IF recPAYMENTS.END_DATE > TRUNC (SYSDATE)
               THEN
            ...
            LOOP
               ...
               THEN
                  ...
                  IF     (recPHONE_STATUS_HISTORY.PHONE_IS_ACTIVE = 1)
                     ...
                     AND (recPHONE_STATUS_HISTORY.TARIFF_ID <>
         ...
         FOR recSERVICES
            IN (SELECT DB_LOADER_ACCOUNT_PHONE_OPTS.OPTION_CODE,
                 ...
                 WHERE     DB_LOADER_ACCOUNT_PHONE_OPTS.ACCOUNT_ID =
                              ACCOUNTS.ACCOUNT_ID              --ACCOUNT_ID=45
         ...
         LOOP
            ...
            IF (vOPTION_GROUP_ID IS NOT NULL)
            THEN
               IF INSTR (vOPTION_GROUP_LIST, recSERVICES.OPTION_CODE) > 0
               THEN
                  L := vOPTION_GROUP_COSTS.LAST;

                  FOR I IN vOPTION_GROUP_COSTS.FIRST .. L
                  LOOP
                     IF (INSTR (vOPTION_GROUP_COSTS (I),
                                recSERVICES.OPTION_CODE) > 0)
                     THEN
                        IF (INSTR (vOPTION_GROUP_COSTS (I),
                                   ';',
                                   1,
                        ...
                        THEN
                           recSERVICES.TURN_ON_COST :=
                              TO_NUMBER (
                                 SUBSTR (vOPTION_GROUP_COSTS (I),
                                           INSTR (vOPTION_GROUP_COSTS (I),
                                                  ';',
                                                  1,
                                                  1)
                                         + 1,
                  ...
                  THEN
                     APPEND_ROW (
                        vSERVICE_START_DATE,
                          -recSERVICES.MONTHLY_PRICE
                        * CASE
                             WHEN     (NOT vCALC_ABON_PAYMENT_TO_MONTHEND)
                                  AND (NVL (recSERVICES.DISCR_SPISANIE, 0) =
                                          1)
                             THEN
                                1
                             ELSE
