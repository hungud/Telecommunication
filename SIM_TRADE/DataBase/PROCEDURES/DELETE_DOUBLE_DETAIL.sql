...
BEGIN
  if SUBSCR_NO is null then
      execute immediate 'delete from CALL_' || to_char(sysdate, 'mm_yyyy') || ' td
    where     
      td.CALL_COST <> 0
      and td.DBF_ID = ' || MS_CONSTANTS.GET_CONSTANT_VALUE('API_DBF_ID') || ' 
      and exists ( 
                    select 1 from CALL_' || to_char(sysdate, 'mm_yyyy') || ' td1
      ...
      execute immediate 'delete from CALL_' || to_char(add_months(sysdate, -1), 'mm_yyyy') || ' td
    where     
      td.CALL_COST <> 0
      and td.DBF_ID = ' || MS_CONSTANTS.GET_CONSTANT_VALUE('API_DBF_ID') || ' 
      and exists ( 
                    select 1 from CALL_' || to_char(add_months(sysdate, -1), 'mm_yyyy') || ' td1
                    where td.START_TIME = td1.START_TIME
                      and td.SUBSCR_NO = td1.SUBSCR_NO
                      and td.CALL_COST = td1.CALL_COST
                      and td1.DBF_ID <> ' || MS_CONSTANTS.GET_CONSTANT_VALUE('API_DBF_ID') || ' 
