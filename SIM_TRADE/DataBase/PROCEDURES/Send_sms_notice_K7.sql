...
--#if GetVersion("SEND_SMS_NOTICE") < 20 then
CREATE OR REPLACE PROCEDURE SEND_SMS_NOTICE AS
--
--#Version=20
--
--v.19 21.01.2013 Назин. Исправил курсор- сделал представление без пересчёта баланса.оптимизировал код.
...
CURSOR C IS
  ...
  FROM 
  --без пересчёта балансов баланс из IOT_CURRENT_BALANCES - - ТОЛЬКО ДЛЯ К7 
...
not exists (select 1 from send_notice sn where nt.PHONE_NUMBER_FEDERAL=sn.phone_number and sn.balance_notice <= nt.balance_notice)
;
  vREC C%ROWTYPE;
--
 ...
 --Проверка на время отправки - по Будням с 9 до 18 в выходные с 12 до 18.
   IF ((TO_NUMBER(TO_CHAR(SYSDATE,'D','NLS_DATE_LANGUAGE=RUSSIAN'))<6) AND (SYSDATE-TRUNC(SYSDATE)>9/24) AND (SYSDATE-TRUNC(SYSDATE)<18/24))
     ...
     --, чтобы их не удалило из SEND_NOTICE 
      update SEND_NOTICE sn set sn.send_date_time=sysdate --0:38 (count=14638)
       ...
       IF acc_notice_is_on(vREC.ACCOUNT_ID)=1 and 
         ...
         IF (SQL%NOTFOUND) THEN
           ...
           IF SMS IS NULL THEN --если отправка удачная
             ...
             UPDATE send_notice s SET s.send_date_time=sysdate, s.balance_notice=vREC.balance_notice
             WHERE s.phone_number = vREC.PHONE_NUMBER_FEDERAL AND s.balance_notice > vREC.balance_notice;
             IF SQL%NOTFOUND THEN
...
insert into aaa (nnn1,sss1) values (0,vREC.PHONE_NUMBER_FEDERAL);*/
     ...
     DELETE FROM send_notice WHERE send_date_time < date_begin;--0:06
     COMMIT;
