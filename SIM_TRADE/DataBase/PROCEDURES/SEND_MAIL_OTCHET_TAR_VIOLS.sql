CREATE OR REPLACE PROCEDURE SEND_MAIL_OTCHET_TAR_VIOLS IS
--Version=1
TEXT CLOB;
BEGIN
  TEXT:=GET_MAIL_TARIFF_VIOLATIONS(TO_NUMBER(TO_CHAR(SYSDATE, 'YYYYMM')));
  FOR rec IN(
    SELECT REPORT_MAIL_RECIPIENTS.MAIL_ADRESS,
           REPORT_TYPES.REPORT_NAME
      FROM REPORT_MAIL_RECIPIENTS, REPORT_TYPES
      WHERE REPORT_MAIL_RECIPIENTS.TYPE_REPORT='CONTROL_TARIFFS'
