...
{$IFDEF FPC}
  LResources,
{$ENDIF}
  MemUtils, Ora, OraError, IniFiles, MainUnit;
...
begin
  ...
  if (UpperCase(pSchemaName) = 'LONTANA') or
     (eSchema.text = 'GSM Корпорация') then begin
...
begin
  if (UpperCase(eSchema.text) = 'LONTANA')
...
begin
  ...
  else FConnectDialog.Session.Password := mainform.md5_30(edPassword.Text);
...
procedure TfmConn.btCancelClick(Sender: TObject);
begin
  MainForm.Close;
end;
