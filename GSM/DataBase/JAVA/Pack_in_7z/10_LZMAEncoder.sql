create or replace and compile java source named LZMAEncoder as
...
public class Encoder
{
  ...
  class LiteralEncoder
  {
    class Encoder2
    {
      ...
      public void Init() { Encoder1.InitBitModels(m_Encoders); }



      public void Encode(Encoder1 rangeEncoder, byte symbol) throws IOException
      {
        int context = 1;
        for (int i = 7; i >= 0; i--)
        {
      ...
      public void EncodeMatched(Encoder1 rangeEncoder, byte matchByte, byte symbol) throws IOException
      {
        int context = 1;
        boolean same = true;
        for (int i = 7; i >= 0; i--)
      ...
      public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
      {
        ...
        if (matchMode)
        {
          for (; i >= 0; i--)
          {
            int matchBit = (matchByte >> i) & 1;
            int bit = (symbol >> i) & 1;
            price += Encoder1.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
            context = (context << 1) | bit;
            if (matchBit != bit)
            {
              i--;
        ...
        {
          int bit = (symbol >> i) & 1;
          price += Encoder1.GetPrice(m_Encoders[context], bit);
          context = (context << 1) | bit;
        }
        return price;
      }
  ...
  class LenEncoder
  {
    ...
    {
      Encoder1.InitBitModels(_choice);
    ...
    public void Encode(Encoder1 rangeEncoder, int symbol, int posState) throws IOException
    {
    ...
    {
      int a0 = Encoder1.GetPrice0(_choice[0]);
      int a1 = Encoder1.GetPrice1(_choice[0]);
      int b0 = a1 + Encoder1.GetPrice0(_choice[1]);
      int b1 = a1 + Encoder1.GetPrice1(_choice[1]);
      int i = 0;
  ...
  Encoder1 _rangeEncoder = new Encoder1();
  ...
  void Init()
  {
    ...
    Encoder1.InitBitModels(_isMatch);
    Encoder1.InitBitModels(_isRep0Long);
    Encoder1.InitBitModels(_isRep);
    Encoder1.InitBitModels(_isRepG0);
    Encoder1.InitBitModels(_isRepG1);
    Encoder1.InitBitModels(_isRepG2);
    Encoder1.InitBitModels(_posEncoders);
  ...
  {
    return Encoder1.GetPrice0(_isRepG0[state]) +
        Encoder1.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
  }
  ...
  int GetPureRepPrice(int repIndex, int state, int posState)
  {
    ...
    if (repIndex == 0)
    {
      price = Encoder1.GetPrice0(_isRepG0[state]);
      price += Encoder1.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
    }
    else
    {
      price = Encoder1.GetPrice1(_isRepG0[state]);
      if (repIndex == 1)
        price += Encoder1.GetPrice0(_isRepG1[state]);
      else
      {
        price += Encoder1.GetPrice1(_isRepG1[state]);
        price += Encoder1.GetPrice(_isRepG2[state], repIndex - 2);
      }
    }
    return price;
  }
  ...
  int GetOptimum(int position) throws IOException
  {
    ...
    _optimum[1].Price = Encoder1.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
        _literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
    _optimum[1].MakeAsChar();

    int matchPrice = Encoder1.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
    int repMatchPrice = matchPrice + Encoder1.GetPrice1(_isRep[_state]);
    ...
    int normalMatchPrice = matchPrice + Encoder1.GetPrice0(_isRep[_state]);
    ...
    while (true)
    {
      ...
      int curAnd1Price = curPrice +
        Encoder1.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
      ...
      matchPrice = curPrice + Encoder1.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
      repMatchPrice = matchPrice + Encoder1.GetPrice1(_isRep[state]);
      ...
      if (!nextIsChar && matchByte != currentByte)
      {
        ...
        if (lenTest2 >= 2)
        {
          ...
          int nextRepMatchPrice = curAnd1Price +
            Encoder1.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
            Encoder1.GetPrice1(_isRep[state2]);
          {
        ...
        if (lenTest < numAvailableBytesFull)
        {
          ...
          if (lenTest2 >= 2)
          {
            ...
            int curAndLenCharPrice =
                ...
                Encoder1.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
            ...
            int nextMatchPrice = curAndLenCharPrice + Encoder1.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
            int nextRepMatchPrice = nextMatchPrice + Encoder1.GetPrice1(_isRep[state2]);
      ...
      if (newLen >= startLen)
      {
        normalMatchPrice = matchPrice + Encoder1.GetPrice0(_isRep[state]);
  ...
  void FillDistancesPrices()
  {
    ...
    for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
    {
      ...
      for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
        _posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << Encoder1.kNumBitPriceShiftBits);
