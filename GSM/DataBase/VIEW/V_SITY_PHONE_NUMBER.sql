...
--Version=7
-- 7 safonova 16.10.2013 Изменение отображения ФИО, комментария  - данные в полях отображается независимо от наличия у абонента доп.статуса
-- 6. safonova 26.09.2013 Добавление доп.статуса, ФИО, комментария
-- 5. Крайнов. Добавление столбца с видом гор номера.
--2. Котенков Добавил выдачу номеров без договоров и столбец contract_flag.
--3. Овсянников Изменение статуса номера с "восстановленный" на "Номер расторгнут" 
--4. Котенков Изменение статусов на: "С контрактом", "Без контракта", "Номер расторгнут", "Без истории".
...
SELECT  T.ACCOUNT_ID,  
        ...
        T.BEGIN_DATE,     
        CASE 
          WHEN (T.YEAR_MONTH = T.MAX_YM) AND (T.CNT > 0) THEN 'С контрактом'
          WHEN (T.YEAR_MONTH = T.MAX_YM) AND (T.CNT = 0) THEN 'Без контракта'
          WHEN T.YEAR_MONTH < T.MAX_YM THEN 'Номер расторгнут'
        END CONTRACT_FLAG,
        T.OPTION_NAME,
        T.DOP_STATUS,
        T.FIO,
...
SELECT -1 ACCOUNT_ID,
       ...
       NULL SYSTEM_BLOCK,
       NULL BEGIN_DATE,
       'Без истории' CONTRACT_FLAG,
       'Неизвестно' OPTION_NAME,
       NULL dop_status,
       NULL fio,
       NULL note
FROM CONTRACTS C
