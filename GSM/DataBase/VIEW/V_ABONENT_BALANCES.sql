CREATE OR REPLACE VIEW V_ABONENT_BALANCES AS
--#Version=18
SELECT t."PHONE_NUMBER_FEDERAL", t."OPERATOR_NAME", t."SURNAME",
       ...
       T."IS_CREDIT_CONTRACT", t."BALANCE_NOTICE_HAND_BLOCK", t."BALANCE_BLOCK_HAND_BLOCK",
       T."DEALER_KOD",
       CASE
  ...
  FROM (SELECT c.phone_number_federal, 
               ...
               a.bdate, a.contact_info,
               CASE
                 WHEN CB.LAST_UPDATE > SYSDATE - 15/(24*60) THEN CB.BALANCE
                 ELSE get_abonent_balance(c.phone_number_federal) 
               END BALANCE,
               ...
               ) AS status_date,  --изменен селект с DB_LOADER_ACCOUNT_PHONE_HISTS.BEGIN_DATE на db_loader_account_phones.last_change_status_date 
          ...
          WHERE c.abonent_id = a.abonent_id
            ...
            AND C.PHONE_NUMBER_FEDERAL = CB.PHONE_NUMBER(+)
