...
AS
   ...
   SELECT                                                         --#Version=6
            --v6. -- И. Матюнин - не показывались смс из sms_log, так как в поле SMS_LOG.SEND_DATE практически всегда было NULL.  
                                                                            --
          ...
          */
          ...
          SELECT                                                --блок/разблок
                t.phone_number,
                 CASE ticket_type
                    WHEN 15
                    THEN                       -- Подключение/отключение услуг
                       (SELECT CASE
          ...
          SELECT phone_number,
                 ...
                 -PROMISED_SUM,
                 TO_NUMBER (NULL) BALANCE,
                    'Закончился обещанный платеж от '
                 || TO_CHAR (payment_date, 'dd.mm.yyyy'),
                 TO_CHAR (promised_sum)
            FROM promised_payments
          UNION ALL
