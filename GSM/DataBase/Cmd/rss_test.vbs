...
sub ProcessFile(InputFileName)
  ...
  if not DoExecFlag then
    if ProcessedFileList.Exists(UCase(InputFileName)) then
      exit sub
    end if
  end if
  
  if not DoExecFlag then
    ProcessedFileList.Add UCase(InputFileName), True
  end if
'  MsgBox InputFileName
  ...
  while not InputFile.AtEndOfStream
    ...
    if UCase(LineText) = "#ELSE" then
      PrintError InputFileName, LineNumber, "Команда #ELSE не поддерживается"
    end if
    if UCase(LineText) = "--#ELSE" then
      PrintError InputFileName, LineNumber, "Команда --#ELSE не поддерживается"
    end if
    ...
    if LockCount = 0 then
      DoWrite = True
      if (UCase(Left(LineText, 8)) = "#EXECUTE") or (UCase(Left(LineText, 10)) = "--#EXECUTE") then
        if (UCase(Left(LineText, 1)) = "#") then
          Expression = Trim(Mid(LineText, 10))
        else
          Expression = Trim(Mid(LineText, 12))
        end if
        if Expression = "" then
          PrintError InputFileName, LineNumber, "Использован #Execute без условия"
        end if
        on error resume next
        Execute Expression
        if Err then
          PrintError InputFileName, LineNumber, Err.Description
        end if
        on error goto 0
        DoWrite = False
      end if
      if (UCase(Left(LineText, 7)) = "#MSGBOX") or (UCase(Left(LineText, 9)) = "--#MSGBOX") then
        if (UCase(Left(LineText, 1)) = "#") then
        ...
        if Expression = "" then
          PrintError InputFileName, LineNumber, "Использован #MsgBox без условия"
        end if
        on error resume next
        ...
        AppendToLog InputFileName&", строка "&LineNumber&": "&Expression
        DoWrite = False
      end if
      if (UCase(Left(LineText, 7)) = "#END IF") or (UCase(Left(LineText, 6)) = "#ENDIF") or _
        (UCase(Left(LineText, 9)) = "--#END IF") or (UCase(Left(LineText, 8)) = "--#ENDIF") then
        DoWrite = False
        CodeLevel = CodeLevel - 1
        if CodeLevel <  0 then
          PrintError InputFileName, LineNumber, "#END IF не имеет соответствующего #IF"
        end if
      end if
...
ExecScript InputFileName
        ...
        DoWrite = False
        CodeLevel = CodeLevel + 1
        if (UCase(Left(LineText, 3)) = "#IF") then
          Expression = Trim(Mid(LineText, 5))
        else
        ...
        if Expression = "" then
          PrintError InputFileName, LineNumber, "Использован #IF без условия"
        end if
        on error resume next
      ...
      if Left(LineText, 1) = "@" then
        ...
        CurrentDirectory = fso.GetParentFolderName(fso.GetAbsolutePathName(InputFileName))
        while left(InputFileName, 3) = "..\"
        ...
        if Instr(IncludeFileName, "*") > 0 or Instr(IncludeFileName, "?") > 0 then
          ...
          if not fso.FileExists(IncludeFileName) then
            PrintError InputFileName, LineNumber, "Файл """&IncludeFileName&""" не найден"
          end if
          ProcessFile(IncludeFileName)
        end if
        DoWrite = False
        ...
        if Trim(SourceLineText) <> "" and Left(LTrim(SourceLineText), 2) <> "--" then
          if WrotedLineCount = 0 then
            ...
            if VerboseOutput then
              WScript.Echo "/* файл " & InputFileName & " */"
  ...
  if CodeLevel <> 0 then
    PrintError InputFileName, LineNumber, "Парный #END IF не найден"
  end if
...
sub Usage
  print "Использование: cscript.exe rss.vbs входной_файл [имя/пароль@алиасБД] [/separate] [/verbose]"
  WScript.Quit 1
...
Sub DetectExeType()
        ...
        If (UCase(ScriptHost) = "WSCRIPT.EXE") Then
                ...
                msg = AddLine(msg,      "       cscript rss.vbs <param>")
...
function GetVersion(ProcName)
  ...
  SQLText = "SELECT * FROM USER_OBJECTS WHERE OBJECT_NAME=? ORDER BY OBJECT_TYPE"
...
function GetColumnSize(TableColumnName)
  ...
  Set Rs = CreateRecordset("select data_length from user_tab_columns where table_name=? and column_name=?", Array(TableName, ColumnName))
  if Rs.EOF then
    Result = 0
  else
