CREATE OR REPLACE PROCEDURE CALC_BALANCE_ROWS_FULL_BILLS(
...
--            FROM CONTRACT_CANCELS)
  ...
  CURSOR cPHONE_STATUS_HISTORY(aPHONE_NUMBER VARCHAR2, pBEGIN_DATE DATE, pEND_DATE DATE, pCALC_ABON_PAYMENT_TO_MONTHEND INTEGER) IS
    SELECT
      CASE 
        WHEN TRUNC(DB_LOADER_ACCOUNT_PHONE_HISTS.BEGIN_DATE) < pBEGIN_DATE THEN
          TRUNC(pBEGIN_DATE)
        ELSE
          CASE
  ...
  CURSOR NEW_DATE_ABON(pSTART_BALANCE_DATE IN DATE,
    ...
    SELECT FIRST_VALUE(DB_LOADER_ACCOUNT_PHONE_HISTS.BEGIN_DATE) OVER (ORDER BY DB_LOADER_ACCOUNT_PHONE_HISTS.END_DATE DESC) 
        FROM DB_LOADER_ACCOUNT_PHONE_HISTS
        WHERE DB_LOADER_ACCOUNT_PHONE_HISTS.PHONE_NUMBER=pPHONE_NUMBER
...
BEGIN
 ...
 -- vSERVER_NAME:=MS_CONSTANTS.GET_CONSTANT_VALUE('SERVER_NAME');
  ...
  FETCH C_BALANCE INTO vSTART_BALANCE_DATE, vSTART_BALANCE_VALUE;
  ...
  FOR rec IN (
    ...
    WHERE
      ...
      /*and ((V_FULL_BALANCE_BILLS.BILL_CHECKED=1 and ms_constants.GET_CONSTANT_VALUE('USE_BILL_CHECK')='1')
            or (ms_constants.GET_CONSTANT_VALUE('USE_BILL_CHECK')='0'))*/ --все счета
      )
  LOOP
    ...
    END IF;
  END LOOP;
  IF (FLAG_CURR_MONTH=0) AND (TO_NUMBER(TO_CHAR(SYSDATE,'DD'))-DAYS_NESTABIL>0) THEN --ЗА ТЕК МЕСЯЦ НЕ БЫЛО НИЧЕГО, ПРОБУЕМ НАЙТИ ОТЧЕТ
    rec_REPORT.DATE_LAST_UPDATE:=NULL;
    ...
    IF NVL(vCREDIT, 0)<>1 THEN -- Если 1, то договор кредитный, тогда абонку не считаем
      ...
      LOOP 
        ...
        IF (recPHONE_STATUS_HISTORY.END_DATE>=recPHONE_STATUS_HISTORY.BEGIN_DATE)
          ...
          IF NOT(vACCOUNT_ID=54 AND TO_NUMBER(TO_CHAR(recPHONE_STATUS_HISTORY.END_DATE, 'YYYYMM'))<=201202) THEN   
            APPEND_ROW(
...
-- Проверка наличия данных этого месяца
    ...
    LOOP
      ...
      IF (recSERVICES.MONTHLY_PRICE <> 0)AND(NVL(vCREDIT, 0)<>1) THEN
        --
        ...
        IF (vSERVICE_START_DATE <= vSERVICE_END_DATE) 
          AND (pBALANCE_DATE IS NULL OR vSERVICE_START_DATE <= pBALANCE_DATE)
          THEN
          APPEND_ROW(
            vSERVICE_START_DATE, 
            --Процент владельца
