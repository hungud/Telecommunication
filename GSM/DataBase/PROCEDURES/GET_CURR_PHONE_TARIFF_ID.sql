...
--Version=2
--
--v.2 Афросин 20.05.2015 переделал получение текущего ID тарифа
--Овсянников 23.01.2012 Определение текущего ID тарифа
BEGIN

 BEGIN
  NDF := 0;
  Select CONTRACT_CANCEL_DATE/*, TARIFF_ID*/ INTO pCONTRACT_CANCEL_DATE /*,pTARIFF_ID*/ FROM v_contracts
        WHERE  CONTRACT_CANCEL_DATE is null
        and PHONE_NUMBER_FEDERAL=pPHONE_NUMBER;  
 EXCEPTION 
     WHEN TOO_MANY_ROWS THEN NDF := 2;
...
-- если договор расторгнут, то 
 ...
 ELSE
  IF pCONTRACT_CANCEL_DATE IS NOT NULL THEN 
    vRESULT:=-1;
  ELSE
--    select tariff_code INTO pTARIFF_CODE from tariffs where TARIFF_ID=pTARIFF_ID;
--  /* 
   BEGIN
...
END;
/
grant execute on GET_CURR_PHONE_TARIFF_ID to LONTANA_ROLE_RO;
grant execute on GET_CURR_PHONE_TARIFF_ID to LONTANA_ROLE;