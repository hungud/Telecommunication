...
BEGIN
  ...
  insert into TARIFER_BILL_FOR_CLIENT_DET(
                                          YEAR_MONTH,
                                          PHONE_NUMBER, 
                                          ROW_DATE,
                                          ROW_COST,
    ...
    where nvl(x.ROW_TYPE, 0) >= 1
        and nvl(x.ROW_TYPE, 0) <= 5
        and TO_CHAR(x.ROW_DATE, 'YYYYMM') = TO_CHAR(ADD_MONTHS(SYSDATE, -1), 'YYYYMM');
  commit;
  
  insert into ACCOUNT_LOADED_BILLS(
                        ACCOUNT_ID,
                        YEAR_MONTH
                        )
    select x.ACCOUNT_ID,
    ...
    where
        x.YEAR_MONTH = TO_CHAR(ADD_MONTHS(SYSDATE, -2), 'YYYYMM')
