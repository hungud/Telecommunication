...
--#Version=14
--
--09.09.2011  райнов. ¬ынесен во вьюшку отбор телефонов 
--05.01.2013 »справлено определение мес€ца в сообщении 
...
BEGIN
  ...
  IF (((TO_NUMBER(TO_CHAR(SYSDATE,'D','NLS_DATE_LANGUAGE=RUSSIAN'))<6) AND (SYSDATE-TRUNC(SYSDATE)>9/24) AND (SYSDATE-TRUNC(SYSDATE)<19/24))
    ...
    LOOP 
      ...
      FETCH BL INTO vBL;
      CLOSE BL;
      SMS_TXT:='—чет за %month% составл€ет %bill%р., просьба оплатить до 11-ого. ƒолг на %date% составл€ет: %dolg%.';
      vMONTH:=CASE
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=2 THEN '€нварь'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=3 THEN 'февраль'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=4 THEN 'март'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=5 THEN 'апрель'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=6 THEN 'май'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=7 THEN 'июнь'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=8 THEN 'июль'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=9 THEN 'август'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=10 THEN 'сент€брь'  
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=11 THEN 'окт€брь'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=12 THEN 'но€брь'
                WHEN TO_NUMBER(TO_CHAR(SYSDATE,'MM'))=1 THEN 'декабрь'  
              END;  
