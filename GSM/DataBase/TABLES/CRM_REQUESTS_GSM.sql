...
--#Version=1
--
--v.1 јфросин 09.10.2014 - изменил :NEW.PHONE_NUMBER is not null на nvl(:NEW.PHONE_NUMBER, '') <> ''
  ...
  IF UPDATING THEN 
    IF (:OLD.ID_STATUS_REQUEST<>:NEW.ID_STATUS_REQUEST) AND (:NEW.ID_STATUS_REQUEST=3) THEN 
      IF :NEW.TYPE_REQUEST_ID <> 8 THEN
        --отправл€ем на номер абонента
        IF nvl(:NEW.PHONE_NUMBER, 0) <> 0 THEN 
            sms:=loader3_pckg.SEND_SMS(:NEW.PHONE_NUMBER, null,'¬ыполнение за€вки є'||:NEW.REQUEST_ID||', временно преостановлено, подробности по телефону 8-495-788-79-08');
        END IF; 
        --если доп. номер <> номеру абонента, то отправл€ем на него тоже   
        IF nvl(:NEW.DOP_CONTACT, 0) <> 0 THEN --если доп. номер не пустой
            IF nvl(:NEW.DOP_CONTACT, 0) <> nvl(:NEW.PHONE_NUMBER, 0) THEN --если доп. номер <> номеру абонента, а иначе получаетс€ что шлем 2 смс на один номер, а это неправильно
                sms:=loader3_pckg.SEND_SMS(:NEW.DOP_CONTACT, null,'¬ыполнение за€вки є'||:NEW.REQUEST_ID||', временно преостановлено, подробности по телефону 8-495-788-79-08');
            END IF;
        END IF;  
      ELSE
        --отправл€ем на номер, по которому звонили, а это доп. номер
        IF nvl(:NEW.DOP_CONTACT, 0) <> 0 THEN --если доп. номер не пустой
            sms:=loader3_pckg.SEND_SMS(:NEW.DOP_CONTACT, null,'¬ыполнение за€вки є'||:NEW.REQUEST_ID||', временно преостановлено, подробности по телефону 8-495-788-79-08');
        ELSE --если доп. номер пустой, то отправл€ем на номер, который ууказан как номер абонента (получаетс€ что с него звонили)
            IF nvl(:NEW.PHONE_NUMBER, 0) <> 0 THEN 
                sms:=loader3_pckg.SEND_SMS(:NEW.PHONE_NUMBER, null,'¬ыполнение за€вки є'||:NEW.REQUEST_ID||', временно преостановлено, подробности по телефону 8-495-788-79-08');
            END IF; 
        END IF;     
...
COMMENT ON COLUMN CRM_REQUESTS.leaving_reason_id IS ' од причины ухода пользователей';