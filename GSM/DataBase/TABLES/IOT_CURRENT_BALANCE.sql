...
(
  ...
  balance      NUMBER,
  constraint PK_CURRENT_BALANCE primary key (PHONE_NUMBER, UPDATE_TYPE)
)
organization index;
-- Add comments to the table 
comment on table IOT_CURRENT_BALANCE
  is 'рейсыхе аюкюмяш днцнбнпмшу мнлепнб';
-- Add comments to the columns 
comment on column IOT_CURRENT_BALANCE.phone_number
  is 'мнлеп ';
comment on column IOT_CURRENT_BALANCE.last_update
  is 'дюрю ябефеярх';
comment on column IOT_CURRENT_BALANCE.update_type
  is 'нрйсдю опхькн намнбкемхе';
comment on column IOT_CURRENT_BALANCE.balance
  is 'аюкюмя';
