...
--#Version=24
--
--v.24 Алексеев.  01.12.2015 Изменил функцию SEND_SMS_MULTI. Если оператор билайн, то жестко шлем через билайн.
--v23. Алексеев. 28.10.2015 При определении оператора добавил проверку на вариант "Error! Соединение сорвалось!"
--v22. Алексеев. 20.10.2015 Поправил SEND_SMS_MULTI. Если оператор на номере не билайн, то отправляем через провадера смс-трафик.
--v21. Афросин. 24.03.2015 Добавил параметр USE_ONLY_SMS_PROV в SEND_SMS_MULTI, указывающий на то,
--20. Афросин. Переделал функцию поулчения GET_PHONE_DETAIL. Получаем файлы напрямую без Робота
...
BEGIN
  ...
  --если отправлять только через смс провайдера, то не выбираем  данные из таблицы SMS_SEND_PARAM_BY_ACCOUNT  
  --через провайдера отправляем если параметр pUSE_ONLY_SMS_PROV = 1 или на номере оператор <> Билайн
  --проверяем оператора. Если при определении оператора была ошибка, то шлем через шлюз
  vOperNoBe := 0;
  begin
    vOperName := GET_OPERATOR_BY_PHONE(vPHONES);
    if (vOperName <> 'Билайн') and (vOperName not like '%Error! Соединение сорвалось!%') then
      vOperNoBe := 1;
    end if;
  exception
    when others then
