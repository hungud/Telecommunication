...
--#Version=08
--
--v8 18.03.2016 Алексеев. Поправил LOAD_OBJ_ID. Дважды прописывался create_conn. Закоментил строку err:=create_conn(plogin,ppassword,51,answer_mes);
--v7 17.02.2016 Афросин поправил получение периода платежа в перехожный период (между годами) (задача http://redmine.tarifer.ru/issues/4155)
--v6 10.08.2015 Соколов. При выгрузке счетов из Билайна выделил отдельно скидку (Корректировка абон.платы) в процедуре LOAD_DETAIL_BILL
--v5 09.07.2015 Алексеев. Поправлена UPDATE_ACCOUNT_TARIFFS. Убран replace, заменяющий запятую
--v4 26.02.2015 Бакунов. Доработки:  добавлена процедура SET_OBJ_ID_BY_PHONE
--v3 19.02.2015 Бакунов. Доработки:  LOAD_OBJ_ID и CREATE_CONN (добавлен необязательный параметр pPHONE_NUMBER).
...
counter    number:=0;
 ...
 --Парсинг строки и Вставка в таблицу через db_loader_pckg.SET_REPORT_DATA
          ...
          db_loader_pckg.ADD_PAYMENT( to_char(rcLoaderPayments.Payment_Date,'YYYY'), --substr(to_char(const_year_month),1,4),
                              to_char(rcLoaderPayments.Payment_Date,'MM'),plogin,rcLoaderPayments.Phone_Number
...
BEGIN
    ...
    then 
     EXECUTE IMMEDIATE  'begin :err :=beeline_api_pckg.account_phone_payments(:pAccount_id); end;' USING OUT err, IN pACCOUNT_ID;
...
begin
  SELECT login,nvl(new_pswd,password) password,n_method
      INTO plogin, ppassword,method
  FROM ACCOUNTS WHERE ACCOUNT_ID=pACCOUNT_ID;
 
   if strInLike(91,nvl(method,'0;'),';','()')=1 
     then 
       EXECUTE IMMEDIATE  'begin :err :=beeline_api_pckg.LOCK_PHONE(:pPHONE_NUMBER,0,''WIO''); end;' USING OUT err, IN pPHONE_NUMBER;
      if regexp_instr(err,'Заявка на блок №\d+')>0 then return('API '||Err);end if;
   end if; 
...
--"9031111593","361826161","Группа счетов 1","Нет группы",,"Формула Свободы. Москва (FS_UNL_F )",,"Заблокированный","26.04.2012","Блокировка","20.09.2011",
          ...
          return(sMSG);
          exception
            when others then err:=err||' ParsErr^:nIndexField='||nIndexField||';Phone_Number='||rcStates.Phone_Number||';sFld='||sFld||';sTMP='||sTMP;
          end;
...
BEGIN
    ...
    --Проверяем нужно ли обновлять через АПИ...
    if strInLike(41,nvl(method,'0;'),';','()')=1 
    then 
     EXECUTE IMMEDIATE  'begin :err :=beeline_api_pckg.account_phone_options(:pAccount_id); end;' USING OUT err, IN pACCOUNT_ID;
  ...
  -------------------------------------
    ...
    --Загрузка данных в базу
     ...
     UPDATE BEELINE_LOADER_INF
        ...
        IF (sql%rowcount = 0)
            THEN insert into BEELINE_LOADER_INF(ACCOUNT_ID,PHONE_NUMBER,OBJ_ID,BAN) 
                 values (pACCOUNT_ID,regexp_substr(Tloader(i), '[^;]+', 1, 1),regexp_substr(Tloader(i), '[^;]+', 1, 2),
                                                                                                case is_collector
                                                                                                when 0 then ACCOUNT
                                                                                                else null
                                                                                                end
  ...
  for c_phones in(                  
   ...
   if c_phones.xml_error is not null or c_phones.xml_phone is null then 
           ...
           for c_ds in (
                  ...
                  table(XmlSequence(c_charges.Charges_xml.extract('charges/ChargeDetails/ChargeDetailsValue'))) dd
                    ...
                    case trim(regexp_substr(c_ds.CDV, '[^;]+', 1, 1))
                      when 'Корректировка абон. платы' then 
     ...
     --  when 'Включенные минуты' then ;
      ...
      ffb.single_payments:=nvl(ffb.single_payments,0);
