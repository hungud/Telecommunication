...
--
  ...
  CURSOR C_FIND_1 IS
    ...
    WHERE
      ...
      AND DB_LOADER_PAYMENTS.PHONE_NUMBER = pPHONE_NUMBER
...
BEGIN
  ...
  IF (NOT vFOUND)/*AND
    (pYEAR*100+pMONTH=TO_NUMBER(TO_CHAR(PPAYMENT_DATE,'YYYYMM'))
   ...
   BEGIN 
    ...
    EXCEPTION 
     WHEN OTHERS THEN 
       verr:=sqlerrm;
        /*  INSERT INTO MANY_LOGS (LOG_SOURCE_ID, YEAR_MONTH, PHONE_NUMBER, DATE_ON, TEXT_MESSAGES, NOTE)
          values
          (13,vYEAR_MONTH, pPHONE_NUMBER, sysdate, verr, 'Ошибка загрузки платежа от '||nvl(pPAYMENT_DATE,to_date('01.01.1900','dd.mm.yyyy'))||' № '||NVL(pPAYMENT_NUMBER,0) );*/
  INSERT INTO MANY_LOGS (LOG_SOURCE_ID, YEAR_MONTH, PHONE_NUMBER, DATE_ON, TEXT_MESSAGES, NOTE)
  Select 13,
     vYEAR_MONTH,
     pPHONE_NUMBER,
     sysdate,
     verr,
     'Ошибка загрузки платежа от ' ||
     nvl(to_char(pPAYMENT_DATE,'dd.mm.yyyy'), to_date('01.01.1900', 'dd.mm.yyyy')) || ' № ' ||
     NVL(pPAYMENT_NUMBER, 0)
           from dual where not exists
           (select 1 from  MANY_LOGS m where m.log_source_id=13 and m.YEAR_MONTH=vYEAR_MONTH and 
           nvl(m.phone_number,0)=nvl(pPHONE_NUMBER,0) and m.text_messages=verr 
           and m.note='Ошибка загрузки платежа от '||nvl(to_char(pPAYMENT_DATE,'dd.mm.yyyy'),to_date('01.01.1900','dd.mm.yyyy'))||' № '||NVL(pPAYMENT_NUMBER,0)); 
...
BEGIN
  ...
  INSERT INTO DB_LOADER_FULL_FINANCE_BILL(ACCOUNT_ID, YEAR_MONTH, PHONE_NUMBER, ABONKA, CALLS, SINGLE_PAYMENTS, DISCOUNTS, BILL_SUM, COMPLETE_BILL, 
                                          ...
                                          DISCOUNT_COUNT_ON_PHONES, DISCOUNT_OTHERS, 
    ...
    VALUES(vACCOUNT_ID, pYEAR_MONTH, pPHONE_NUMBER, pABONKA, pCALLS, pSINGLE_PAYMENTS, pDISCOUNTS, pBILL_SUM, pCOMPLETE_BILL,
           ...
           pDISCOUNT_COUNT_ON_PHONES, pDISCOUNT_OTHERS, 
  ...
  ELSE
    UPDATE DB_LOADER_FULL_FINANCE_BILL FB
      SET FB.ABONKA=pABONKA, FB.CALLS=pCALLS, FB.SINGLE_PAYMENTS=pSINGLE_PAYMENTS, FB.DISCOUNTS=pDISCOUNTS, FB.BILL_SUM=pBILL_SUM, FB.COMPLETE_BILL=pCOMPLETE_BILL, 
          ...
          FB.DISCOUNT_COUNT_ON_PHONES=pDISCOUNT_COUNT_ON_PHONES, FB.DISCOUNT_OTHERS=pDISCOUNT_OTHERS, 
