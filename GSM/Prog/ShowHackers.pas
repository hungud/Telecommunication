...
uses ShowUserStat, ContractsRegistration_Utils, Main, IntecExportGrid;
...
//Проставляем признак проверки
  if qHackers.Active and (qHackers.RecordCount > 0) then begin
...
begin
  ...
  {cbIndex := cbStatuses.ItemIndex;
  cbStatuses.Clear;
  cbStatuses.Items.Add('Все статусы');
  while not qStatuses.EOF do
    begin
      cbStatuses.Items.Add(
        qStatuses.FieldByName('CHECKED_TYPE_NAME').AsString);
      qStatuses.Next;
    end;
    if cbIndex > cbStatuses.Items.Count then
      cbStatuses.ItemIndex := 1
    else
      cbStatuses.ItemIndex := cbIndex; }
