...
type
  TEditContractForm = class(TForm)
    ...
    procedure PHONE_NUMBER_FEDERALKeyPress(Sender: TObject; var Key: Char);
    procedure PHONE_NUMBER_CITYKeyPress(Sender: TObject; var Key: Char);
    procedure PHONE_NUMBER_FEDERALKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure PHONE_NUMBER_CITYKeyUp(Sender: TObject; var Key: Word;
      Shift: TShiftState);
...
var
  ...
  procedure SetqTariffsParam(isAdm : boolean; IS_INSERT : Integer);
  begin
    qTariffs.ParamByName('IS_INSERT').Value := IS_INSERT;
    if isAdm then
      qTariffs.ParamByName('IS_ADMIN').Value := 1
    else
      qTariffs.ParamByName('IS_ADMIN').Value := 0;
  end;
...
{      else if (Trim(SIM_NUMBER.Text) <> '') and (Length(Trim(SIM_NUMBER.Text)) <> 20) then
      ...
      begin
        ...
        begin
          ...
          //
          qContracts.FieldByName('IS_CREDIT_CONTRACT').AsInteger := 0;
          //
...
begin
  ...
  begin
    //возможность изменения номера договора  и даты
    CONTRACT_NUM.ReadOnly := true;
    CONTRACT_DATE.Enabled := False;
  end
  else
  begin
    CONTRACT_NUM.ReadOnly := false;
    CONTRACT_DATE.Enabled := true;
  end;
...
begin
  ...
  begin
    ...
    begin
      CanClose := (IDYES = Application.MessageBox('Отменить ввод и закрыть форму?', Pchar(Application.Title), MB_YESNO + MB_ICONQUESTION) );
...
procedure TEditContractForm.PHONE_NUMBER_CITYKeyPress(Sender: TObject;
  var Key: Char);
begin
 if not (Key  in ['0'..'9', #08, #8, #13, #3, #22]) then
   Key := #0;
end;

procedure TEditContractForm.PHONE_NUMBER_CITYKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
IF StrIsNumber(PHONE_NUMBER_CITY.Text)= false THEN
 begin
 ShowMessage('Не правильно введен номер телефона');
 PHONE_NUMBER_CITY.Clear;
 end;
end;

procedure TEditContractForm.PHONE_NUMBER_FEDERALKeyPress(Sender: TObject;
  var Key: Char);
begin
 if not (Key  in ['0'..'9', #08, #8, #13, #3, #22]) then
   Key := #0;
...
procedure TEditContractForm.PHONE_NUMBER_FEDERALKeyUp(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
IF StrIsNumber(PHONE_NUMBER_FEDERAL.Text)= false THEN
 begin
 ShowMessage('Не правильно введен номер телефона');
 PHONE_NUMBER_FEDERAL.Clear;
 end;
end;
...
begin
  // Ручной биллинг
  DataSet.FieldByName('HANDS_BILLING').AsInteger := 0;
