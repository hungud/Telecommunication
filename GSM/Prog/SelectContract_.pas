...
type
  TSelectContractForm_1 = class(TForm)
    ...
    qMainPHONE: TStringField;
    rbIvideon: TsRadioButton;
    spSelectAbonent: TOraStoredProc;
    ...
    procedure dxMemData1AfterOpen(DataSet: TDataSet);
    procedure rbIvideonClick(Sender: TObject);
...
procedure SelectPhoneNumber_ (var rezPfone: string; var rezContractID : Integer; var rezAbonent: Integer; UseFilialBlockAccess : Boolean; oss: TOraSession; Filial: integer);
...
begin
  ...
  try
    ...
    begin
       ...
       rezAbonent := SelectContractFrm_1.FAbonent;
    end;
  finally
...
procedure TSelectContractForm_1.dxMemData1AfterOpen(DataSet: TDataSet);
begin
...
procedure TSelectContractForm_1.CRDBGrid1CellClick(Column: TColumn);
begin
  cur_id := qMain.FieldByName('CONTRACT_ID').AsInteger;
end;

procedure TSelectContractForm_1.CRDBGrid1DrawColumnCell(Sender: TObject;
...
var ContractId: integer;
    ...
    vSurname, vName, vPatronymic, vStr : String;
begin
if not fInitForm  then exit;

  if (rbIvideon.Checked and (Length(eSearch.Text) > 5)) then
   begin
     spSelectAbonent.ParamByName('pMAIL').AsString:=eSearch.Text;
     spSelectAbonent.ExecSQL;
     if spSelectAbonent.ParamByName('RESULT').AsInteger<>0 then
     begin
       FAbonent := spSelectAbonent.ParamByName('RESULT').AsInteger;
  ...
  begin
    ...
    if (rbFullSelect.Checked and (Length(eSearch.Text) > 0)) then
    begin
      begin
...
begin
  ...
  begin
    qMain.SQL.Text:='SELECT C.CONTRACT_NUM,'+#13#10+
                    ...
                    '    AP.PHONE_NUMBER_FEDERAL PHONE,'+#13#10+
...
procedure TSelectContractForm_1.rbIvideonClick(Sender: TObject);
begin
   sPanel2.Visible := false;
   height := 88;
 //  if fInitForm  then
