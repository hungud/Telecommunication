...
uses CommCtrl, ContractsRegistration_Utils,SelectContract_, SelectContract,  Main,
  ShowUserStat, LoadData, dmShowUserStat, ShowUserStat_IVIDEON;
  ...
  begin
    ...
    case qMain.FieldByName('DOCUM_TYPE_ID').AsInteger of
      TYPE_DOC_PAYMENT :
        begin
          if (not IsAdmin) then
            MessageDlg('Отсутствуют права на изменение платежей!',mtError, [mbOK], 0)
          else
          begin
  ...
  begin
    case qMain.FieldByName('DOCUM_TYPE_ID').AsInteger of
      TYPE_DOC_PAYMENT :
        begin
          if  (not IsAdmin)  then
            MessageDlg('Отсутствуют права на удаление платежей!',mtError, [mbOK], 0)
          else
          begin
      ...
      TYPE_DOC_CONTRACT_CANCEL :
        begin
          if  (not IsAdmin)  then
            MessageDlg('Отсутствуют права на удаление расторжения договора!',mtError, [mbOK], 0)
          else
          begin
...
begin
  ...
  try
    ...
    if (dmShowUserSt.qCheckUserPrivileges.FieldByName('RIGHT_CANCEL_CONTRACT').AsInteger = 0) AND (not IsAdmin)  then
      // скрытие возможности расторжения договора
      aCancelContract.Visible :=False
    else
      aCancelContract.Visible :=True;
    dmShowUserSt.qCheckUserPrivileges.Close;
  finally
...
begin
{
  ...
  begin
   ...
   SelectPhoneNumber_(vPhoneNumber, vContractID, vAbonent, MainForm.FUseFilialBlockAccess, MainForm.OraSession, MainForm.FFilial);
